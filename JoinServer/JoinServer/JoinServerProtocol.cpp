...
#include "..\\..\\Util\\MD5.h"
#include "..\\..\Util\libbcrypt\include\bcrypt\BCrypt.hpp"
...
void JoinServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		//LogAdd(LOG_GREEN, "Protocol [%x]", head);
		case 0x00:
		...
		case 0x30:
			...
			break;
		case 0x40:
			GJSetOffExpRecv((SDHP_SET_OFFEXP_RECV*)lpMsg,index);
			break;
		case 0x41:
			JGLastVoteTimeRecv((SDHP_ACCOUNT_VOTE_TIME_RECV*)lpMsg,index);
			break;
		case 0x42:
			GJLastVoteTimeSave((SDHP_ACCOUNT_VOTE_TIME_SAVE*)lpMsg,index);
			break;
		case 0x43:
			GJSetBlockCodeRecv((SDHP_SET_BLOCK_CODE_RECV*)lpMsg,index);
			break;
	}
...
void GJConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg,int index) // OK
{
	...
	memcpy(pMsg.password, lpMsg->password, sizeof(pMsg.password));
	...
	{
		if (AccountInfo.isOffExp == 0)
		{
			LogAdd(LOG_RED, "isOffExp = 0 wtf [%s]",AccountInfo.Account);
			pMsg.result = 3;
			gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
			JGAccountAlreadyConnectedSend(AccountInfo.GameServerCode,AccountInfo.UserIndex,AccountInfo.Account, pMsg.password);
			return;
		}
		//JGExternalDisconnectAccountSend(AccountInfo.GameServerCode,AccountInfo.UserIndex,AccountInfo.Account);

		//gQueryManager.ExecQuery("EXEC WZ_DISCONNECT_MEMB '%s'",AccountInfo.Account);

		//gQueryManager.Close();

		//gSocketManager.DataSend(AccountInfo.ServerIndex,(BYTE*)&pMsg,pMsg.header.size);

		//gAccountManager.RemoveAccountInfo(AccountInfo);

		JGDisconnectAccountSend(AccountInfo.Account);
		LogAdd(LOG_RED,"Successfully disconnected offexp user [%s]",AccountInfo.Account);
	}
	...
	if(MD5Encryption == 0 && BCryptEncryption == 0)
	{
	...
	else if (BCryptEncryption == 1) 
	{
		...
		if (BCryptHash.validatePassword(lpMsg->password, password) == 0)
		{
			gQueryManager.Close();
			pMsg.result = 0;
			gSocketManager.DataSend(index, (BYTE*)&pMsg, pMsg.header.size);
	...
	else
	{
		...
		unsigned char password[17] = {0};

		gQueryManager.GetAsString("memb__pwd",(char*)password,sizeof(password));
	...
	if(gQueryManager.ExecQuery("SELECT sno__numb,bloc_code,region FROM MEMB_INFO WHERE memb___id='%s'",lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
		pMsg.result = 2;
		gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
	...
	gQueryManager.GetAsString("region",pMsg.region,sizeof(pMsg.region));
	...
	AccountInfo.ServerIndex = index;
	AccountInfo.isOffExp = 0;
	memcpy(AccountInfo.region,pMsg.region,sizeof(AccountInfo.region));
...
void JGDisconnectAccountSend(char* account)
{
	...
	gAccountManager.GetAccountInfo(&AccountInfo,account);

	pMsg.index = AccountInfo.UserIndex;

	memcpy(pMsg.account,account,sizeof(pMsg.account));

	pMsg.result = 1;

	gQueryManager.ExecQuery("EXEC WZ_DISCONNECT_MEMB '%s'",account);
...
void JGDisconnectAccountOffExpSend(char* account)
{
	SDHP_DISCONNECT_ACCOUNT_SEND pMsg;
	...
	gLog.Output(LOG_ACCOUNT,"[AccountInfo] Account OffExp disconnected (Account: %s, IpAddress: %s, GameServerCode: %d)",AccountInfo.Account,AccountInfo.IpAddress,AccountInfo.GameServerCode);
}
...
void GJSetOffExpRecv(SDHP_SET_OFFEXP_RECV* lpMsg,int index)
{
	ACCOUNT_INFO AccountInfo;
	...
	if(gAccountManager.GetAccountInfo(&AccountInfo,lpMsg->account) == 0)
	{
		//pMsg.result = 0;
		//gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
		LogAdd(LOG_RED, "[GJSetOffExpRecv] Can't find Account [%s]",lpMsg->account);
		return;
	}

	if(AccountInfo.UserIndex != lpMsg->index)
	{
		LogAdd(LOG_RED, "[GJSetOffExpRecv] Incorrect user index [%s] [%d - %d]",lpMsg->account,AccountInfo.UserIndex,lpMsg->index);
		//pMsg.result = 0;
		//gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
	...
	if(AccountInfo.GameServerCode != gServerManager[index].m_ServerCode)
	{
		LogAdd(LOG_RED, "[GJSetOffExpRecv] Incorrect gameserver code [%s] [%d - %d]",lpMsg->account,AccountInfo.GameServerCode,gServerManager[index].m_ServerCode);
		//pMsg.result = 0;
		//gSocketManager.DataSend(index,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}

	AccountInfo.Map = lpMsg->map;
	AccountInfo.X = lpMsg->x;
	AccountInfo.Y = lpMsg->y;
	AccountInfo.isOffExp = lpMsg->isOffExp;

	gAccountManager.InsertAccountInfo(AccountInfo);

	LogAdd(LOG_RED,"Successfully set OffExp = %d [%s]",lpMsg->isOffExp,AccountInfo.Account);
}
...
void GJMapServerMoveAuthRecv(SDHP_MAP_SERVER_MOVE_AUTH_RECV* lpMsg,int index) // OK
{
	...
	memcpy(pMsg.region,AccountInfo.region,sizeof(pMsg.region));
...
void JGAccountAlreadyConnectedSend(int GameServerCode,int UserIndex,char* account, char* password) // OK
{
	CServerManager* lpServerManager = FindServerByCode(GameServerCode);
...
void JGLastVoteTimeRecv(SDHP_ACCOUNT_VOTE_TIME_RECV* lpMsg, int index)
{
	ACCOUNT_INFO AccountInfo;
	...
	if(gQueryManager.ExecQuery("SELECT LastVoteTime FROM MEMB_INFO WHERE memb___id =  '%s'",lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
	...
	{
		pMsg.last_vote_time = gQueryManager.GetAsInteger("LastVoteTime");
...
void GJLastVoteTimeSave(SDHP_ACCOUNT_VOTE_TIME_SAVE* lpMsg, int index)
{
	gQueryManager.ExecQuery("UPDATE MEMB_INFO SET LastVoteTime = %d WHERE memb___id = '%s'",lpMsg->last_vote_time, lpMsg->account);
	gQueryManager.Close();
}

void GJSetBlockCodeRecv(SDHP_SET_BLOCK_CODE_RECV* lpMsg, int index)
{
	gQueryManager.ExecQuery("UPDATE MEMB_INFO SET bloc_code = %d WHERE memb___id = '%s'",lpMsg->BlockCode, lpMsg->account);
	gQueryManager.Close();
}