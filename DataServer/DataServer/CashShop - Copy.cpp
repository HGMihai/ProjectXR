...
bool CCashShop::BankExReqPoint(char * AccountID, BANKEX_DG_GET_POINT * Result)
{
	...
	if( gQueryManager.ExecQuery("SELECT * FROM BankEx WHERE AccountID = '%s'", AccountID) == 0 || gQueryManager.Fetch() == SQL_NO_DATA )
	{
		...
		Result->Harmony		= 0;
		// ----
		if( gQueryManager.ExecQuery("INSERT INTO BankEx (AccountID, Credits, Bless, Soul, Life) VALUES ('%s', %d, %d, %d, %d)", AccountID, 0, 0, 0, 0) == 0 )
		{
			return false;
		}
		// ----
	...
	{
		Result->Credits		= gQueryManager.GetAsInteger("Credits");
...
void CCashShop::BankExSavePoint(BANKEX_GD_SAVE_POINT * aRecv)
{
	//CString Query;
	// ----
	//Query.Format("UPDATE BankEx SET Credits = %d, Bless = %d, Soul = %d, Chaos = %d, Creation = %d, Life = %d, Harmony = %d WHERE AccountID = '%s'", aRecv->Credits, aRecv->Bless, aRecv->Soul, aRecv->Chaos, aRecv->Creation, aRecv->Life, aRecv->Harmony ,aRecv->AccountID);
	// ----
	if( gQueryManager.ExecQuery("UPDATE BankEx SET Credits = %d, Bless = %d, Soul = %d, Chaos = %d, Creation = %d, Life = %d, Harmony = %d WHERE AccountID = '%s'", aRecv->Credits, aRecv->Bless, aRecv->Soul, aRecv->Chaos, aRecv->Creation, aRecv->Life, aRecv->Harmony ,aRecv->AccountID) == 0 )
	{
		LogAdd(LOG_RED, "[%s] Can't save points! Account: %s", __FUNCTION__, aRecv->AccountID);
		gQueryManager.Close();
		return;
	}
	// ----
