...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	wsprintf(buff,"[%s %s] (QueueSize : %d)",DATASERVER_TITLE, DATASERVER_VERSION,0);
	...
	if(WSAStartup(MAKEWORD(2,2),&wsa) == 0)
	{
		...
		GetPrivateProfileString("Config","BankExTable","BankEx",gCashShop.BankExTable,sizeof(gCashShop.BankExTable),".\\DataServer.ini");

		GetPrivateProfileString("Config","BankExColumn","Credits",gCashShop.BankExColumn,sizeof(gCashShop.BankExColumn),".\\DataServer.ini");

		GetPrivateProfileString("Config","BankExAccount","AccountID",gCashShop.BankExAccount,sizeof(gCashShop.BankExAccount),".\\DataServer.ini");

		WORD DataServerPort = GetPrivateProfileInt("DataServerInfo","DataServerPort",55960,".\\DataServer.ini");

		int WriteLogs = GetPrivateProfileInt("Config","WriteLogs",0,".\\DataServer.ini");

		gQueryData.load(".\\Query.txt");

		if(gQueryManager.Connect(DataServerODBC,DataServerUSER,DataServerPASS, WriteLogs) == 0)
		{
		...
		else
		{
			...
			else
			{
				...
				gQueryManager.ExecQuery("UPDATE MEMB_STAT SET ConnectStat = 0");
				gQueryManager.Close();
