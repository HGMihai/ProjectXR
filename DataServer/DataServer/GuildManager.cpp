...
BYTE CGuildManager::AddGuild(int index,char* szGuildName,char* szMasterName,BYTE* lpMark,BYTE btType)
{
	...
	else
	{
		if(gQueryManager.GetResult(0) == 0)
		{
			...
			gQueryManager.ExecQuery("UPDATE Guild SET G_Mark=?,G_Type=%d,Warehouse=CONVERT(varbinary(%d),REPLICATE(char(0xFF),%d)) WHERE G_Name='%s'",btType,WAREHOUSE_SIZE*ITEM_BYTES_SIZE,WAREHOUSE_SIZE*ITEM_BYTES_SIZE,szGuildName);

			gQueryManager.Close();

			gQueryManager.ExecQuery("UPDATE GuildMember SET G_Status=%d, G_WareStatus=1 WHERE Name='%s'",0x80,szMasterName);
