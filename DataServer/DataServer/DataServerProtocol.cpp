...
void DataServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x05:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				case 0x00:
					...
					break;
				case 0x10:
					gGuildWarehouse.GDWarehouseItemRecv((SDHP_GUILDWAREHOUSE_ITEM_RECV*)lpMsg,index);
					break;
				case 0x15:
					gGuildWarehouse.GDWarehouseItemSaveRecv((SDHP_GUILDWAREHOUSE_ITEM_SAVE_RECV*)lpMsg);
					break;
				case 0x30:
		...
		case 0xFB:
			...
			{
				case 0x00:
					gDiabloDBSet.DSDB_GetGuildList(index);
					break;
				case 0x01:
					gDiabloDBSet.DSDB_ChangeDate((GD_DIABLO_CHANGEDATE*)lpMsg);
					break;
				case 0x05:
				...
				case 0x06:
					...
					break;
				case 0x07:
					gGuildWarehouse.GDWareStatusRecv((GD_GET_WARE_SATUS_RECV*)lpMsg,index);
					//gOfflineData.OfflineDataRecv((GD_OFFLINE_DATA_RECV*)lpMsg,index);
					break;
				case 0x08:
					gGuildWarehouse.GDUsersWareStatusSave((PMSG_WARE_SATUS_SEND*)lpMsg);
					//gOfflineData.OfflineDelRecv((GD_OFFLINE_DATA_DEL_RECV*)lpMsg,index);
					break;
				case 0x09:
					gGuildWarehouse.GDCheckAllowWarehouse((PMSG_STATUS_CHECK_SEND*)lpMsg,index);
					break;
				case 0x0A:
					GDCustomStartItemStateRecv((PMSG_GD_STARTITEM_STATE_RECV*)lpMsg);
					break;
				case 0x0B:
				...
				case 0x0C:
					...
					break;
				case 0x0D:
					gRankings.GDRecvRequest((PMSG_DB_REQ_SEND*)lpMsg,index);
					break;
				case 0x0E:
					gRankings.GDCharacterInfoRequest((GD_RANKINGS_INFO_REQUEST_RECV*)lpMsg,index);
					break;
				case 0x0F:
					GDSetCharacterBlockRecv((PMSG_SET_CHARACTER_BLOCK_RECV*)lpMsg);
					break;
				case 0x10: GDRecvEntryCountRequest((PMSG_ENTRY_COUNT_REQ_RECV*)lpMsg, index); break;
				case 0x11: GDRecvEntryCountUpdate((PMSG_ENTRY_COUNT_UPDATE_RECV*)lpMsg); break;
			}
			break;
		case 0xE2:
			gDiabloDBSet.DSDB_SaveGuildList((GD_DIABLO_GUILDLIST*)lpMsg);
			break;
		case 0xE3:
		...
		case 0xEC:
		{
			auto lpDef = (PMSG_DEFAULT2*)lpMsg;
			//g_ExGDManager.Protocol(lpDef->subcode, aRecv, aIndex);
			g_ItemMarket.Protocol(lpDef->subcode, lpMsg, index);
		}
		break;
	}
...
void GDCharacterListMultiRecv(SDHP_CHARACTER_LIST_MULTI_RECV* lpMsg, int index)
{
	...
	for(int n=0;n < 5;n++)
	{
		...
		else
		{
			...
			BYTE Inventory[12][ITEM_BYTES_SIZE];
			...
			info.GReset = gQueryManager.GetAsInteger("MasterResetCount");
...
void GDCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg,int index) // OK
{
	...
	else
	{
		...
		pMsg.NewStat = gQueryManager.GetAsFloat("NewStat");
		...
		pMsg.PointsCount = gQueryManager.GetAsInteger("PointsCount");

		pMsg.PointsTime = gQueryManager.GetAsInteger("PointsTime");

		pMsg.ChatLimitTime = gQueryManager.GetAsInteger("ChatLimitTime");

		pMsg.IsStartItem = gQueryManager.GetAsInteger("IsStartItem");

		pMsg.BCEntryCount = gQueryManager.GetAsInteger("BCEntryCount");

		pMsg.MapQuestSystemMap = gQueryManager.get("MapQuestSystemMap").as_int();
		pMsg.MapQuestSystemIndex = gQueryManager.get("MapQuestSystemIndex").as_int();
		pMsg.MapQuestSystemState = gQueryManager.get("MapQuestSystemState").as_int();

		gQueryManager.GetAsString("BCLastEntry", pMsg.BCLastEntry, sizeof(pMsg.BCLastEntry));

		//pMsg.Rank = (BYTE)gQueryManager.GetAsInteger("Rank");
...
void GDCreateItemRecv(SDHP_CREATE_ITEM_RECV* lpMsg,int index) // OK
{
	...
	if (lpMsg->HaveSerial == 0)
	{
		pMsg.Serial = 0;
	}
	else if(gQueryManager.ExecQuery("EXEC WZ_GetItemSerial") == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
...
#ifdef IS_NEW_CLIENT
		pMsg.game_settings = gQueryManager.GetAsInteger("game_settings");
#endif
		#if(DATASERVER_UPDATE>=701)
...
void GDCharacterInfoSaveRecv(SDHP_CHARACTER_INFO_SAVE_RECV* lpMsg) // OK
{
	...
	gQueryManager.ExecQuery("UPDATE Character SET cLevel=%d,Class=%d,LevelUpPoint=%d,Experience=%d,Strength=%d,Dexterity=%d,Vitality=%d,Energy=%d,Leadership=%d,NewStat=%d,Inventory=?,MagicList=?,Money=%d,Life=%f,MaxLife=%f,Mana=%f,MaxMana=%f,BP=%f,MaxBP=%f,Shield=%f,MaxShield=%f,MapNumber=%d,MapPosX=%d,MapPosY=%d,MapDir=%d,PkCount=%d,PkLevel=%d,PkTime=%d,Quest=?,EffectList=?,FruitAddPoint=%d,FruitSubPoint=%d,ExtInventory=%d, IsMarried=%d, MarryName='%s', GensExitTime=%d, QuestIndex=%d, QuestState=%d, QuestVar=%d, QuestStat=%d, ChatLimitTime=%d, PointsCount=%d, PointsTime=%d WHERE AccountID='%s' AND Name='%s'",lpMsg->Level,lpMsg->Class,lpMsg->LevelUpPoint,lpMsg->Experience,lpMsg->Strength,lpMsg->Dexterity,lpMsg->Vitality,lpMsg->Energy,lpMsg->Leadership,lpMsg->NewStat,lpMsg->Money,(float)lpMsg->Life,(float)lpMsg->MaxLife,(float)lpMsg->Mana,(float)lpMsg->MaxMana,(float)lpMsg->BP,(float)lpMsg->MaxBP,(float)lpMsg->Shield,(float)lpMsg->MaxShield,lpMsg->Map,lpMsg->X,lpMsg->Y,lpMsg->Dir,lpMsg->PKCount,lpMsg->PKLevel,lpMsg->PKTime,lpMsg->FruitAddPoint,lpMsg->FruitSubPoint,lpMsg->ExtInventory, lpMsg->IsMarried, lpMsg->MarryName, lpMsg->GensExitTick, lpMsg->iQuestIndex, lpMsg->iQuestState, lpMsg->iQuestVar, lpMsg->iQuestStat, lpMsg->ChatLimitTime, lpMsg->PointsCount, lpMsg->PointsTime, lpMsg->account,lpMsg->name);
	...
	gQueryManager.BindParameterAsBinary(3,lpMsg->Quest,sizeof(lpMsg->Quest));
	gQueryManager.BindParameterAsBinary(4,lpMsg->Effect[0],sizeof(lpMsg->Effect));
	gQueryManager.ExecQuery("UPDATE Character SET cLevel=%d,Class=%d,LevelUpPoint=%d,Experience=%d,Strength=%d,Dexterity=%d,Vitality=%d,Energy=%d,Leadership=%d,Inventory=?,MagicList=?,Money=%d,Life=%f,MaxLife=%f,Mana=%f,MaxMana=%f,BP=%f,MaxBP=%f,Shield=%f,MaxShield=%f,MapNumber=%d,MapPosX=%d,MapPosY=%d,MapDir=%d,PkCount=%d,PkLevel=%d,PkTime=%d,Quest=?,EffectList=?,FruitAddPoint=%d,FruitSubPoint=%d, IsMarried=%d, MarryName='%s', GensExitTime=%d WHERE AccountID='%s' AND Name='%s'",lpMsg->Level,lpMsg->Class,lpMsg->LevelUpPoint,lpMsg->Experience,lpMsg->Strength,lpMsg->Dexterity,lpMsg->Vitality,lpMsg->Energy,lpMsg->Leadership,lpMsg->Money,(float)lpMsg->Life,(float)lpMsg->MaxLife,(float)lpMsg->Mana,(float)lpMsg->MaxMana,(float)lpMsg->BP,(float)lpMsg->MaxBP,(float)lpMsg->Shield,(float)lpMsg->MaxShield,lpMsg->Map,lpMsg->X,lpMsg->Y,lpMsg->Dir,lpMsg->PKCount,lpMsg->PKLevel,lpMsg->PKTime,lpMsg->FruitAddPoint,lpMsg->FruitSubPoint, lpMsg->IsMarried, lpMsg->MarryName, lpMsg->GensExitTick, lpMsg->account,lpMsg->name);
	gQueryManager.Close();
	...
	gQueryManager.ExecQuery("Update Character set MapQuestSystemMap=%d, MapQuestSystemIndex=%d, MapQuestSystemState=%d  where AccountID='%s' and Name='%s'",
		lpMsg->MapQuestSystemMap, lpMsg->MapQuestSystemIndex, lpMsg->MapQuestSystemState, lpMsg->account, lpMsg->name);
...
void GDOptionDataSaveRecv(SDHP_OPTION_DATA_SAVE_RECV* lpMsg) // OK
{
	if(gQueryManager.ExecQuery("SELECT Name FROM OptionData WHERE Name='%s'",lpMsg->name) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		...
		gQueryManager.BindParameterAsBinary(1,lpMsg->SkillKey,sizeof(lpMsg->SkillKey));
#ifdef IS_NEW_CLIENT
		gQueryManager.ExecQuery("INSERT INTO OptionData (Name,SkillKey,GameOption,Qkey,Wkey,Ekey,ChatWindow,Rkey,QWERLevel, game_settings) VALUES ('%s',?,%d,%d,%d,%d,%d,%d,%d,%d)", lpMsg->name, lpMsg->GameOption, lpMsg->QKey, lpMsg->WKey, lpMsg->EKey, lpMsg->ChatWindow, lpMsg->RKey, lpMsg->QWERLevel, lpMsg->game_settings);
#else
	...
	else
	{
		...
		gQueryManager.BindParameterAsBinary(1,lpMsg->SkillKey,sizeof(lpMsg->SkillKey));
#ifdef IS_NEW_CLIENT
		gQueryManager.ExecQuery("UPDATE OptionData SET SkillKey=?,GameOption=%d,Qkey=%d,Wkey=%d,Ekey=%d,ChatWindow=%d,Rkey=%d,QWERLevel=%d, game_settings=%d WHERE Name='%s'", lpMsg->GameOption, lpMsg->QKey, lpMsg->WKey, lpMsg->EKey, lpMsg->ChatWindow, lpMsg->RKey, lpMsg->QWERLevel, lpMsg->game_settings, lpMsg->name);
#else
...
{
	gQueryManager.ExecQuery("EXEC WZ_TvTRanking '%s','%s','%d','%d','%d'",lpMsg->account,lpMsg->name,lpMsg->number,lpMsg->killcount,lpMsg->deathcount);
	gQueryManager.Fetch();
	gQueryManager.Close();
}
...
void GDCustomStartItemStateRecv(PMSG_GD_STARTITEM_STATE_RECV* lpMsg)
{
	gQueryManager.ExecQuery("UPDATE Character SET IsStartItem = %d WHERE AccountID = '%s' and Name = '%s'", lpMsg->state, lpMsg->Account,lpMsg->Name);
	gQueryManager.Close();
}

void GDSetCharacterBlockRecv(PMSG_SET_CHARACTER_BLOCK_RECV* lpMsg)
{
	gQueryManager.ExecQuery("UPDATE Character SET CtlCode = %d WHERE AccountID = '%s' and Name = '%s'", lpMsg->ctlCode, lpMsg->account,lpMsg->name);
	gQueryManager.Close();
}
...
void DGAchievementsLoad(PMSG_GDREQ_ACH_LOAD* lpMsg, int aIndex)
{
	...
	pMsg.h.set(0xFB,0x04,sizeof(pMsg));
...
void GDRecvEntryCountRequest(PMSG_ENTRY_COUNT_REQ_RECV* lpMsg, int aIndex)
{
	PMSG_ENTRY_COUNT_REQ_SEND pMsg{};

	pMsg.header.set(0xFB, 0x07, sizeof(pMsg));

	pMsg.lpObjAddr = lpMsg->lpObjAddr;
	...
	if(lpMsg->type == 0)
	{
		sprintf(text, "EXEC WZ_GetBloodCastleEntryCount '%s','%s'", lpMsg->account, lpMsg->name);
	}
	else
	{
		sprintf(text, "EXEC WZ_GetDevilSquareEntryCount '%s','%s'", lpMsg->account, lpMsg->name);
	}
	
	if (gQueryManager.ExecQuery(text) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
	...
	{
		pMsg.EntryCount = gQueryManager.GetAsInteger("EntryCount");

		gQueryManager.GetAsString("LastEntry", pMsg.LastEntryTime, sizeof(pMsg.LastEntryTime));
...
void GDRecvEntryCountUpdate(PMSG_ENTRY_COUNT_UPDATE_RECV* lpMsg)
{
	if(lpMsg->type == 0)
		gQueryManager.ExecQuery("update Character set BCEntryCount = BCEntryCount + 1, BCLastEntry = getdate() where AccountID = '%s' and Name = '%s'",lpMsg->account,lpMsg->name);
	else
		gQueryManager.ExecQuery("update Character set DSEntryCount = DSEntryCount + 1, DSLastEntry = getdate() where AccountID = '%s' and Name = '%s'", lpMsg->account, lpMsg->name);
}
