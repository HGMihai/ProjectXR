...
CDevilSquare::CDevilSquare() // OK
{
	...
	memset(this->m_DevilSquareRewardML,0,sizeof(this->m_DevilSquareRewardML));
}
		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 2)
				{
					...
					if (DS_LEVEL_RANGE(level) != false)
					{
						gDEVILSQUARE_LEVEL[level].NormalCharacterMinLevel = lpMemScript->GetAsNumber();
						gDEVILSQUARE_LEVEL[level].NormalCharacterMaxLevel = lpMemScript->GetAsNumber();
						gDEVILSQUARE_LEVEL[level].SpecialCharacterMinLevel = lpMemScript->GetAsNumber();
						gDEVILSQUARE_LEVEL[level].SpecialCharacterMaxLevel = lpMemScript->GetAsNumber();
					}
				}
				else if(section == 3)
				{
				...
				{
					/*if (gProtection.m_Index != eCustomers::NationMu && gProtection.m_Index != eCustomers::WorkPC)
					{
						break;
					}*/
					...
					for(int n=0;n < MAX_DS_RANK;n++)
					{
						this->m_DevilSquareRewardML[level][n] = lpMemScript->GetAsNumber();
					}
				}
				else
				{
...
int CDevilSquare::GetUserAbleLevel(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Class == CLASS_MG || lpObj->Class == CLASS_DL || lpObj->Class == CLASS_RF)
	{
		if(lpObj->ChangeUp < 2)
		{
			if (lpObj->Level >= gDEVILSQUARE_LEVEL[0].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[0].SpecialCharacterMaxLevel)
				level = 0;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[1].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[1].SpecialCharacterMaxLevel)
				level = 1;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[2].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[2].SpecialCharacterMaxLevel)
				level = 2;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[3].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[3].SpecialCharacterMaxLevel)
				level = 3;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[4].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[4].SpecialCharacterMaxLevel)
				level = 4;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[5].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[5].SpecialCharacterMaxLevel)
				level = 5;
		}
		else
			level = 6;
	...
	else
	{
		if(lpObj->ChangeUp < 2)
		{
			if (lpObj->Level >= gDEVILSQUARE_LEVEL[0].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[0].NormalCharacterMaxLevel)
				level = 0;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[1].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[1].NormalCharacterMaxLevel)
				level = 1;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[2].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[2].NormalCharacterMaxLevel)
				level = 2;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[3].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[3].NormalCharacterMaxLevel)
				level = 3;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[4].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[4].NormalCharacterMaxLevel)
				level = 4;
			else if (lpObj->Level >= gDEVILSQUARE_LEVEL[5].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[5].NormalCharacterMaxLevel)
				level = 5;
		}
		else
			level = 6;
...
int CDevilSquare::GetUserAbleLevelNew(LPOBJ lpObj) // OK
{
	if(lpObj->Class == CLASS_MG || lpObj->Class == CLASS_DL || lpObj->Class == CLASS_RF)
	{
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[0].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[0].SpecialCharacterMaxLevel)
			return 0;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[1].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[1].SpecialCharacterMaxLevel)
			return 1;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[2].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[2].SpecialCharacterMaxLevel)
			return 2;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[3].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[3].SpecialCharacterMaxLevel)
			return 3;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[4].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[4].SpecialCharacterMaxLevel)
			return 4;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[5].SpecialCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[5].SpecialCharacterMaxLevel)
			return 5;
		if (lpObj->Level >= 400)
			return 6;
	}
	else
	{
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[0].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[0].NormalCharacterMaxLevel)
			return 0;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[1].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[1].NormalCharacterMaxLevel)
			return 1;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[2].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[2].NormalCharacterMaxLevel)
			return 2;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[3].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[3].NormalCharacterMaxLevel)
			return 3;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[4].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[4].NormalCharacterMaxLevel)
			return 4;
		if (lpObj->Level >= gDEVILSQUARE_LEVEL[5].NormalCharacterMinLevel && lpObj->Level <= gDEVILSQUARE_LEVEL[5].NormalCharacterMaxLevel)
			return 5;
		if (lpObj->Level >= 400)
			return 6;
	}
...
void CDevilSquare::GiveUserRewardMoney(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_DS_USER;n++)
	{
		...
		//if (gProtection.m_Index == eCustomers::NationMu || gProtection.m_Index == eCustomers::WorkPC)
		//{
		//	if (gObj[lpLevel->User[n].Index].MasterLevel+this->m_DevilSquareRewardML[lpLevel->Level][lpLevel->User[n].Rank] <= gServerInfo.m_MasterSkillTreeMaxLevel)
		//	{
		//		gObj[lpLevel->User[n].Index].MasterLevel += this->m_DevilSquareRewardML[lpLevel->Level][lpLevel->User[n].Rank];
		//		gObj[lpLevel->User[n].Index].MasterPoint += this->m_DevilSquareRewardML[lpLevel->Level][lpLevel->User[n].Rank] * gServerInfo.m_MasterSkillTreePoint;

		//		//gMasterSkillTree.GCMasterSkillListSend(lpLevel->User[n].Index);
		//		gMasterSkillTree.GCMasterInfoSend(&gObj[lpLevel->User[n].Index]);
		//		LogAdd(LOG_GREEN,"[%s - %s] Rank %d Get %d ML levels in DevilSquare",gObj[lpLevel->User[n].Index].Account,gObj[lpLevel->User[n].Index].Name, lpLevel->User[n].Rank,this->m_DevilSquareRewardML[lpLevel->Level][lpLevel->User[n].Rank]);
		//	}
		//}
	}
}
...
void CDevilSquare::NpcCharon(LPOBJ lpObj) // OK
{
	if(gServerInfo.m_DevilSquareEvent == 0)
	{
		return;
	...
	if(gServerInfo.m_DevilSquareNewLevelSettings)
		level = this->GetUserAbleLevelNew(lpObj);
	else
		level = this->GetUserAbleLevel(lpObj);
	...
	if (lpObj->DSEntryCount >= gServerInfo.m_DevilSquareMaxDayEntry[lpObj->AccountLevel])
	{
		GCServerCommandSend(lpObj->Index, 63, lpObj->DSEntryCount, gServerInfo.m_DevilSquareMaxDayEntry[lpObj->AccountLevel]);
		return;
	}

	GCEntryCountSend(lpObj, 1);
...
void CDevilSquare::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gServerInfo.m_DevilSquareNewLevelSettings)
		level = this->GetUserAbleLevelNew(lpTarget);
	else
		level = this->GetUserAbleLevel(lpTarget);
...
void CDevilSquare::start_now()
{
	...
	LogAdd(LOG_GREEN,"[Set DS Start] At %2d:%2d:00",hour,minute);
