	...
	if ( lpMsg->Result == 1 || lpMsg->Result == 4)
	{
		...
		for ( int n=0;n<MAX_GUILD_USER;n++)
		{
			...
			if ( lpGuild->Use[n] > 0 && iGuildMemberIndex != -1 )
			{
				...
				gEffectManager.DelEffect(lpObj,EFFECT_GUILD_BUFF);
...
void GDGuildMemberAddResult(SDHP_GUILDMEMBERADD_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		if ( HereUserIndex >= 0 && lpNode != NULL )
		{
			...
			gGuildClass.AddGuildBuff(gObj[HereUserIndex].Guild);
		}
	}
}
				...
				if ( gObj[iMember].GuildNumber > 0 )
				{
					if ( gObj[iMember].Name[0] == szMember[0] && gObj[iMember].Name[1] == szMember[1] )
					{
						if ( strcmp(gObj[iMember].Name, szMember) == 0 )
						{
							...
							gEffectManager.DelEffect(&gObj[iMember],EFFECT_GUILD_BUFF);
			...
			if ( gObj[n].Name[0] == lpMsg->MemberID[0] )
			{
				if ( strcmp(lpMsg->MemberID, gObj[n].Name ) == 0 )
				{
					...
					if ( gObj[n].Guild != NULL )
					{
						...
						gGuildClass.AddGuildBuff(gObj[n].Guild);

						/*int value = gGuildClass.GetGuildBonus(&gObj[n]);

						for(int i=0;i < MAX_GUILD_USER;i++)
						{
							if(gObj[n].Guild->Use[i] > 0)
							{
								int index = gObj[n].Guild->Index[i];
						...
						/*if (gEffectManager.CheckEffect(&gObj[n],EFFECT_GUILD_BUFF))
						{
							gEffectManager.DelEffect(&gObj[n],EFFECT_GUILD_BUFF);
						}
						...
						if (value != 0)
						{
							gEffectManager.AddEffect(&gObj[n],0,EFFECT_GUILD_BUFF,0,value,0,0,0);
						}		*/				
						//
					}
					else
...
void DGGuildAnsAssignStatus(EXSDHP_GUILD_ASSIGN_STATUS_RESULT * lpMsg)
{
	if ( lpMsg->btFlag == 1 )
	{
		...
		//gGuild.CGGuildListRecv(lpMsg->wUserIndex);
