...
void CAchievements::Load()
{
	...
	if (!gProtection.GetCustomState(CUSTOM_ACHIEVEMENTS))
	{
		return;
	}
	
	this->Read(gPath.GetFullPath("..\\LTPData\\Achievements.ini"));
...
void CAchievements::Read(char* path)
{
	...
	for(auto item = mission.child("info"); item; item=item.next_sibling())
	{
		...
		info.itemReward.clear();

		if (info.RewardType == REWARD_ITEMS)
		{
			auto itemChild = item.child("item");
			info.itemReward.itemIndex = itemChild.attribute("index").as_int(0);
			info.itemReward.itemLevel = itemChild.attribute("level").as_int(0);
			info.itemReward.itemSkill = itemChild.attribute("skill").as_int(0);
			info.itemReward.itemLuck = itemChild.attribute("luck").as_int(0);
			info.itemReward.itemOption = itemChild.attribute("option").as_int(0);
			info.itemReward.itemExcellentOption = itemChild.attribute("excellent").as_int(0);
			info.itemReward.itemCount = itemChild.attribute("count").as_int(0);
		}
...
void CAchievements::GDPlayerLoad(int aIndex)
{
	...
	pMsg.h.set(0xFB, 0x0B, sizeof(pMsg));
	pMsg.aIndex = aIndex;
...
void CAchievements::GCSettingsData(int aIndex)
{
	...
	pMsg.h.set(0xFB, 0x90, sizeof(pMsg));
	...
	for(int i=0;i<ACHIEVEMENTS_COUNT;i++)
	{
		...
		pMsg.Data[i].itemReward = this->m_AchInfo.at(i).itemReward;
		//LogAdd(LOG_TEST,"Counter = %d Text = %s",pMsg.Data[i].counter,pMsg.Data[i].Name);
	}
...
void CAchievements::GCPlayerData(int aIndex, bool bWin)
{
	...
	pMsg.h.set(0xFB, 0x91, sizeof(pMsg));
	pMsg.bWinOpen = bWin;
	memcpy(&pMsg.counter, &lpUser->ach.counter, sizeof(pMsg.counter));
	DataSend(aIndex, (LPBYTE)&pMsg, sizeof(pMsg));
}
...
void CAchievements::PlayerReward(int aIndex, int missionIndex)
{
	...
	if (lpObj->ach.counter[missionIndex] >= this->m_AchInfo.at(missionIndex).Count)
	{
		...
		if (this->m_AchInfo.at(missionIndex).RewardType == REWARD_ZEN) // Zen
		{
		...
		else if (this->m_AchInfo.at(missionIndex).RewardType == REWARD_WCOIN_C) // WCoinC
		{
			gCashShop.GDCashShopAddPointSaveSend(lpObj->Index, nullptr, this->m_AchInfo.at(missionIndex).RewardValue, 0, 0);
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"[Achievements]: +%d WCoinC",this->m_AchInfo.at(missionIndex).RewardValue);
		}
		else if (this->m_AchInfo.at(missionIndex).RewardType == REWARD_WCOIN_P) // WCoinP
		{
			gCashShop.GDCashShopAddPointSaveSend(lpObj->Index, nullptr, 0, this->m_AchInfo.at(missionIndex).RewardValue, 0);
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"[Achievements]: +%d WCoinP",this->m_AchInfo.at(missionIndex).RewardValue);
		}
		else if (this->m_AchInfo.at(missionIndex).RewardType == REWARD_WCOIN_G) // WCoinG
		{
			gCashShop.GDCashShopAddPointSaveSend(lpObj->Index, nullptr, 0, 0, this->m_AchInfo.at(missionIndex).RewardValue);
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"[Achievements]: +%d Goblin Points",this->m_AchInfo.at(missionIndex).RewardValue);
		}
		else if (this->m_AchInfo.at(missionIndex).RewardType == REWARD_CREDITS) // Credits
		{
		...
		else if (this->m_AchInfo.at(missionIndex).RewardType == REWARD_SUPER_POINTS) // SuperPoints
		{
			lpObj->iQuestStat+=this->m_AchInfo.at(missionIndex).RewardValue;
		...
		else if (this->m_AchInfo.at(missionIndex).RewardType == REWARD_ITEMS)
		{
			ACHIEVEMENTS_ITEM_REWARD_INFO* item = &this->m_AchInfo.at(missionIndex).itemReward;
			for(int i=0;i<item->itemCount; i++)
			{
				GDCreateItemSend(aIndex,0xEB,0,0,item->itemIndex,item->itemLevel,0,item->itemSkill,item->itemLuck,item->itemOption,-1,item->itemExcellentOption,0,0,0,nullptr,0xFF,0);
			}
			//GDCreateItemSend(lpObj->Index,0xEB,0,0,GET_ITEM(item->Section,item->Type),item->Level,item->Dur,item->Skill,item->Luck,item->Option,0,item->ExcOption,item->AncOption,item->JoHOption,item->ExOption,item->SocketOpt,0xFF,item->Time);
		}
...
void CAchievements::ImperialGuardianEvent(int aIndex, int monster_index)
{
	if (!gProtection.GetCustomState(CUSTOM_ACHIEVEMENTS))
	{
		return;
	...
	if(!gProtection.check_access(eCustomers::mufighters))
		return;
	...
	if(!gObjIsConnected(aIndex) || !gObjIsConnected(monster_index))
	{
		return;
	}

	LPOBJ lpUser = &gObj[aIndex];
	LPOBJ lpMonster = &gObj[monster_index];
...
void CAchievements::MonsterKill(int aIndex, int aMonsterIndex)
{
	...
	{
		if(gProtection.m_Index == novamu)
		{
			if(lpMonster->Class == 58)
			{
				lpUser->ach.counter[43]++;
				this->PlayerReward(aIndex, 43); // erohim
			}
		}
		else
		{
	...
	if(gProtection.check_access(eCustomers::mufighters))
	{
		if(lpUser->Map == MAP_DEVIL_SQUARE2 && lpMonster->Class == 440)
		{
		...
		if((lpUser->Map >= MAP_DOUBLE_GOER1 && lpUser->Map <= MAP_DOUBLE_GOER4) && lpMonster->Class == 531)
		{
	...
	if(gProtection.m_Index == novamu)
	{
		if(this->m_AchInfo.at(32).Count > lpUser->ach.counter[32]) // grand reset
		{
			if(lpMonster->Class == 716)
			{
				lpUser->ach.counter[32]++;
				this->PlayerReward(aIndex, 32);
			}
		}
		else if(this->m_AchInfo.at(23).Count > lpUser->ach.counter[23]) //mmotop
		{
			if(lpMonster->Class == 673)
			{
				lpUser->ach.counter[23]++;
				this->PlayerReward(aIndex, 23); 
			}
	...
	if(gProtection.m_Index == House_LTP)
	{
		if (this->m_AchInfo.at(23).Count > lpUser->ach.counter[23]) //mmotop
		{
...
void CAchievements::PlayerKill(int aIndex, int aTargetIndex)
{
	...
	if(gProtection.check_access(eCustomers::mufighters))
	{
		if(lpUser->Map == 45 || lpUser->Map == 46 || lpUser->Map == 47 || 
			lpUser->Map == 48 || lpUser->Map == 49 || lpUser->Map == 50)
		{
...
void CAchievements::GrandResetUp(int aIndex)
{
	...
	if(gProtection.m_Index == novamu || gProtection.m_Index == WorkPC)
		return;
...
void CAchievements::MootopVoteUp(int aIndex)
{
	...
	if(gProtection.m_Index == novamu || gProtection.m_Index == WorkPC || gProtection.check_access(eCustomers::mufighters) || gProtection.m_Index == House_LTP)
		return;
...
void CAchievements::CalculatorPower(int aIndex)
{
	...
	int iAddExcellentPercent = 0;
	int iAddCriticalPercent = 0;

	int iAddDamage = this->m_DivDamage * iAchCount;
	int AddDefense = this->m_DivDefence * iAchCount;
	int iAddLife = this->m_DivLife * iAchCount;

	if(iAchCount > 0)
	{
		iAddExcellentPercent = ( iAchCount * this->m_DivExellentPercent ) / 100;
	...
	if(AddDefense > 0)
	{
		if(AddDefense > this->iDefenceMax)
		{
			AddDefense = this->iDefenceMax;
		}
		lpUser->Defense += AddDefense;
		lpUser->MagicDefense += AddDefense;
	}
	...
	if(iAddExcellentPercent > 0)
	{
		if(iAddExcellentPercent > this->iExellentDamageMax)
		{
			iAddExcellentPercent = this->iExellentDamageMax;
		}
		lpUser->ExcellentDamageRate += iAddExcellentPercent;
	}
