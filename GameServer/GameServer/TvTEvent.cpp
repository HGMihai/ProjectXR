...
void CTvTEvent::Init()
{
	if (gProtection.GetCustomState(CUSTOM_TVTEVENT) == 0)
	{
		return;
	}
...
void CTvTEvent::ReadTvTEventInfo(char* section,char* path)
{
	...
	this->m_TvTEventMinUsers = GetPrivateProfileInt(section,"TvTEventMinUsers",0,path);
...
void CTvTEvent::ProcState_STAND(TVT_EVENT_INFO* lpInfo) // OK
{
	...
	if(this->GetUserCount(lpInfo) < this->m_TvTEventMinUsers)
	{
	...
	if(lpInfo->RemainTime <= 0)
	{
		if (lpInfo->TotalPlayer >=this->m_TvTEventMinUsers)
		{
		...
		if(OBJECT_RANGE(lpInfo->User[n].Index) != 0)
		{
			...
			if(lpInfo->User[n].Team == TeamReward)
			{
				...
				if (lpInfo->RuleInfo.RewardItemCount > 0)
				{
					...
					{
						GDCreateItemSend(lpObj->Index,0xEB,0,0,lpInfo->RuleInfo.ItemRewardIndex,lpInfo->RuleInfo.ItemRewardLevel,0,0,0,0,lpObj->Index,0,0,0,0,0,0xFF,0);
					}
				}
...
bool CTvTEvent::AddUser(TVT_EVENT_INFO* lpInfo,int aIndex) // OK
{
	...
	if (gGate.GetInfo(lpInfo->RuleInfo.WaitingGate, &info))
	{
		if (info.MinLevel != -1 && gObj[aIndex].Level < info.MinLevel)
		{
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(230),info.MinLevel);
			return 0;
		}

		if (info.MinReset != -1 && gObj[aIndex].Reset < info.MinReset)
		{
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(231),info.MinReset);
			return 0;
		}

		if (info.AccountLevel != -1 && gObj[aIndex].AccountLevel < info.AccountLevel)
		{
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(494));
			return 0;
		}
	}
...
void CTvTEvent::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	for(int n=0;n < MAX_TVTEVENT;n++)
	{
		...
		for (int i=0;i<MAX_TVTEVENT_USER;i++)
		{
			...
			{
				gNotice.GCNoticeSend(lpInfo->User[i].Index,0,0,0,0,0,0,"[BlueTeam]: %d Vs [RedTeam]: %d",lpInfo->ScoreBlue,lpInfo->ScoreRed);

			}
			else if(lpInfo->User[i].Team == TVT_REDTEAM)
			{
				gNotice.GCNoticeSend(lpInfo->User[i].Index,0,0,0,0,0,0,"[RedTeam]: %d Vs [BlueTeam]: %d",lpInfo->ScoreRed,lpInfo->ScoreBlue);
			}
		}
	}
}
