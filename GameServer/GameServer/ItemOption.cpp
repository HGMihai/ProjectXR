...
bool CItemOption::GetItemOption(int index,CItem* lpItem) // OK
{
	...
	if(ItemOptionInfo != this->m_ItemOptionInfo.end())
	{
		for(std::vector<ITEM_OPTION_INFO>::iterator it=ItemOptionInfo->second.begin();it != ItemOptionInfo->second.end();it++)
		{
			...
			if(it->ItemOption1 != -1 && it->ItemOption1 > lpItem->m_SkillOption)
			{
				continue;
			}

			if(it->ItemOption2 != -1 && it->ItemOption2 > lpItem->m_LuckOption)
			{
				continue;
			}

			if(it->ItemOption3 != -1 && it->ItemOption3 > lpItem->m_Option)
			{
				continue;
			}
			...
			if (gOptionReplace.IsItemOptionReplace(lpItem->m_Index,it->OptionIndex))
			{
				gOptionReplace.DoItemOptionReplace(it->OptionIndex,index,lpItem);
			}
			else
			{
