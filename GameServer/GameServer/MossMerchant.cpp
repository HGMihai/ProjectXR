		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if (section == 1)
				{
					...
					this->SpawnInfo.Map = lpMemScript->GetNumber();
					this->SpawnInfo.X = lpMemScript->GetAsNumber();
					this->SpawnInfo.Y = lpMemScript->GetAsNumber();
					this->SpawnInfo.Dir = lpMemScript->GetAsNumber();
				}
				else if(section == 2)
				{
					if(strcmp("end",lpMemScript->GetAsString()) == 0)
...
void CMossMerchant::SetMossMerchant() // OK
{
	...
	int px = this->SpawnInfo.X;
	int py = this->SpawnInfo.Y;
	...
	lpObj->StartX = px;
	lpObj->StartY = py;
	lpObj->Dir = this->SpawnInfo.Dir;
	lpObj->Map = this->SpawnInfo.Map;
...
bool CMossMerchant::GambleItem(LPOBJ lpObj,int group) // OK
{
	...
	gItemOptionRate.MakeNewOption(ItemIndex,ItemNewOption,&ItemNewOption,lpMossMerchantItemInfo->Option4);
