...
void CLuckyItem::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			this->m_LuckyItemInfo.push_back(info);
...
bool CLuckyItem::IsLuckyItem(CItem* lpItem) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
...
bool CLuckyItem::GetLuckyItemIndex(LPOBJ lpObj,int section,int group,WORD* index) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
		if((it->Index/MAX_ITEM_TYPE) == section && it->Group == group && it->Class == lpObj->Class/*gItemManager.CheckItemRequireClass(lpObj,it->second.Index) != 0*/)
		{
			(*index) = it->Index;
			return 1;
		}
...
bool CLuckyItem::GetLuckyItemOption0(int index,BYTE* option) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
		if (it->Index == index)
		{
			return gItemOptionRate.GetItemOption0(it->Option0,option);
		}
	}
...
bool CLuckyItem::GetLuckyItemOption1(int index,BYTE* option) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
		if (it->Index == index)
		{
			return gItemOptionRate.GetItemOption1(it->Option1,option);
		}
	}
...
bool CLuckyItem::GetLuckyItemOption2(int index,BYTE* option) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
		if (it->Index == index)
		{
			return gItemOptionRate.GetItemOption2(it->Option2,option);
		}
	}
...
bool CLuckyItem::GetLuckyItemOption3(int index,BYTE* option) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
		if (it->Index == index)
		{
			return gItemOptionRate.GetItemOption3(it->Option3,option);
		}
	}
...
bool CLuckyItem::GetLuckyItemOption4(int index,BYTE* option) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
		if (it->Index == index)
		{
			return gItemOptionRate.GetItemOption4(it->Option4,option);
		}
	}
...
bool CLuckyItem::GetLuckyItemOption5(int index,BYTE* option) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
		if (it->Index == index)
		{
			return gItemOptionRate.GetItemOption5(it->Option5,option);
		}
	}
...
bool CLuckyItem::GetLuckyItemOption6(int index,BYTE* option) // OK
{
	...
	for(std::vector<LUCKY_ITEM_INFO>::iterator it=gLuckyItem.m_LuckyItemInfo.begin();it != gLuckyItem.m_LuckyItemInfo.end();it++)
	{
		if (it->Index == index)
		{
			return gItemOptionRate.GetItemOption6(it->Option6,option);
		}
	}
