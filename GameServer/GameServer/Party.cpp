...
#if (NEW_PARTY_CUSTOM == TRUE)
			this->m_PartyInfo[count].Index[5] = -1;
			this->m_PartyInfo[count].Index[6] = -1;
			this->m_PartyInfo[count].Index[7] = -1;
			this->m_PartyInfo[count].Index[8] = -1;
			this->m_PartyInfo[count].Index[9] = -1;
#endif*/
...
bool CParty::Destroy(int index) // OK
{
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		if(OBJECT_RANGE(this->m_PartyInfo[index].Index[n]) != 0)
		{
			if (gEffectManager.CheckEffectByGroup(&gObj[this->m_PartyInfo[index].Index[n]],118))
			{
				//gEffectManager.AddEffect(&gObj[this->m_PartyInfo[index].Index[n]],0,EFFECT_PARTY_BUFF,0,0,0,0,0);
				gEffectManager.DelEffectByGroup(&gObj[this->m_PartyInfo[index].Index[n]],118);
			}
		}
	}
...
bool CParty::AddMember(int index,int aIndex) // OK
{
	...
	if (this->GetMemberCount(index) >= gServerInfo.m_PartyMaxUser)
	{
		return 0;
	}
...
void CParty::CGPartyRequestRecv(PMSG_PARTY_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	{	
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You can't create party with %s (%s need %d more levels)", gObj[bIndex].Name, gObj[bIndex].Name, TotalLevel - gServerInfo.m_MaxPartyLevelDiff - gObj[bIndex].Level);
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You can't create party with %s (need %d more levels)", gObj[bIndex].Name, gObj[bIndex].Level - gServerInfo.m_MaxPartyLevelDiff - TotalLevel);
		this->GCPartyResultSend(aIndex,5);
		return;
	}	
...
void CParty::CGPartyListRecv(int aIndex) // OK
{
	...
	if(this->IsParty(lpObj->PartyNumber) != 0)
	{
		...
		for(int n=0;n < MAX_PARTY_USER;n++)
		{
			...
			info.Reset = (WORD)lpTarget->Reset;

			info.GrandReset = (WORD)lpTarget->MasterReset;
...
void CParty::GCPartyListSend(int index) // OK
{
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		...
		info.Reset = (WORD)lpObj->Reset;

		info.GrandReset = (WORD)lpObj->MasterReset;
...
{
	if (gEffectManager.CheckEffectByGroup(&gObj[aIndex],118))
	{
		gEffectManager.DelEffectByGroup(&gObj[aIndex],118);
	}
