...
void FriendAddRequest(PMSG_FRIEND_ADD_REQ * lpMsg, int aIndex)
{
	...
	if ( !strcmp(gObj[aIndex].Name, szName))
	{
		...
		#pragma message("PVS_STUDIO FIX 05");
		memcpy(pResult.Name, lpMsg->Name, sizeof(lpMsg->Name));
		pResult.Result = 5;
		pResult.State = -1;
...
void FriendMessageSend(PMSG_FRIEND_MEMO1 * lpMsg, int aIndex, int bIndex)
{
	if ( !gObjIsConnectedGP(aIndex))
	{
		return;
