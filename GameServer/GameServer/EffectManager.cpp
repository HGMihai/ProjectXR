...
void CEffectManager::Load(char* path) // OK
{
	...
	pugi::xml_node EffectManager = file.child("EffectManager");
	for (pugi::xml_node effect = EffectManager.child("Effect"); effect; effect = effect.next_sibling())
	{
		EFFECT_INFO info;

		info.Index = effect.attribute("Index").as_int();

		info.Group = effect.attribute("Group").as_int();

		info.ItemIndex = effect.attribute("ItemIndex").as_int();

		strcpy_s(info.Name,effect.attribute("Name").as_string());

		info.Save = effect.attribute("Save").as_int();

		info.Type = effect.attribute("Type").as_int();

		info.Flag = effect.attribute("Flag").as_int();

		info.Count = effect.attribute("Count").as_int();

		info.Value[0] = effect.attribute("Value1").as_int();

		info.Value[1] = effect.attribute("Value2").as_int();

		info.Value[2] = effect.attribute("Value3").as_int();

		info.Value[3] = effect.attribute("Value4").as_int();
...
//
//    pugi::xml_node root = doc.append_child("EffectManager");
//
//	try
//	{
//		while(true)
...
//
//			pugi::xml_node nodeChild = root.append_child("Effect");
//
//			nodeChild.append_attribute("Index") = info.Index;
//
//			nodeChild.append_attribute("Group") = info.Group;
//
//			nodeChild.append_attribute("ItemIndex") = info.ItemIndex;
//
//			nodeChild.append_attribute("Name") = info.Name;
//
//			nodeChild.append_attribute("Save") = info.Save;
//
//			nodeChild.append_attribute("Type") = info.Type;
//
//			nodeChild.append_attribute("Flag") = info.Flag;
//
//			nodeChild.append_attribute("Count") = info.Count;
//
//			nodeChild.append_attribute("Value1") = info.Value[0];
//
//			nodeChild.append_attribute("Value2") = info.Value[1];
//
//			nodeChild.append_attribute("Value3") = info.Value[2];
//
//			nodeChild.append_attribute("Value4") = info.Value[3];
//
//			this->SetInfo(info);
//		}
//	}
...
//	}
//
//	doc.save_file("EffectManager.xml");
//	delete lpMemScript;
//}
...
{
	if (index >= EFFECT_PREMIUM_BUFF1 && index <= EFFECT_NOOB_BUFF)
	{
		if (gProtection.GetCustomState(CUSTOM_BUFFS) == 0)
		{
			return 0;
		}
	}	
...
void CEffectManager::InsertEffect(LPOBJ lpObj,CEffect* lpEffect) // OK
{
	switch(lpEffect->m_index)
	{
		...
		case EFFECT_MASTER_SCROLL_OF_QUICK:
			...
			lpObj->EffectOption.AddMagicSpeed += lpEffect->m_value[0];
			break;
		case EFFECT_PREMIUM_BUFF1:
			lpObj->EffectOption.MulPhysiDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulMagicDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulCurseDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulDefense += lpEffect->m_value[1];
			break;
		case EFFECT_PREMIUM_BUFF2:	
			lpObj->EffectOption.MulPhysiDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulMagicDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulCurseDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulDefense += lpEffect->m_value[1];
			break;
		case EFFECT_PREMIUM_BUFF3:			
			lpObj->EffectOption.MulPhysiDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulMagicDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulCurseDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulDefense += lpEffect->m_value[1];
			break;
		case EFFECT_PARTY_BUFF:
			lpObj->EffectOption.AddExperienceRate += lpEffect->m_value[0];
			lpObj->EffectOption.AddMasterExperienceRate += lpEffect->m_value[0];
			lpObj->EffectOption.MulPhysiDamage += lpEffect->m_value[1];
			lpObj->EffectOption.MulMagicDamage += lpEffect->m_value[1];
			lpObj->EffectOption.MulCurseDamage += lpEffect->m_value[1];
			lpObj->EffectOption.MulDefense += lpEffect->m_value[2];
			break;
		case EFFECT_GUILD_BUFF:
			...
			lpObj->EffectOption.MulMagicDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulCurseDamage += lpEffect->m_value[0];
			lpObj->EffectOption.MulDropZen += lpEffect->m_value[0];
			lpObj->EffectOption.MulChaosSuccessRate += lpEffect->m_value[0];
			lpObj->EffectOption.MulDefense += lpEffect->m_value[0];
			break;
		case EFFECT_NOOB_BUFF:
			lpObj->EffectOption.AddExperienceRate += lpEffect->m_value[0];
			lpObj->EffectOption.AddMasterExperienceRate += lpEffect->m_value[0];
			lpObj->EffectOption.MulPhysiDamage += lpEffect->m_value[1];
			lpObj->EffectOption.MulMagicDamage += lpEffect->m_value[1];
...
void CEffectManager::RemoveEffect(LPOBJ lpObj,CEffect* lpEffect) // OK
{
	switch(lpEffect->m_index)
	{
		...
		case EFFECT_GREATER_IGNORE_DEFENSE_RATE:
			...
			break;
		case EFFECT_FITNESS:
		//g_Console.AddMessage(5, "%s => %d", lpObj->Name, lpEffect->m_value[0]);
			lpObj->EffectOption.AddVitality -= lpEffect->m_value[0];
			break;
		case EFFECT_GREATER_DEFENSE_SUCCESS_RATE:
			lpObj->EffectOption.AddDefenseSuccessRate -= lpEffect->m_value[0];
...
int CEffectManager::GenerateEffectList(LPOBJ lpObj,BYTE* data) // OK
{
	int count = 0;
	...
	for(int n=0;n < MAX_EFFECT_LIST;n++)
	{
		...
		{
			data[count]  = lpObj->Effect[n].m_index;
		...
		case EFFECT_POISON:
			{
				...
				damage = (int)((lpObj->Life * lpEffect->m_value[2]) / 100);

				if(damage > lpEffect->m_value[3])
				{
					damage = ((lpEffect->m_value[3] == 0) ? damage : lpEffect->m_value[3]);
				}
    ...
    for( int i = 0; i < MAX_EFFECT_LIST; i++)
    {
       // if(lpUser->Effect[i].m_group == BuffIndex)
        ...
        {   
            ...
            memcpy(pMsg.szBuffName, lpBuffData->Name, sizeof(pMsg.szBuffName));
