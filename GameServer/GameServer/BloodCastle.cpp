		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 2)
				{
					...
					int BridgeNum = lpMemScript->GetNumber();

					if (BC_LEVEL_RANGE(BridgeNum) != false)
					{
						gBLOODCASTLE_LEVEL[BridgeNum].NormalCharacterMinLevel = lpMemScript->GetAsNumber();
						gBLOODCASTLE_LEVEL[BridgeNum].NormalCharacterMaxLevel = lpMemScript->GetAsNumber();
						gBLOODCASTLE_LEVEL[BridgeNum].SpecialCharacterMinLevel = lpMemScript->GetAsNumber();
						gBLOODCASTLE_LEVEL[BridgeNum].SpecialCharacterMaxLevel = lpMemScript->GetAsNumber();
					}
				}
				else if(section == 3)
				{
...
int CBloodCastle::GetUserAbleLevel(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Class == CLASS_MG || lpObj->Class == CLASS_DL || lpObj->Class == CLASS_RF)
	{
		if(lpObj->ChangeUp < 2)
		{
			if (lpObj->Level >= gBLOODCASTLE_LEVEL[0].SpecialCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[0].SpecialCharacterMaxLevel)
				level = 0;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[1].SpecialCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[1].SpecialCharacterMaxLevel)
				level = 1;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[2].SpecialCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[2].SpecialCharacterMaxLevel)
				level = 2;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[3].SpecialCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[3].SpecialCharacterMaxLevel)
				level = 3;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[4].SpecialCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[4].SpecialCharacterMaxLevel)
				level = 4;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[5].SpecialCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[5].SpecialCharacterMaxLevel)
				level = 5;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[6].SpecialCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[6].SpecialCharacterMaxLevel)
				level = 6;
		}
		else
			level = 7;
	...
	else
	{
		if(lpObj->ChangeUp < 2)
		{
			if (lpObj->Level >= gBLOODCASTLE_LEVEL[0].NormalCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[0].NormalCharacterMaxLevel)
				level = 0;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[1].NormalCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[1].NormalCharacterMaxLevel)
				level = 1;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[2].NormalCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[2].NormalCharacterMaxLevel)
				level = 2;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[3].NormalCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[3].NormalCharacterMaxLevel)
				level = 3;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[4].NormalCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[4].NormalCharacterMaxLevel)
				level = 4;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[5].NormalCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[5].NormalCharacterMaxLevel)
				level = 5;
			else if (lpObj->Level >= gBLOODCASTLE_LEVEL[6].NormalCharacterMinLevel && lpObj->Level <= gBLOODCASTLE_LEVEL[6].NormalCharacterMaxLevel)
				level = 6;
		}
		else
			level = 7;
...
int CBloodCastle::GetUserAbleLevelNew(LPOBJ lpObj) // OK
{
	int level = -1;
...
void CBloodCastle::NpcAngelMessenger(LPOBJ lpObj) // OK
{
	if(gServerInfo.m_BloodCastleEvent == 0)
	{
		return;
	...
	if(gServerInfo.m_BloodCastleNewLevelSettings)
	...
	if(lpObj->BCEntryCount >= gServerInfo.m_BloodCastleMaxDayEntry[lpObj->AccountLevel])
	{
		GCServerCommandSend(lpObj->Index, 62, lpObj->BCEntryCount, gServerInfo.m_BloodCastleMaxDayEntry[lpObj->AccountLevel]);
		return;
	}

	GCEntryCountSend(lpObj,0);
...
void CBloodCastle::start_now()
{
	...
	LogAdd(LOG_GREEN,"[Set BC Start] At %2d:%2d:00",hour,minute);
