...
//{	
//	if(lpIoBuffer->size < 3/*+recv_length*/)
//	{
//		return 1;
//	}
//	
...
//
//	int count=0,current_size=0,DecSize=0,DecEncrypt=0,DecSerial=0;
...
//	//int counter = 0;
//
//	BYTE lpMsg[MAX_MAIN_PACKET_SIZE];
//	
//	while(true)
//	{
//		LogAdd(LOG_TEST,"Header = %x",lpIoBuffer->buff[0]);
//		/*unsigned long long recv_index = *((unsigned long long*)&lpMsg[count]);
//
//		if (recv_index != gObj[index].recv_index)
//		{			
//			LogAdd(LOG_RED,"Error OnRecv [%llu - %llu] count: %d",recv_index,gObj[index].recv_index,count);
//			return 0;
//		}
//
//		gObj[index].recv_index++;
//		
//		count += recv_length;*/
//
//		unsigned long long recv_index;
//
//		const size_t recv_length = sizeof(recv_index);
//
//		int headerSize;
//		
//		if(lpIoBuffer->buff[count] == 0xC1 || lpIoBuffer->buff[count] == 0xC3)
//		{
//			header = lpIoBuffer->buff[count];
//			current_size = lpIoBuffer->buff[count+1];
//			head = lpIoBuffer->buff[count+2];
//			headerSize = 3;
//			recv_index = *((unsigned long long*)&lpIoBuffer->buff[3]);
//		}
//		else if(lpIoBuffer->buff[count] == 0xC2 || lpIoBuffer->buff[count] == 0xC4)
//		{
//			header = lpIoBuffer->buff[count];
//			current_size = MAKEWORD(lpIoBuffer->buff[count+2],lpIoBuffer->buff[count+1]);
//			head = lpIoBuffer->buff[count+3];
//			headerSize = 4;
//			recv_index = *((unsigned long long*)&lpIoBuffer->buff[4]);
//		}
//		else
//		{
//			gLog.Output(LOG_CONNECT,"[SocketManager] Protocol header error (Index: %d, Header: %x, RecvIndex: %llu)",index,lpIoBuffer->buff[count],gObj[index].recv_index-1);
//			return 0;
//		}
//
//		memcpy(lpMsg,lpIoBuffer->buff,headerSize);
//		memcpy(&lpMsg[headerSize],&lpIoBuffer->buff[headerSize+recv_length],current_size-headerSize);
//
//		if(current_size < 3 || current_size > MAX_MAIN_PACKET_SIZE)
//		{
//			gLog.Output(LOG_CONNECT,"[SocketManager] Protocol size error (Index: %d, Header: %x, Size: %d, Head: %x)",index,header,current_size,head);
//			return 0;
//		}
//
//		if(current_size <= lpIoBuffer->size)
//		{
//			if(header == 0xC3 || header == 0xC4)
//			{
//				if(header == 0xC3)
//				{
//					DecSize = gPacketManager.Decrypt(&DecBuff[1],&lpMsg[count+2],(current_size-2))+1;
//
//					DecSerial = DecBuff[1];
//
//					header = 0xC1;
...
//				else
//				{
//					DecSize = gPacketManager.Decrypt(&DecBuff[2],&lpMsg[count+3],(current_size-3))+2;
//
//					DecSerial = DecBuff[2];
//
//					header = 0xC2;
...
//			else
//			{
//				if(gPacketManager.AddData(&lpMsg[count],current_size) == 0 || gPacketManager.ExtractPacket(DecBuff) == 0)
//				{
//					return 0;
//				}
//
...
//				QueueInfo.head = head;
//
//				memcpy(QueueInfo.buff,DecBuff,current_size);
//
//				QueueInfo.size = current_size;
//
//				QueueInfo.encrypt = 0;
//
//				QueueInfo.serial = -1;
...
//			count += current_size;
//
//			lpIoBuffer->size -= current_size/*+recv_length*/;
//
...
//bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
//{
//	const size_t recv_length = sizeof(unsigned long long);
//	
//	if(lpIoBuffer->size < 3/*+recv_length*/)
//	{
//		return 1;
...
//		{
//			header = lpMsg[count];
//			current_size = lpMsg[count+1]+recv_length;
//			head = lpMsg[count+2];
//		}
//		else if(lpMsg[count] == 0xC2 || lpMsg[count] == 0xC4)
//		{
//			header = lpMsg[count];
//			current_size = MAKEWORD(lpMsg[count+2],lpMsg[count+1])+recv_length;
//			head = lpMsg[count+3];
//		}
//		else
//		{
//			gLog.Output(LOG_CONNECT,"[SocketManager] Protocol header error (Index: %d, Header: %x, RecvIndex: %llu)",index,lpMsg[count],gObj[index].recv_index-1);
//			return 0;
//		}
//
//		if(current_size < 3 || current_size > MAX_MAIN_PACKET_SIZE)
...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	/*std::ofstream out("out_protocol.txt", std::ios::app);
	...
	char text[256];
	sprintf_s(text, "[%02d:%02d:%02d] index: %llu buff: ", Time.wHour, Time.wMinute, Time.wSecond, gObj[index].recv_index);
	out << text;

	for(int i=0;i<lpIoBuffer->size;i++)
	{
		out << "0x" << std::hex << (int)lpIoBuffer->buff[i] << " ";
	}
	...
	while(true)
	{
		unsigned long long recv_index = *reinterpret_cast<unsigned long long*>(&lpMsg[count]);
		//unsigned long long recv_index = *(unsigned long long*)&lpMsg[count];

		//LogAdd(LOG_RED,"recv Index= %llu - %llu",recv_index,gObj[index].recv_index);

		if (recv_index != gObj[index].recv_index)
		{
			/*std::ofstream out("out_protocol_error.txt", std::ios::app);
			...
			char text[256];
			sprintf_s(text, "[%02d:%02d:%02d] index: %llu buff: ", Time.wHour, Time.wMinute, Time.wSecond, gObj[index].recv_index-1);
			out << text;

			for(int i=count;i<lpIoBuffer->size;i++)
			{
				out << "0x" << std::hex << (int)lpMsg[i] << " ";
			}
			...
			{
				out << "0x" << std::hex << (int) lpMsg[count+i] << " ";
			}
		...
		count += recv_length;
		//memmove(lpMsg,&lpMsg[8],lpIoBuffer->size);
		
		if(lpMsg[count] == 0xC1 || lpMsg[count] == 0xC3)
		{
			header = lpMsg[count];
			current_size = lpMsg[count+1];
			head = lpMsg[count+2];
		}
		else if(lpMsg[count] == 0xC2 || lpMsg[count] == 0xC4)
		{
			header = lpMsg[count];
			current_size = MAKEWORD(lpMsg[count+2],lpMsg[count+1]);
			head = lpMsg[count+3];
		}
		else
		{
			...
			char text[256];
			sprintf_s(text, "[%02d:%02d:%02d] index: %llu, count: %d size: %d buff: ", Time.wHour, Time.wMinute, Time.wSecond, gObj[index].recv_index-1,count,lpIoBuffer->size);
			out << text;
...
bool CSocketManager::DataRecvNew(int index, IO_MAIN_BUFFER* lpIoBuffer)
{
...
bool CSocketManager::DataRecvDefault(int index, IO_MAIN_BUFFER* lpIoBuffer)
{
	if(lpIoBuffer->size < 3)
	{
		return 1;
...
void CSocketManager::Disconnect(int index) // OK
{
	...
	//if (gObj[index].Socket == (SOCKET)0)
	//if (strcmp(gObj[index].IpAddr,""))
	/*if (strlen(gObj[index].IpAddr) == 0)
	{
		this->m_critical.unlock();
		return;
	}*/
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	if(IoSize == 0)
	{
		//LogAdd(LOG_RED,"Error Recv IoSize == 0");
		this->Disconnect(index);
		this->m_critical.unlock();
		return;
	}
	...
	DecryptData(&lpIoContext->IoMainBuffer.buff[lpIoContext->IoMainBuffer.size],IoSize, index);

	#endif

	lpIoContext->IoMainBuffer.size += IoSize;

	/*unsigned long long recv_index = *((unsigned long long*)lpIoContext->IoMainBuffer.buff);

	if (recv_index != lpObj->recv_index)
	{
		LogAdd(LOG_RED,"Error OnRecv [%llu - %llu]",recv_index,lpObj->recv_index);
		this->Disconnect(index);
		this->m_critical.unlock();
		return;
	}
	...
	if(gServerInfo.UseWrongPacketsCheck == 1)
	{
	...
	{
		if(this->DataRecvDefault(index,&lpIoContext->IoMainBuffer) == 0)
		{
			this->Disconnect(index);
			this->m_critical.unlock();
			return;
	...
	/*else if(gServerInfo.UseWrongPacketsCheck == 2)
	{
		if(this->DataRecvNew(index,&lpIoContext->IoMainBuffer) == 0)
		{
			this->Disconnect(index);
			this->m_critical.unlock();
			return;
...
void CSocketManager::OnSend(int index,DWORD IoSize,IO_SEND_CONTEXT* lpIoContext) // OK
{
	...
	if(IoSize == 0)
	{
		//LogAdd(LOG_RED,"Error Send IoSize == 0");
		this->Disconnect(index);
		this->m_critical.unlock();
		return;
	}
...
DWORD WINAPI CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		...
		//LogAdd(LOG_TEST,"IoSize = %d",IoSize);
