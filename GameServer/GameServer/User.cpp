...
void gObjEventRunProc() // OK
{
	...
	gVipLottery.MainProc();

	/*if (gDiablo.m_DiabloInfo.State == DIABLO_STATE_BLANK)
	{
...
#if (GAMESERVER_UPDATE == 603 && GAMESERVER_TYPE == 0)
	gDiablo.MainProc();
#endif
}
...
void gObjAllLogOut() // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObj[n].Connected != OBJECT_OFFLINE && gObj[n].Type == OBJECT_USER)
		{
			...
			#if USE_TESTOFFEXP == TRUE
			gOffLevel.OnAttackAlreadyConnected(&gObj[n]);
			#endif
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	lpObj->BCEntryCount = 0;
	lpObj->DSEntryCount = 0;
...
//	lpObj->Rank = 0;
	...
	lpObj->last_vote_time = 0;
	lpObj->is_use_auto_health = false;
	lpObj->auto_health_tick = 0;

	lpObj->MapQuestSystemMap = 0;
	lpObj->MapQuestSystemIndex = 0;
	lpObj->MapQuestSystemState = 0;
...
short gObjDelOffExp(int aIndex)
{
	gObj[aIndex].AttackCustomOffline = 0;
	gObj[aIndex].AttackCustomOfflineTime = 0;
	gObj[aIndex].AttackCustomOfflineMoneyDelay = 0;
	gObj[aIndex].AttackCustomOfflineMoneyFreeDelay = 0;
	gObj[aIndex].PShopCustomOfflineTime = 0;
	gObj[aIndex].PShopCustomOffline = 0;
...
short gObjDel(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		if (lpObj->GuildNumber > 0)
		{
			...
			{
				if (lpObj->Guild->WarehouseUserIndex == aIndex)
				{
					lpObj->Guild->WarehouseInUse = 0;

					lpObj->Guild->WarehouseUserIndex = -1;
				}
			}
		}
...
void gObjViewportListProtocolCreate(LPOBJ lpObj) // OK
{
	...
	{
		gCostumeSystem.GCCostumeSend(lpObj, lpObj->Index);
...
void gObjViewportListProtocol(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		//LogAdd(LOG_TEST,"ViewportList [%s]",lpObj->Name);
		gViewport.GCViewportCostumeSend(aIndex);
...
void gObjPKDownCheckTime(LPOBJ lpObj,int TargetLevel) // OK
{
	...
	if(lpObj->PKLevel < 3)
	{
		...
		{
			if(lpObj->PKCount < gServerInfo.m_MaxPKCount)
			{
				lpObj->PKCount++;
			}
...
void gObjKillReward(LPOBJ lpObj, LPOBJ lpKiller)
{
	if (lpObj->PKLevel < 6)
		return;
	...
	DWORD zen_value = lpObj->Money / 100 * zen_rate;

	if (lpKiller->Money + zen_value > MAX_MONEY)
		zen_value = MAX_MONEY - lpKiller->Money;

	if(zen_value != 0)
	{
		lpObj->Money -= zen_value;
		lpKiller->Money += zen_value;

		GCMoneySend(lpObj->Index, lpObj->Money);
		GCMoneySend(lpKiller->Index, lpKiller->Money);

		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You lost %d zen", zen_value);
		gNotice.GCNoticeSend(lpKiller->Index, 1, 0, 0, 0, 0, 0, "You got %d zen", zen_value);
	}

	/* MONEY END */

	/*if ((GetLargeRand() % 100) < zen_rate)
	{
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		if (gServerInfo.m_ShowKillMessage == 1)
		{
			GCChatTargetSend(lpObj, lpTarget->Index, gMessage.GetMessage(516));
		}
	}

	if (lpTarget->Type == OBJECT_USER)
		gObjKillReward(lpObj, lpTarget);
	...
	if (gServerInfo.m_CustomDeathMessage != 0)
	{
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if((map == MAP_ICARUS || map == MAP_KANTURU3) && lpObj->Inventory[7].IsItem() == 0 && lpObj->Inventory[8].m_Index != GET_ITEM(13,3) && lpObj->Inventory[8].m_Index != GET_ITEM(13,4) && lpObj->Inventory[8].m_Index != GET_ITEM(13,37))
	{
...
#pragma message("Simple kalima fix (test this!!!)");
	...
	/*if (CC_MAP_RANGE(map))
	{
		gAchievements.TeleportInCC(aIndex);
	}*/
...
void gObjTeleport(int aIndex,int map,int x,int y) // OK
{
	...
	{
		gWarehouse.CGWarehouseClose(aIndex, 3);
	}
...
bool gObjGuildWarehouseSearchSerialNumber(LPOBJ lpObj,DWORD serial) // OK
{
	if (lpObj->GuildNumber == 0)
	{
		return 1;
	...
	for(int n=0;n < WAREHOUSE_SIZE;n++)
	{
		if(lpObj->GuildWarehouse[n].m_Serial != 0 && lpObj->GuildWarehouse[n].m_Serial == serial && (count++) > 0)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(482));
			gObjUserKill(lpObj->Index);
			return 0;
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_NPC)
			{
				...
				if(lpObj->Class == 229)
				{
					if (gServerInfo.m_QuestNPCTeleport != 0)
					{
		...
		if(lpObj->Connected >= OBJECT_LOGGED && lpObj->Type == OBJECT_USER &&	lpObj->CloseCount > 0)
		{
			if(lpObj->CloseCount == 1)
			{
				...
				{
#pragma message("DONT FORGET FIX MONSTER IN OFFEXP");
					//lpObj->CloseCount--;
		...
		if(lpObj->Connected > OBJECT_LOGGED &&	lpObj->Type == OBJECT_USER)
		{
			...
			if (lpObj->IsOffExpReconnect == true && lpObj->AttackCustomOffline == 0)
			{
...
BOOL gObjBackSpring2(LPOBJ lpObj, LPOBJ lpTargetObj, int count)
{
	...
	*/
	...
	lpObj->OldY = y;
	CGPositionRecv(&pMsg,lpObj->Index);
	//LogAdd(LOG_TEST,"Send BackSpring");
	return TRUE;
}
	...
	for ( int n=0;n<MAX_SELF_DEFENSE;n++)
	{
		if ( lpObj->SelfDefense[n] >= 0 )
		{
			if ( GetTickCount() > lpObj->SelfDefenseTime[n] )
			{
				...
				LogAdd(LOG_BLACK,"[%s][%s] ReSet SelfDefense [%s][%s]", lpObj->Account, lpObj->Name, gObj[lpObj->SelfDefense[n]].Account, gObj[lpObj->SelfDefense[n]].Name);
				lpObj->SelfDefense[n] = -1;
			}
		}
	}
...
int gObjCheckOffModeByIP(char* IpAdress)
{
	int Count = 0;

	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnectedGP(n) != 0 && strcmp(gObj[n].IpAddr,IpAdress) == 0 && (gObj[n].AttackCustomOffline != 0 || gObj[n].PShopCustomOffline != 0))
		{
			Count++;
		}
	}
...
{
	PMSG_FRIEND_MEMO1 pMsg;

	pMsg.WindowGuid = 0;

	//strcpy(pMsg.Name,"AFK");
	memcpy(pMsg.Name,"AFK",sizeof("AFK"));
	memcpy(pMsg.Subject,"System Message.",sizeof("System Message."));
	...
	memcpy(pMsg.Memo,text,sizeof(text));
	pMsg.MemoSize = sizeof(text);

	FriendMessageSend(&pMsg,lpTarget->Index,lpKiller->Index);
}

void gObjFindAndUseHealthPotion(LPOBJ lpObj)
{
	DWORD CurrentTick	= GetTickCount();
	DWORD Delay			= CurrentTick - lpObj->auto_health_tick;
	// ----
	if( Delay < (DWORD)gServerInfo.m_AutoHealthPotionDelay[lpObj->AccountLevel] )
		return;
	...
	lpObj->auto_health_tick = GetTickCount();
}