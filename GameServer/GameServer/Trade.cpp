...
void CTrade::CGTradeRequestRecv(PMSG_TRADE_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gBotAlchemist.TradeOpen(aIndex,bIndex) == true)
		return;
	...
	if (gItemManager.ChaosBoxHasItem(lpTarget) || gItemManager.TradeHasItem(lpTarget))
	{
		return;
	}

	if(DE_MAP_RANGE(lpTarget->Map) != 0 || CA_MAP_RANGE(lpTarget->Map) != 0 || DS_MAP_RANGE(lpTarget->Map) != 0 || BC_MAP_RANGE(lpTarget->Map) != 0 || CC_MAP_RANGE(lpTarget->Map) != 0 || IT_MAP_RANGE(lpTarget->Map) != 0 || DA_MAP_RANGE(lpTarget->Map) != 0 || DG_MAP_RANGE(lpTarget->Map) != 0 || IG_MAP_RANGE(lpTarget->Map) != 0)
	{
		this->GCTradeResponseSend(aIndex,0,lpTarget->Name,0,0);
		return;
	}
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg,int aIndex) // OK
{
	...
	if ( gBotAlchemist.IsInTrade(aIndex) == 1 && lpMsg->flag == 1)
	{
		gBotAlchemist.TradeOk(aIndex);
		return;
	}
	...
	if (lpTarget->Money < lpTarget->TradeMoney || lpObj->Money < lpObj->TradeMoney)
	{
		this->ResetTrade(aIndex);
		this->GCTradeResultSend(aIndex,5);
		this->ResetTrade(bIndex);
...
void CTrade::CGTradeCancelButtonRecv(int aIndex) // OK
{
	...
	if ( gBotAlchemist.IsInTrade(aIndex) == 1)
	{
		gBotAlchemist.TradeCancel(aIndex);
		return;
	}
