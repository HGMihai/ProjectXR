...
CPremiumDrop::CPremiumDrop() : m_PremiumDropEnable(false)
{
	this->m_PremiumDropInfo.clear();
}

CPremiumDrop::~CPremiumDrop() = default;
...
bool CPremiumDrop::DropPremium(LPOBJ lpObj, LPOBJ lpTarget) const
{
	...
	for (auto& it : this->m_PremiumDropInfo)
	{
		...
		if ((GetLargeRand()%1000000) <= it.DropRate)
		{
			...
			if (lpTarget->AccountLevel > it.PremiumLevel)
			{
				...
				gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, "You won the premium extension for %d minutes!",
				                     it.TimeForBiggerAL/60);
			}
			else
			{
				GJAccountLevelSaveSend(lpTarget->Index, it.PremiumLevel, it.TimeValue);
				GJAccountLevelSend(lpTarget->Index);
				gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, "You won %s VIP type for %d minutes!", szStatus,
				                     it.TimeValue/60);
			}
