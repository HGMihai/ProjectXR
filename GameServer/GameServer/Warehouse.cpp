...
void CWarehouse::CGWarehouseMoneyRecv(PMSG_WAREHOUSE_MONEY_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gGuildWarehouse.CGGuildWarehouseMoneyRecv(lpMsg,aIndex);
		return;
	}
...
void CWarehouse::CGWarehouseClose(int aIndex, int type) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	if (lpObj->LoadGuildWarehouse != 0)
	{
		...
		gGuildWarehouse.GDGuildWarehouseItemSaveSend(aIndex);

		memset(lpObj->GuildWarehouseMap,0xFF,WAREHOUSE_SIZE);

		for(int n=0;n < WAREHOUSE_SIZE;n++){lpObj->GuildWarehouse[n].Clear();}
...
void CWarehouse::CGWarehouseCloseTest(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	//DataSend(aIndex,(BYTE*)&pMsg,pMsg.size);
	//LogAdd(LOG_RED, "Warehouse closed!");
}
...
void CWarehouse::CGWarehousePasswordRecv(PMSG_WAREHOUSE_PASSWORD_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->LoadGuildWarehouse)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You can't set password for Guild Warehouse!");
		return;
	}
...
void CWarehouse::DGWarehouseItemRecv(SDHP_WAREHOUSE_ITEM_RECV* lpMsg) // OK
{
	...
	for(int n=0;n < WAREHOUSE_SIZE;n++)
	{
		...
		//LogAdd(LOG_TEST,"index = %d [%x%x%x%x]",n,lpMsg->WarehouseItem[n][0],lpMsg->WarehouseItem[n][1],lpMsg->WarehouseItem[n][2],lpMsg->WarehouseItem[n][3]);
	...
	//LogAdd(LOG_RED, "loaded %d %d %d %d %d", lpObj->Interface.use, lpObj->Interface.type, lpObj->Interface.state, lpObj->LoadWarehouse, lpObj->WarehouseLock);
	//LogAdd(LOG_RED, "Warehouse loaded!");
}
