...
void CServerInfo::SendServerInfo(int aIndex)
{
	...
	pMsg.Customs.CustomStatsAdd = gProtection.GetCustomState(CUSTOM_STATSADD);
	pMsg.Customs.CustomExpandedCharacters = gProtection.GetCustomState(CUSTOM_EXPANDED_CHARACTER);
	pMsg.Customs.CustomOfflineMode = gProtection.GetCustomState(CUSTOM_OFFLINE_MODE);
	pMsg.Customs.CustomPartySearch = gProtection.GetCustomState(CUSTOM_PARTY_SEARCH);
	pMsg.Customs.CustomSmithy = gProtection.GetCustomState(CUSTOM_SMITHY);
	pMsg.Customs.CustomBuffIcon = gProtection.GetCustomState(CUSTOM_BUFFICON);
	pMsg.Customs.CustomEventTimer = gProtection.GetCustomState(CUSTOM_EVENTTIMER);
	pMsg.Customs.CustomGuildWarehouse = gProtection.GetCustomState(CUSTOM_GUILD_WAREHOUSE);
	pMsg.Customs.CustomAchievements = gProtection.GetCustomState(CUSTOM_ACHIEVEMENTS);
	pMsg.Customs.CustomRankings = gProtection.GetCustomState(CUSTOM_RANKINGS);
	pMsg.Customs.CustomOptionReplace = gProtection.GetCustomState(CUSTOM_OPTION_REPLACE);
	pMsg.Customs.CustomQuestSystem = gProtection.GetCustomState(CUSTOM_QUEST_SYSTEM);
	pMsg.Customs.CustomActiveInvasions = gProtection.GetCustomState(CUSTOM_ACTIVE_INVASIONS);
	pMsg.Customs.CustomStat = gProtection.GetCustomState(CUSTOM_STAT);

	for (int i=0; i<MAX_ACCOUNT_LEVEL; i++)
	{
		pMsg.CustomCharInterface[i] = this->m_CustomCharInterface[i];
		pMsg.CustomResetPoint[i] = this->m_CustomResetPoint[i];
		pMsg.CustomResetPointPriceWC[i] = this->m_CustomResetPointPriceWC[i];
		pMsg.CustomResetPointPricePoint[i] = this->m_CustomResetPointPricePoint[i];
	...
	pMsg.new_bc_level_rule = gServerInfo.m_BloodCastleNewLevelSettings;
	pMsg.new_ds_level_rule = gServerInfo.m_DevilSquareNewLevelSettings;
	pMsg.new_it_level_rule = gServerInfo.m_IllusionTempleNewLevelSettings;
	pMsg.AnimatedWarehouse = this->m_CustomWarehouseAnimated;
	...
	pMsg.NewStat = gDefaultClassInfo.m_DefaultClassInfo[gObj[aIndex].Class].NewStat;
	pMsg.PartyTeleportSwitch = this->m_PartyTeleportSwitch;
	pMsg.PartyTeleportMoney = this->m_PartyTeleportMoney;

	pMsg.PartyMaxUser = this->m_PartyMaxUser;

	pMsg.CustomStoreEnableJoB = gCustomStore.m_CustomStoreEnableJoB[gObj[aIndex].AccountLevel];
	pMsg.CustomStoreEnableJoS = gCustomStore.m_CustomStoreEnableJoS[gObj[aIndex].AccountLevel];
	pMsg.CustomStoreEnableJoC = gCustomStore.m_CustomStoreEnableJoC[gObj[aIndex].AccountLevel];
	pMsg.CustomStoreEnableWCC = gCustomStore.m_CustomStoreEnableWCC[gObj[aIndex].AccountLevel];
	pMsg.CustomStoreEnableWCP = gCustomStore.m_CustomStoreEnableWCP[gObj[aIndex].AccountLevel];
	pMsg.CustomStoreEnableWCG = gCustomStore.m_CustomStoreEnableWCG[gObj[aIndex].AccountLevel];
	pMsg.ExtendedCharacters = m_ExtendedCharacters[gObj[aIndex].AccountLevel];
	pMsg.CanUseAutoHealth = this->m_UseAutoHealthPotion[gObj[aIndex].AccountLevel];
	pMsg.AutoHealEnable = this->m_AutoHealthPotionEnable;
	...
	pMsg.customer_id = gProtection.m_Index;

	DataSend(aIndex, (LPBYTE)&pMsg, sizeof(pMsg));

	//gMapManager.GCSendHealthBar(aIndex);
}
...
{	
	...
	g_ResetSystem.Load();
	gGRSystem.Load();
	gChangeClass.Load(gPath.GetFullPath("..\\LTPData\\ChangeClass.ini"));
	gBuyAccountLevel.Load(gPath.GetFullPath("..\\LTPData\\BuyVip.ini"));
	//gItemRank.Load(gPath.GetFullPath("..\\LTPData\\ItemRank.txt"));
	//gTeleportPanel.Load("GameServerInfo", gPath.GetFullPath("..\\LTPData\\TeleportPanel.ini"));
#endif

	gOptionReplace.Load("..\\LTPData\\Item\\OptionReplace.txt");
	
	gDisableFunctions.Load(gPath.GetFullPath("..\\LTPData\\DisableSystem.ini"));
	
	gAutoReset.Load(gPath.GetFullPath("..\\LTPData\\AutoReset.ini"));
	//CAutoReset::getInstance().Load(gPath.GetFullPath("..\\LTPData\\AutoReset.ini"));

	gDiablo.Load();

	gTeleporter.Load(gPath.GetFullPath("..\\LTPData\\Teleporter.txt"));

	gPVPZone.Load(gPath.GetFullPath("..\\LTPData\\PVPZone.txt"));

	gCustomAttack.ReadCustomAttackInfo("CustomAttack",gPath.GetFullPath("..\\LTPData\\CustomAttack.txt"));

	gPickup.Load(gPath.GetFullPath("..\\LTPData\\PickupItems.txt"));
	gPickup.ReadCustomPickInfo("CustomPickup", gPath.GetFullPath("..\\LTPData\\CustomPickup.txt"));

	gMarrySystem.Load("MarrySystem", gPath.GetFullPath("..\\LTPData\\MarrySystem.ini"));

	gCustomQuest.Load();

	//gMapQuestSystem.load(gPath.GetFullPath("..\\LTPData\\MapQuestSystem\\MapQuestSystem.xml"));

	gShopPointEx.Load(gPath.GetFullPath("..\\LTPData\\ShopPoint.txt"));

	gBotBuffer.Load(gPath.GetFullPath("..\\LTPData\\Bots\\BotBuffer.txt"));

	gPartySearch.Load("PartySearch", gPath.GetFullPath("..\\LTPData\\PartySearch.txt"));

	gChaosBoxManager.Read(gPath.GetFullPath("..\\LTPData\\ChaosMachineManager.xml"));

	//gVipLottery.Load(gPath.GetFullPath("..\\LTPData\\Events\\Lottery.xml"));

	gZenDrop.Load(gPath.GetFullPath("..\\LTPData\\ZenDrop.xml"));

	gPremiumDrop.Load(gPath.GetFullPath("..\\LTPData\\PremiumDrop.xml"));

	Marry.LoadFile(gPath.GetFullPath("..\\LTPData\\Marry.xml"));

	#if USE_TESTOFFEXP == TRUE
	gOffLevel.Load();
	#endif

	gTvTEvent.Load(gPath.GetFullPath("..\\LTPData\\Events\\TvTEvent.txt"));

	gTvTEvent.ReadTvTEventInfo("GameServerInfo",".\\Data\\GameServerInfo - Custom.dat");

	gOfflineMode.LoadSkillsInfo("..\\LTPData\\OfflineMode\\OfflineModeBuffs.xml");
	gOfflineMode.Load("OfflineMode","..\\LTPData\\OfflineMode\\OfflineMode.ini");

	gRandomMonsterCall.Load(gPath.GetFullPath("..\\LTPData\\Monster\\MonsterCallRandom.xml"));

	gMonsterCall.Load(gPath.GetFullPath("..\\LTPData\\MonsterCall.txt"));

	gMonsterSpawn.Load(gPath.GetFullPath("..\\LTPData\\MonsterSpawn.txt"));

	gSuperPoints.Load(gPath.GetFullPath("..\\LTPData\\SuperPoints.txt"));

	gPackSystem.Load("..\\LTPData\\PackSystem.xml");

	gCustomPets.Load("..\\LTPData\\CustomPets\\CustomPets.txt");

	gEventTimer.Load("..\\LTPData\\EventTimer\\EventTimer.txt");

	gDailyReward.Load("..\\LTPData\\DailyReward\\DailyReward.txt");

	gGuildWarehouse.Load("..\\LTPData\\GuildWarehouse.ini");

	gItemToPoints.Load("..\\LTPData\\ItemToPoints.xml");

	gCustomStartItem.Load("..\\LTPData\\Item\\StartItem.xml");

	gRoulette.load("..\\LTPData\\Roulette\\Roulette.xml");

	gMMOTOP.Load("..\\LTPData\\Vote.ini");

	//gNightTime.Load(gPath.GetFullPath("..\\LTPData\\NightTime.txt"));

	gAchievements.Read(gPath.GetFullPath("..\\LTPData\\Achievements.xml"));

	gCostumeSystem.load("..\\LTPData\\CostumeSystem.xml");

	g_ExWinQuestSystem.Load();

	gCustomMove.load("..\\LTPData\\Move\\CustomMove.xml");

	gTeleportPanel.load("..\\LTPData\\Move\\TeleportPanel.xml");

	if (gProtection.m_Index == eCustomers::Marks) // MARKS
	{
		gRegionSystem.load("..\\LTPData\\RegionSystem.xml");
	}
	
	gCharacterCalc.Load("..\\LTPData\\Scripts\\Character\\CalcCharacter.lua");

	gBotAlchemist.Read(gPath.GetFullPath("..\\LTPData\\Bots\\BotAlchemist.xml"));
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnectedGP(n) != 0)
		{
			...
			gGRSystem.SendResetData(&gObj[n], false);
			g_UserPanel.SendUserPanel(n);
			//gDonateShop.GCDonateShopInfo(n);
			gCustomStore.SendNamesToClient(n);
			gChangeClass.SendData(n);
			gBuyAccountLevel.SendInfo(n);
			gServerInfo.SendServerInfo(n);
		}
	}
#endif

	LogAdd(LOG_BLUE,"[ServerInfo] LTPData loaded successfully");
}
...
void CServerInfo::ReadCommonInfo() // OK
{
	...
	gEffectManager.Load(gPath.GetFullPath("Effect.xml"));

	gMapManager.Load(gPath.GetFullPath("MapManager.xml"));

	gMapServerManager.Load(gPath.GetFullPath("MapServerInfo.dat"));

	gMessage.Load(gPath.GetFullPath("Message.xml"));

	gCommandManager.Load("..\\LTPData\\Commands\\CommandManager.xml");
...
{
	gItemBagManager.Load(gPath.GetFullPath("EventItemBagManager.xml"));
...
void CServerInfo::ReadMonsterInfo() // OK
{
	...
	//gMonsterManager.LoadTXT(gPath.GetFullPath("Monster\\Monster.txt"));
	gMonsterManager.load_xml(gPath.GetFullPath("Monster\\MonsterList.xml"));

	gMonsterSetBase.Load(gPath.GetFullPath("Monster\\MonsterSpawn.xml"));
	//gMonsterSetBase.LoadTXT(gPath.GetFullPath("Monster\\MonsterSetBase.txt"));
...
void CServerInfo::ReadShopInfo() // OK
{
	...
	gShopManager.Load(gPath.GetFullPath("ShopManager.xml"));

	gShopManager.LoadShop();

	gShopManager.LoadDonateShop();
...
void CServerInfo::ReadSkillInfo() // OK
{
	...
	gComboSkillData.load(gPath.GetFullPath("Skill\\ComboSkill.xml"));
...
void CServerInfo::ReadUtilInfo() // OK
{
	...
	gFilter.Load(gPath.GetFullPath("Util\\Filter.xml"));

	gGameMaster.Load(gPath.GetFullPath("Util\\GameMaster.txt"));

	gNotice.Load(gPath.GetFullPath("Util\\Notice.txt"));

	gResetTable.Load(gPath.GetFullPath("Util\\ResetTable.xml"));

	gFileCheck.Load(gPath.GetFullPath("Util\\CRCCheck.txt"));
...
void CServerInfo::ReloadMonsterInfo() // OK
{
	...
	for ( int n=MAX_OBJECT_MONSTER;n<OBJ_STARTUSERINDEX;n++)
	{
		if ( gObj[n].Type == OBJECT_MONSTER || gObj[n].Type == OBJECT_NPC )
		{
...
void CServerInfo::ReadStartupInfo(char* section,char* path) // OK
{
	...
	wsprintf(this->m_ServerMutex,"LTP_MU_GS_MUTEX_%d",this->m_ServerCode);
	...
	this->m_ServerMinUserReset = GetPrivateProfileInt(section,"ServerMinUserReset",0,path);

	this->m_ServerMaxUserReset = GetPrivateProfileInt(section,"ServerMaxUserReset",0,path);

	this->m_ServerMinUserGReset = GetPrivateProfileInt(section,"ServerMinUserGrandReset",0,path);

	this->m_ServerMaxUserGReset = GetPrivateProfileInt(section,"ServerMaxUserGrandReset",0,path);
...
void CServerInfo::ReadChaosMixInfo(char* section,char* path) // OK
{
	...
	this->m_SeniorMixPrice = GetPrivateProfileInt(section, "SeniorMixPrice", 1000000000, path);

	this->m_NewWingMixRate_Part1[0] = GetPrivateProfileInt(section,"NewWingMixRate_Part1_AL0",0,path);

	this->m_NewWingMixRate_Part1[1] = GetPrivateProfileInt(section,"NewWingMixRate_Part1_AL1",0,path);

	this->m_NewWingMixRate_Part1[2] = GetPrivateProfileInt(section,"NewWingMixRate_Part1_AL2",0,path);

	this->m_NewWingMixRate_Part1[3] = GetPrivateProfileInt(section,"NewWingMixRate_Part1_AL3",0,path);

	this->m_NewWingMixRate_Part2[0] = GetPrivateProfileInt(section,"NewWingMixRate_Part2_AL0",0,path);

	this->m_NewWingMixRate_Part2[1] = GetPrivateProfileInt(section,"NewWingMixRate_Part2_AL1",0,path);

	this->m_NewWingMixRate_Part2[2] = GetPrivateProfileInt(section,"NewWingMixRate_Part2_AL2",0,path);

	this->m_NewWingMixRate_Part2[3] = GetPrivateProfileInt(section,"NewWingMixRate_Part2_AL3",0,path);

	this->m_RemoveRFWings = GetPrivateProfileInt(section,"RemoveRFWings",0,path);

	this->m_RemoveSUWings = GetPrivateProfileInt(section,"RemoveSUWings",0,path);
...
void CServerInfo::ReadCharacterInfo(char* section,char* path) // OK
{
	...
	this->m_AttackBySD = GetPrivateProfileInt(section,"AttackBySD",1,path);
	...
	this->m_DiabloEventDamageRate = GetPrivateProfileInt(section,"DiabloEventDamageRate",0,path);
	...
	this->m_DamageMax[0] = GetPrivateProfileInt(section,"DWMaxDMG",0,path);
	this->m_DamageMax[1] = GetPrivateProfileInt(section,"DKMaxDMG",0,path);
	this->m_DamageMax[2] = GetPrivateProfileInt(section,"FEMaxDMG",0,path);
	this->m_DamageMax[3] = GetPrivateProfileInt(section,"MGMaxDMG",0,path);
	this->m_DamageMax[4] = GetPrivateProfileInt(section,"DLMaxDMG",0,path);
	this->m_DamageMax[5] = GetPrivateProfileInt(section,"SUMaxDMG",0,path);
	#if(GAMESERVER_UPDATE>=601)
	this->m_DamageMax[6] = GetPrivateProfileInt(section,"RFMaxDMG",0,path);
	#endif
...
void CServerInfo::ReadCommandInfo(char* section,char* path) // OK
{
	...
	this->m_CommandResetNeedItem = GetPrivateProfileInt(section,"CommandResetNeedItem",0,path);
	this->m_CommandResetItemID = GetPrivateProfileInt(section,"CommandResetItemID",0,path);
	this->m_CommandResetItemLevel = GetPrivateProfileInt(section,"CommandResetItemLevel",0,path);
	this->m_CommandResetItemGR = GetPrivateProfileInt(section,"CommandResetItemForGR",0,path);
	this->m_CommandResetItemCount[0] = GetPrivateProfileInt(section,"CommandResetItemCount_AL0",0,path);
	this->m_CommandResetItemCount[1] = GetPrivateProfileInt(section,"CommandResetItemCount_AL1",0,path);
	this->m_CommandResetItemCount[2] = GetPrivateProfileInt(section,"CommandResetItemCount_AL2",0,path);
	this->m_CommandResetItemCount[3] = GetPrivateProfileInt(section,"CommandResetItemCount_AL3",0,path);

	this->m_CommandResetGiveCredits[0] = GetPrivateProfileInt(section,"CommandResetCredits_AL0",0,path);
	this->m_CommandResetGiveCredits[1] = GetPrivateProfileInt(section,"CommandResetCredits_AL1",0,path);
	this->m_CommandResetGiveCredits[2] = GetPrivateProfileInt(section,"CommandResetCredits_AL2",0,path);
	this->m_CommandResetGiveCredits[3] = GetPrivateProfileInt(section,"CommandResetCredits_AL3",0,path);
	...
	this->m_CommandBanChatSwitch = GetPrivateProfileInt(section,"CommandBanChatSwitch",0,path);

	this->m_CommandBanChatGameMasterLevel = GetPrivateProfileInt(section,"CommandBanChatGameMasterLevel",0,path);

	this->m_CommandMLResetSwitch = GetPrivateProfileInt(section,"CommandMLResetSwitch",0,path);

	this->m_CommandMLResetPrice_Zen[0] = GetPrivateProfileInt(section,"MLResetPriceZen_AL0",0,path);
	this->m_CommandMLResetPrice_Zen[1] = GetPrivateProfileInt(section,"MLResetPriceZen_AL1",0,path);
	this->m_CommandMLResetPrice_Zen[2] = GetPrivateProfileInt(section,"MLResetPriceZen_AL2",0,path);
	this->m_CommandMLResetPrice_Zen[3] = GetPrivateProfileInt(section,"MLResetPriceZen_AL3",0,path);

	this->m_CommandMLResetPrice_WCoinC[0] = GetPrivateProfileInt(section,"MLResetPriceWCoinC_AL0",0,path);
	this->m_CommandMLResetPrice_WCoinC[1] = GetPrivateProfileInt(section,"MLResetPriceWCoinC_AL1",0,path);
	this->m_CommandMLResetPrice_WCoinC[2] = GetPrivateProfileInt(section,"MLResetPriceWCoinC_AL2",0,path);
	this->m_CommandMLResetPrice_WCoinC[3] = GetPrivateProfileInt(section,"MLResetPriceWCoinC_AL3",0,path);

	this->m_CommandMLResetPrice_WCoinP[0] = GetPrivateProfileInt(section,"MLResetPriceWCoinP_AL0",0,path);
	this->m_CommandMLResetPrice_WCoinP[1] = GetPrivateProfileInt(section,"MLResetPriceWCoinP_AL1",0,path);
	this->m_CommandMLResetPrice_WCoinP[2] = GetPrivateProfileInt(section,"MLResetPriceWCoinP_AL2",0,path);
	this->m_CommandMLResetPrice_WCoinP[3] = GetPrivateProfileInt(section,"MLResetPriceWCoinP_AL3",0,path);

	this->m_CommandMLResetPrice_WCoinG[0] = GetPrivateProfileInt(section,"MLResetPriceWCoinG_AL0",0,path);
	this->m_CommandMLResetPrice_WCoinG[1] = GetPrivateProfileInt(section,"MLResetPriceWCoinG_AL1",0,path);
	this->m_CommandMLResetPrice_WCoinG[2] = GetPrivateProfileInt(section,"MLResetPriceWCoinG_AL2",0,path);
	this->m_CommandMLResetPrice_WCoinG[3] = GetPrivateProfileInt(section,"MLResetPriceWCoinG_AL3",0,path);

	this->m_CommandMLResetPrice_Credits[0] = GetPrivateProfileInt(section,"MLResetPriceCredits_AL0",0,path);
	this->m_CommandMLResetPrice_Credits[1] = GetPrivateProfileInt(section,"MLResetPriceCredits_AL1",0,path);
	this->m_CommandMLResetPrice_Credits[2] = GetPrivateProfileInt(section,"MLResetPriceCredits_AL2",0,path);
	this->m_CommandMLResetPrice_Credits[3] = GetPrivateProfileInt(section,"MLResetPriceCredits_AL3",0,path);
}
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->UseWrongPacketsCheck = GetPrivateProfileInt(section,"UseWrongPacketsCheck",0,path);
	
	this->m_AntihackCRC = GetPrivateProfileInt(section,"AntihackCRC",0,path);

	this->m_DllCRC = GetPrivateProfileInt(section,"LtpCRC",0,path);

	this->m_MainCRC = GetPrivateProfileInt(section,"MainCRC",0,path);
	...
	this->m_PetsRenew = GetPrivateProfileInt(section,"PetsRenew",0,path);
	...
	this->m_ShowKillMessage = GetPrivateProfileInt(section,"ShowKillMessage",0,path);
	...
	this->m_QuestNPCTeleport = GetPrivateProfileInt(section,"QuestNPCTeleport",0,path);

	this->m_QuestNPCTeleportTime = GetPrivateProfileInt(section,"QuestNPCTeleportTime",0,path);
	...
	this->m_MaxPKCount = GetPrivateProfileInt(section,"MaxPKCount",100,path);

	this->m_MaxPKKillCount = GetPrivateProfileInt(section,"MaxPKKillCount",100,path);
	...
	this->m_MaxExperienceMonsterLevel[0] = GetPrivateProfileInt(section,"MaxExperienceMonsterLevel_AL0",0,path);

	this->m_MaxExperienceMonsterLevel[1] = GetPrivateProfileInt(section,"MaxExperienceMonsterLevel_AL1",0,path);

	this->m_MaxExperienceMonsterLevel[2] = GetPrivateProfileInt(section,"MaxExperienceMonsterLevel_AL2",0,path);

	this->m_MaxExperienceMonsterLevel[3] = GetPrivateProfileInt(section,"MaxExperienceMonsterLevel_AL3",0,path);
	...
	this->m_MoneyMasterAmountDropRate[0] = GetPrivateProfileInt(section,"MoneyMasterAmountDropRate_AL0",100,path);

	this->m_MoneyMasterAmountDropRate[1] = GetPrivateProfileInt(section,"MoneyMasterAmountDropRate_AL1",100,path);

	this->m_MoneyMasterAmountDropRate[2] = GetPrivateProfileInt(section,"MoneyMasterAmountDropRate_AL2",100,path);

	this->m_MoneyMasterAmountDropRate[3] = GetPrivateProfileInt(section,"MoneyMasterAmountDropRate_AL3",100,path);
	...
	this->m_FKCreateType = GetPrivateProfileInt(section,"FKCreateType",0,path);
	...
	this->m_CharacterDeleteGReset[0] = GetPrivateProfileInt(section,"CharacterDeleteGReset_AL0",0,path);
	this->m_CharacterDeleteGReset[1] = GetPrivateProfileInt(section,"CharacterDeleteGReset_AL1",0,path);
	this->m_CharacterDeleteGReset[2] = GetPrivateProfileInt(section,"CharacterDeleteGReset_AL2",0,path);
	this->m_CharacterDeleteGReset[3] = GetPrivateProfileInt(section,"CharacterDeleteGReset_AL3",0,path);

	this->m_CharacterDeleteReset[0] = GetPrivateProfileInt(section,"CharacterDeleteReset_AL0",0,path);
	this->m_CharacterDeleteReset[1] = GetPrivateProfileInt(section,"CharacterDeleteReset_AL1",0,path);
	this->m_CharacterDeleteReset[2] = GetPrivateProfileInt(section,"CharacterDeleteReset_AL2",0,path);
	this->m_CharacterDeleteReset[3] = GetPrivateProfileInt(section,"CharacterDeleteReset_AL3",0,path);

	this->m_CharacterDeleteLevel[0] = GetPrivateProfileInt(section,"CharacterDeleteLevel_AL0",0,path);
	this->m_CharacterDeleteLevel[1] = GetPrivateProfileInt(section,"CharacterDeleteLevel_AL1",0,path);
	this->m_CharacterDeleteLevel[2] = GetPrivateProfileInt(section,"CharacterDeleteLevel_AL2",0,path);
	this->m_CharacterDeleteLevel[3] = GetPrivateProfileInt(section,"CharacterDeleteLevel_AL3",0,path);
	...
	this->m_PartyMaxUser = GetPrivateProfileInt(section,"PartyMaxUser",0,path);

	if(!gProtection.GetCustomState(CUSTOM_NEW_PARTY))
	{
		this->m_PartyMaxUser = 5;
	}
	...
	this->m_PartyUseOriginalPartyExpFormula = GetPrivateProfileInt(section,"PartyUseOriginalPartyExpFormula",1,path);

	for(int i=0;i<MAX_PARTY_USER;i++)
	{
		char text1[50];
		char text2[50];
		sprintf(text1,"PartyGeneralExperience%d",i+1);
		sprintf(text2,"PartySpecialExperience%d",i+1);

		this->m_PartyGeneralExperience[i] = GetPrivateProfileInt(section,text1,0,path);
		this->m_PartySpecialExperience[i] = GetPrivateProfileInt(section,text2,0,path);
	...
	this->gParty3ExpPercent = GetPrivateProfileInt(section,"Party3ExpPercent", 230, path);
	this->gParty4ExpPercent = GetPrivateProfileInt(section,"Party4ExpPercent", 270, path);
	this->gParty5ExpPercent = GetPrivateProfileInt(section,"Party5ExpPercent", 300, path);
	this->gPartyExpPercentOther = GetPrivateProfileInt(section,"PartyExpPercentOther", 120, path);

	this->gParty2ExpSetPercent = GetPrivateProfileInt(section,"Party2ExpSetPercent", 160, path);
	this->gParty3ExpSetPercent = GetPrivateProfileInt(section,"Party3ExpSetPercent", 180, path);
	this->gParty4ExpSetPercent = GetPrivateProfileInt(section,"Party4ExpSetPercent", 200, path);
	this->gParty5ExpSetPercent = GetPrivateProfileInt(section,"Party5ExpSetPercent", 220, path);
	this->gParty1ExpSetPercent = GetPrivateProfileInt(section,"Party1ExpSetPercent", 150, path);
	...
	this->m_DisconnectOnConenct[0] = GetPrivateProfileInt(section,"DisconnectOnConnect_AL0",0,path);
	this->m_DisconnectOnConenct[1] = GetPrivateProfileInt(section,"DisconnectOnConnect_AL1",0,path);
	this->m_DisconnectOnConenct[2] = GetPrivateProfileInt(section,"DisconnectOnConnect_AL2",0,path);
	this->m_DisconnectOnConenct[3] = GetPrivateProfileInt(section,"DisconnectOnConnect_AL3",0,path);

	this->m_AutoHealthPotionEnable = GetPrivateProfileInt(section, "AutoHealthPotionEnable", 0, path);
	
	this->m_UseAutoHealthPotion[0] = GetPrivateProfileInt(section,"UseAutoHealthPotion_AL0",0,path);
	this->m_UseAutoHealthPotion[1] = GetPrivateProfileInt(section,"UseAutoHealthPotion_AL1",0,path);
	this->m_UseAutoHealthPotion[2] = GetPrivateProfileInt(section,"UseAutoHealthPotion_AL2",0,path);
	this->m_UseAutoHealthPotion[3] = GetPrivateProfileInt(section,"UseAutoHealthPotion_AL3",0,path);

	this->m_AutoHealthPotionDelay[0] = GetPrivateProfileInt(section,"AutoHealthPotionDelay_AL0",0,path);
	this->m_AutoHealthPotionDelay[1] = GetPrivateProfileInt(section,"AutoHealthPotionDelay_AL1",0,path);
	this->m_AutoHealthPotionDelay[2] = GetPrivateProfileInt(section,"AutoHealthPotionDelay_AL2",0,path);
	this->m_AutoHealthPotionDelay[3] = GetPrivateProfileInt(section,"AutoHealthPotionDelay_AL3",0,path);
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	...
	this->m_CustomNoobBuffMaxReset[0] = GetPrivateProfileInt(section,"CustomNoobBuffMaxReset_AL0",0,path);

	this->m_CustomNoobBuffMaxReset[1] = GetPrivateProfileInt(section,"CustomNoobBuffMaxReset_AL1",0,path);

	this->m_CustomNoobBuffMaxReset[2] = GetPrivateProfileInt(section,"CustomNoobBuffMaxReset_AL2",0,path);

	this->m_CustomNoobBuffMaxReset[3] = GetPrivateProfileInt(section,"CustomNoobBuffMaxReset_AL3",0,path);

	this->m_CustomGuildBuffConstA = GetPrivateProfileInt(section,"CustomGuildBuffConstA",0,path);

	this->m_CustomGuildBuffStartReset = GetPrivateProfileInt(section,"CustomGuildBuffStartReset",0,path);

	this->m_CustomGuildBuffMin = GetPrivateProfileInt(section,"CustomGuildBuffMinValue",0,path);

	this->m_CustomGuildBuffMax = GetPrivateProfileInt(section,"CustomGuildBuffMaxValue",0,path);
	...
	this->m_CustomResetPoint[0] = GetPrivateProfileInt(section,"CustomResetPoints_AL0",0,path);

	this->m_CustomResetPoint[1] = GetPrivateProfileInt(section,"CustomResetPoints_AL1",0,path);

	this->m_CustomResetPoint[2] = GetPrivateProfileInt(section,"CustomResetPoints_AL2",0,path);

	this->m_CustomResetPoint[3] = GetPrivateProfileInt(section,"CustomResetPoints_AL3",0,path);

	this->m_CustomResetPointPriceWC[0] = GetPrivateProfileInt(section,"CustomResetPointsPriceWC_AL0",0,path);

	this->m_CustomResetPointPriceWC[1] = GetPrivateProfileInt(section,"CustomResetPointsPriceWC_AL1",0,path);

	this->m_CustomResetPointPriceWC[2] = GetPrivateProfileInt(section,"CustomResetPointsPriceWC_AL2",0,path);

	this->m_CustomResetPointPriceWC[3] = GetPrivateProfileInt(section,"CustomResetPointsPriceWC_AL3",0,path);

	this->m_CustomResetPointPricePoint[0] = GetPrivateProfileInt(section,"CustomResetPointsPricePoint_AL0",0,path);

	this->m_CustomResetPointPricePoint[1] = GetPrivateProfileInt(section,"CustomResetPointsPricePoint_AL1",0,path);

	this->m_CustomResetPointPricePoint[2] = GetPrivateProfileInt(section,"CustomResetPointsPricePoint_AL2",0,path);

	this->m_CustomResetPointPricePoint[3] = GetPrivateProfileInt(section,"CustomResetPointsPricePoint_AL3",0,path);

	this->m_ExtendedCharacters[0] = GetPrivateProfileInt(section,"ExtendedCharacters_AL0",0,path);

	this->m_ExtendedCharacters[1] = GetPrivateProfileInt(section,"ExtendedCharacters_AL1",0,path);

	this->m_ExtendedCharacters[2] = GetPrivateProfileInt(section,"ExtendedCharacters_AL2",0,path);

	this->m_ExtendedCharacters[3] = GetPrivateProfileInt(section,"ExtendedCharacters_AL3",0,path);

	this->m_CustomDeathMessage = GetPrivateProfileInt(section,"CustomDeathMessageSwitch",0,path);
}
...
void CServerInfo::ReadEventInfo(char* section,char* path) // OK
{
	...
	this->m_BloodCastleNewLevelSettings = GetPrivateProfileInt(section,"BloodCastleNewLevelSettings",0,path);

	this->m_BloodCastleMaxDayEntry[0] = GetPrivateProfileInt(section, "BloodCastleMaxDayEntry_AL0", 0, path);
	this->m_BloodCastleMaxDayEntry[1] = GetPrivateProfileInt(section, "BloodCastleMaxDayEntry_AL1", 0, path);
	this->m_BloodCastleMaxDayEntry[2] = GetPrivateProfileInt(section, "BloodCastleMaxDayEntry_AL2", 0, path);
	this->m_BloodCastleMaxDayEntry[3] = GetPrivateProfileInt(section, "BloodCastleMaxDayEntry_AL3", 0, path);
	...
	this->m_DevilSquareNewLevelSettings = GetPrivateProfileInt(section,"DevilSquareNewLevelSettings",0,path);

	this->m_DevilSquareMaxDayEntry[0] = GetPrivateProfileInt(section, "DevilSquareMaxDayEntry_AL0", 0, path);
	this->m_DevilSquareMaxDayEntry[1] = GetPrivateProfileInt(section, "DevilSquareMaxDayEntry_AL1", 0, path);
	this->m_DevilSquareMaxDayEntry[2] = GetPrivateProfileInt(section, "DevilSquareMaxDayEntry_AL2", 0, path);
	this->m_DevilSquareMaxDayEntry[3] = GetPrivateProfileInt(section, "DevilSquareMaxDayEntry_AL3", 0, path);
	...
	this->m_IllusionTempleNewLevelSettings = GetPrivateProfileInt(section,"IllusionTempleNewLevelSettings",0,path);
...
void CServerInfo::ReadHackInfo(char* section,char* path) // OK
{
	...
	gDupeScanner.load_settings(path,section);
}
