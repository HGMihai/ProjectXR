...
CItemOptionRate::CItemOptionRate() // OK
{
	...
	this->m_ItemExcellentRateInfo.clear();
}
		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 7)
				{
					...
					ITEM_EXCELLENT_RATE_INFO info;

					info.Index = lpMemScript->GetNumber();

					for(int n=0;n < MAX_ITEM_EXCELLENT_RATE;n++)
					{
						info.Rate[n] = lpMemScript->GetAsNumber();
					}

					this->m_ItemExcellentRateInfo.insert(std::pair<int,ITEM_EXCELLENT_RATE_INFO>(info.Index,info));
				}
				else
				{
					break;
...
bool CItemOptionRate::GetItemExcellent(int index,int max_option,BYTE* option)
{
	std::map<int,ITEM_EXCELLENT_RATE_INFO>::iterator it = this->m_ItemExcellentRateInfo.find(index);

	if(it == this->m_ItemExcellentRateInfo.end())
	{
		return 0;
	}
	...
	for(int n=0;n < /*MAX_ITEM_EXCELLENT_RATE*/max_option;n++)
	{
		RandomManager.AddElement(n,it->second.Rate[n]);
	}
...
bool CItemOptionRate::MakeNewOption(int ItemIndex,int value,BYTE* option, int index) // OK
{
	(*option) = 0;
	...
	//LogAdd(LOG_TEST,"Need to make %d exc options",value);
	...
	while(true)
	{
		...
		this->GetItemExcellent(index,MaxNewOption,&exc);
		
		//int exc_value = 1 << (GetLargeRand()%MaxNewOption);
		int exc_value = 1 << exc;

		if(((*option) & exc_value) == 0)
		{
			(*option) |= exc_value;
			count++;
		}
