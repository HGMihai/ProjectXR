...
void CNotice::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.Color = RGBA(lpMemScript->GetAsNumber(),lpMemScript->GetAsNumber(),lpMemScript->GetAsNumber(),255);
...
void CNotice::GCNoticeSendToMap(int Map,BYTE type,BYTE count,BYTE opacity,WORD delay,DWORD color,BYTE speed,char* message,...)
{
	char buff[256] = {0};
