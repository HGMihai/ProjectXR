...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if (gItemToPoints.Dialog(lpObj,lpNpc))
	{
		return 1;
	}

	if (gSuperPoints.Dialog(lpObj, lpNpc))
	{
		return 1;
	}

	if (gGuildWarehouse.Dialog(lpObj,lpNpc))
	{
		return 1;
	}
	...
	if (gGRSystem.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
	...
	if (gMarrySystem.NPCTalk(lpNpc,lpObj) != 0)
	{
		return 1;
	}

	if (gDiablo.NPCTalk(lpNpc,lpObj) != 0)
	{
		return 1;
	}

	/*if (gMapQuestSystem.dialog(lpObj, lpNpc))
		return true;*/
	...
	if (gTeleporter.Dialog(lpObj, lpNpc))
	{
		return 1;
	}

	if (gTeleportPanel.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
...
{
	if(gServerInfo.m_BloodCastleMaxDayEntry[lpObj->AccountLevel] == 0)
		gBloodCastle.NpcAngelMessenger(lpObj);
	else
		GDSendEntryCountRequest(lpObj,0);
}
...
{
	if (gServerInfo.m_DevilSquareMaxDayEntry[lpObj->AccountLevel] == 0)
		gDevilSquare.NpcCharon(lpObj);
	else
		GDSendEntryCountRequest(lpObj, 1);
...
{
	if (lpObj->Interface.type == INTERFACE_CHAOS_BOX || lpObj->Interface.type == INTERFACE_TRADE || lpObj->Interface.type == INTERFACE_PERSONAL_SHOP || lpObj->Interface.type == INTERFACE_TRAINER)
	{
		return;
	}
...
void CNpcTalk::NpcLahap(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	lpObj->Interface.use = 1;
	lpObj->Interface.type = INTERFACE_COMMON;
	lpObj->Interface.state = 0;
...
void CNpcTalk::NpcElfBuffer(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if (gDisableFunctions.m_disableElfBufferQuest == 0)
	{
...
void CNpcTalk::CGNpcTalkRecv(PMSG_NPC_TALK_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->PKLevel > 4)
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GetMessage(512));
		return;
	}
	...
	#if (GAMESERVER_UPDATE == 603)
	if (lpNpc->IsDonateShop == 1)
	{
		int CurrentPage = lpObj->DonateShopPage;
		int ShopNumber = gShopManager.GetShopNumberByDonateShopIndex(lpNpc->Class,CurrentPage);

		lpObj->TargetShopNumber = ShopNumber;
		gShopManager.GCShopItemListSendByIndex(ShopNumber,aIndex);

		gDonateShop.GCSendState(STATE_SHOW, lpObj->Index);

		gDonateShop.GCSendDonateItems(lpObj->Index);
	}
	else
	{
...
void CNpcTalk::CGNpcTalkCloseRecv(int aIndex) // OK
{
	...
	if( gObj[aIndex].Interface.type == 10 && gObj[aIndex].TargetShopNumber == 229 )
	{
		if (gServerInfo.m_QuestNPCTeleport != 0)
		{
	...
	switch(lpObj->Interface.type)
	{
		...
		case INTERFACE_SHOP:
			...
			gDonateShop.GCSendState(STATE_DONTSHOW, lpObj->Index);
			#endif
			break;
