...
bool CProtect::GetComputerHardwareId() // OK
{
	...
	std::ofstream out;
    out.open("ProtectHardwareId.txt");

	if (out.is_open())
    {
        out << this->m_ComputerHardwareId << std::endl;
    }
...
bool CProtect::AuthServerDataSend(eAuthServerType ServerType) // OK
{
	...
	memcpy(pMsg.CustomerName,gProtection.CustomerName,sizeof(pMsg.CustomerName));
...
void CProtect::StartAuth(eAuthServerType ServerType) // OK
{
	...
	if(this->ConnectToAuthServer("licenseinfo.ltp-team.com"/*this->GetEncryptedString(gProtectString3,sizeof(gProtectString3))*/,PROTECT_PORT) == 0)
	{
	...
	if(strcmp(this->m_AuthInfo.CustomerName,gProtection.CustomerName) != 0)
	{
		closesocket(this->m_socket);
		this->m_socket = INVALID_SOCKET;
		this->ErrorMessageBox();
