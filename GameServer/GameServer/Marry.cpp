...
void CMarry::Propose(int aIndex, int uIndex)
{
	...
	if (gObj[aIndex].Name == gObj[uIndex].Name)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You cannot get married to yourself, take a look around serve, it is full of potential partners");
		return;
	}

	if (!this->m_bMarryEnable)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"Marry System is disabled, sorry");
	...
	if (gObj[aIndex].IsMarried != 0 || gObj[uIndex].IsMarried != 0)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You or your fiance are already married!");
		return;
	}
	...
	if (isFemale(aIndex) == isFemale(uIndex) && !this->m_bHomoSexualAllow)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"We do not allow homo-sexual weddings.");
		return;
	}
	...
	if (this->m_iMarryNeedMoney > gObj[aIndex].Money || this->m_iMarryNeedMoney > gObj[uIndex].Money)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You or your fiance are short of money.");
		return;
	}
	...
	if (this->m_iMarryMinUserLevel > gObj[aIndex].Level || this->m_iMarryMinUserLevel > gObj[uIndex].Level)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You or your fiance are short of level.");
		return;
	}
	...
	if (!this->CheckPosition(aIndex, uIndex))
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You are not in church!");
		return;
	}
	...
	if (bCheck1 == false || bCheck2 == false)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You and your fiance need a special item to marry");
		return;
	}
	...
	gNotice.GCNoticeSend(uIndex,1,0,0,0,0,0,"%s wants to marry with you, you have 60 seconds to answer.",gObj[aIndex].Name);
	//GSProtocol.GCServerMsgStringSend(Lang.GetText(0,381), aIndex, 1);
	gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"Request was sent to your fiance");
...
bool CMarry::Accept(int aIndex)
{
	...
	if (gObj[aIndex].MarryRequested != 1)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"No one sent you marriage propose!");
		return false;
	}
	...
	if (GetTickCount() - gObj[aIndex].MarryRequestTime > 60000)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"Time for answer is finished. Sorry.");
	...
	if (gObj[uIndex].Money < gObj[uIndex].Money - this->m_iMarryNeedMoney || gObj[aIndex].Money < gObj[aIndex].Money - this->m_iMarryNeedMoney)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You do not have money!");
		return false;
	}
	...
	gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"Congratulations! You are married!");
	...
	gNotice.GCNoticeSend(uIndex,0,0,0,0,0,0,"Congratulations! You are married!");
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,"Congratulations! You are married!");
...
void CMarry::Divorce(int aIndex)
{
	if (gObj[aIndex].IsMarried == 0)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You are not married, maybe it is a time to find a good partner?");
		return;
	}
	...
	if (this->m_bDivorceAllow == false)
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"Divorcing is not allowed");
		return;
	}
	...
	if (!this->CheckDivorceItem(aIndex))
	{
		...
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You need a special item to divorce");
		return;
	}

	//GSProtocol.GCServerMsgStringSend(Lang.GetText(0,392), aIndex, 1);
	gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"You are divorced, enjoy your freedom till you can!");
	...
	if (lpTarget != 0)
	{
		...
		gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,"You are divorced, enjoy your freedom till you can!");
	...
	LogAdd(LOG_BLUE, "[Marry] Divorce: [%s] [%d]", gObj[aIndex].Name, (lpTarget->Name!=NULL)?lpTarget->Name:"null");
