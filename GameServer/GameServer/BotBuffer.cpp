	...
	if(this->Enabled == true)
	{
		for(int botNum=0;botNum<MAX_BOTBUFFER;botNum++)
		{
			if(this->bot[botNum].Enabled == true)
			{
				...
				if(aIndex >= 0)
				{
					//MsgBox("Set bot with index: %d",aIndex);
					...
					LogAdd(LOG_BLUE, "[BotBuffer][Bot:%d] Successfully set (aIndex: %d)",botNum,aIndex);
				}
			}
		}
	}
...
{
	if(gBotBuffer.bot[botindex].OnlyVip == 1 && lpObj->AccountLevel == 0)
	{
		GCChatTargetSend(lpBot, lpObj->Index, "Im only work for VIPs!");
		return;
	}
	...
	if (gBotBuffer.bot[botindex].PriceType == 0)
	{
		...
		{
			char Message[100];
			sprintf(Message,"Not enough %s!",gMessage.GetMessage(523));
			GCChatTargetSend(lpBot, lpObj->Index, Message);
			return;
		}
	}
	else if (gBotBuffer.bot[botindex].PriceType == 1)
	{
		if (lpObj->BankEx.Credits < gBotBuffer.bot[botindex].PriceCount)
		{
			char Message[100];
			sprintf(Message,"Not enough %s!",gMessage.GetMessage(527));
			GCChatTargetSend(lpBot, lpObj->Index, Message);
			return;
		}
	}
	else if (gBotBuffer.bot[botindex].PriceType == 2)
	{
		...
		{
			char Message[100];
			sprintf(Message,"Not enough %s!",gMessage.GetMessage(524));
			GCChatTargetSend(lpBot, lpObj->Index, Message);
			return;
		}
	}
	else if (gBotBuffer.bot[botindex].PriceType == 3)
	{
		...
		{
			char Message[100];
			sprintf(Message,"Not enough %s!",gMessage.GetMessage(525));
			GCChatTargetSend(lpBot, lpObj->Index, Message);
			return;
		}
	}
	else if (gBotBuffer.bot[botindex].PriceType == 4)
	{
		if (GoblinPoint < gBotBuffer.bot[botindex].PriceCount)
		{
			char Message[100];
			sprintf(Message,"Not enough %s!",gMessage.GetMessage(526));
			GCChatTargetSend(lpBot, lpObj->Index, Message);
			return;
		}
	}
	...
	{
		GCChatTargetSend(lpBot, lpObj->Index, "Wrong currency! Check config!!!");
		return;
	}
	...
	{
		lpObj->BankEx.Credits -= gBotBuffer.bot[botindex].PriceCount;
	...
	for(int x=0;x<gBotBuffer.bot[botindex].SkillCount;x++)
	{
		switch(gBotBuffer.bot[botindex].skill[x].skill)
		{
			...
			case SKILL_GREATER_DEFENSE:
			{
				...
				value += (value*gMasterSkillTree.GetMasterSkillValue(lpObj,SKILL_GREATER_DEFENSE))/100;
			...
			case SKILL_GREATER_DAMAGE:
			{
				...
				value += (value*gMasterSkillTree.GetMasterSkillValue(lpObj,SKILL_GREATER_DAMAGE))/100;
