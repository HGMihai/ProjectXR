		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					info.EnableSlotPendant = lpMemScript->GetAsNumber();
				...
				else if (section == 3)
				{
					...
					CUSTOM_JEWEL_ITEM_REPLACE_INFO info;
					
					memset(&info,0,sizeof(info));

					info.index = lpMemScript->GetNumber();

					info.old_item_index = lpMemScript->GetAsNumber();
					
					info.new_item_index = lpMemScript->GetAsNumber();

					this->set_replace_info(info);
				}
				else
				{
					break;
...
void CCustomJewel::set_replace_info(CUSTOM_JEWEL_ITEM_REPLACE_INFO info)
{
	if(info.index < 0 || info.index >= MAX_CUSTOM_JEWEL)
	{
		return;
	...
	this->m_CustomJewelInfo[info.index].replace_info.push_back(info);
...
CUSTOM_JEWEL_ITEM_REPLACE_INFO* CCustomJewel::get_replace_info(int jewel_item_id, int item_id)
{
	CUSTOM_JEWEL_INFO* lpInfo = this->GetInfoByItem(jewel_item_id);
	...
	for(int i=0; i< lpInfo->replace_info.size(); i++)
	{
		if(lpInfo->replace_info[i].old_item_index == item_id)
			return &lpInfo->replace_info[i];
	}
...
bool CCustomJewel::CheckCustomJewelApplyItem(int ItemIndex,CItem* lpItem) // OK
{
	...
	if(lpItem->m_Index > GET_ITEM(12,6) && (lpItem->m_Index < GET_ITEM(12,36) || lpItem->m_Index > GET_ITEM(12,43)) 
		...
		lpItem->m_Index != GET_ITEM(13,30) && gCustomWing.CheckCustomWingByItem(lpItem->m_Index) == 0)
	{
		if((lpInfo->EnableSlotRing == 0 && lpItem->IsRing()) || (lpInfo->EnableSlotPendant == 0 && lpItem->IsPendant()))
		{
			return 0;
		}
	}
	...
	if(lpInfo->MaxItemOption1 != -1 && lpItem->m_SkillOption > lpInfo->MaxItemOption1)
	{
		return 0;
	}

	if(lpInfo->MaxItemOption2 != -1 && lpItem->m_LuckOption > lpInfo->MaxItemOption2)
	{
		return 0;
	}

	if(lpInfo->MinItemOption3 != -1 && lpItem->m_Option < lpInfo->MinItemOption3)
	{
		return 0;
	}

	if(lpInfo->MaxItemOption3 != -1 && lpItem->m_Option > lpInfo->MaxItemOption3)
	{
		return 0;
	}
	...
	if(!this->check_custom_jewel_change_item(lpInfo,lpItem))
		return 0;
...
{
	if (lpInfo->SuccessInfo.SetOption > 0)
	{
		if (lpInfo->MaxItemSetOption != -1 && gSetItemType.GetSetOption(lpItem->m_Index) == 0)
		{
			return 0;
		}
	}
...
bool CCustomJewel::check_custom_jewel_change_item(CUSTOM_JEWEL_INFO* lpInfo, CItem* lpItem)
{
	if(lpInfo->replace_info.size() == 0)
		return true;
	
	for(int i=0; i<lpInfo->replace_info.size(); i++)
	{
		if(lpInfo->replace_info[i].old_item_index == lpItem->m_Index)
			return true;
	}
...
int CCustomJewel::GetCustomJewelSetOption(CItem* lpItem,int value) // OK
{
	...
	{
		int option = 0;
		if(gSetItemType.CheckSetItemType(lpItem->m_Index) == 0 || value == 0)
		{
			option = 0;
			return 0;
		}
		...
		return ((lpItem->m_SetOption)=(option));
	}
}
...
int CCustomJewel::custom_jewel_replace_item(int aIndex, CItem* lpItem, int slot,CUSTOM_JEWEL_ITEM_REPLACE_INFO* replace_info)
{
	if(replace_info == nullptr)
		return false;

	if(lpItem->m_Index != replace_info->old_item_index)
		return false;
	
	lpItem->m_Index = replace_info->new_item_index;

	gItemManager.GCItemChangeSend(aIndex,slot);
