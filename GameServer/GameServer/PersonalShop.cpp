...
void CPersonalShop::CGPShopSetItemPriceRecvEx(PMSG_PSHOP_SET_ITEM_PRICE_EX_RECV* lpMsg,int aIndex)
{
	...
	int price = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->Price1[3],lpMsg->Price1[2]),MAKE_NUMBERW(lpMsg->Price1[1],lpMsg->Price1[0]));

	int iBonusPrice = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->Price2[3],lpMsg->Price2[2]),MAKE_NUMBERW(lpMsg->Price2[1],lpMsg->Price2[0]));
	short aBlessCnt = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->Price3[3],lpMsg->Price3[2]),MAKE_NUMBERW(lpMsg->Price3[1],lpMsg->Price3[0]));
	short aSoulCnt = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->Price4[3],lpMsg->Price4[2]),MAKE_NUMBERW(lpMsg->Price4[1],lpMsg->Price4[0]));
	short aChaosCnt = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->Price5[3],lpMsg->Price5[2]),MAKE_NUMBERW(lpMsg->Price5[1],lpMsg->Price5[0]));
	short aLifeCnt = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->Price6[3],lpMsg->Price6[2]),MAKE_NUMBERW(lpMsg->Price6[1],lpMsg->Price6[0]));

	/*int price = lpMsg->Price[0];
	int iBonusPrice = lpMsg->Price[1];
	int aBlessCnt = lpMsg->Price[2];
	int aSoulCnt = lpMsg->Price[3];
	int aChaosCnt = lpMsg->Price[4];
	int aLifeCnt = lpMsg->Price[5];*/

	//int iBonusPrice = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->BonusPrice[4], lpMsg->BonusPrice[3]), MAKE_NUMBERW(lpMsg->BonusPrice[2], lpMsg->BonusPrice[1]));
	...
	if(price <= 0 || iBonusPrice < 0 || aBlessCnt < 0 || aSoulCnt < 0 || aChaosCnt < 0 || aLifeCnt < 0)
	{
		this->GCPShopSetItemPriceSend(aIndex,4,lpMsg->slot);
		return;
	}
	...
	if(!g_PersonalShopEx.CheckPrice(price, iBonusPrice, aBlessCnt, aSoulCnt, aChaosCnt, aLifeCnt))
	{
		LogAdd(LOG_RED,"[PShop] [%s][%s] ERROR : Personal Shop Item Check Price", gObj[aIndex].Account, gObj[aIndex].Name);
		this->GCPShopSetItemPriceSend(aIndex, 0, lpMsg->slot);
		return;
	}
#endif

	lpObj->Inventory[lpMsg->slot].m_PShopValue = price;
	lpObj->Inventory[lpMsg->slot].m_iPShopBonus = iBonusPrice;
	lpObj->Inventory[lpMsg->slot].m_PShopBlessValue = aBlessCnt;
	lpObj->Inventory[lpMsg->slot].m_PShopSoulValue = aSoulCnt;
	lpObj->Inventory[lpMsg->slot].m_PShopChaosValue = aChaosCnt;
	lpObj->Inventory[lpMsg->slot].m_PShopLifeValue = aLifeCnt;
...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	gLog.Output(LOG_TRADE,"[SellPesonalShopItem][%s][%s] - (Account: %s, Name: %s, Value: %d, Index: %04d, Level: %02d, Serial: %08X, Option1: %01d, Option2: %01d, Option3: %01d, NewOption: %03d, JewelOfHarmonyOption: %03d, ItemOptionEx: %03d, SocketOption: %03d, %03d, %03d, %03d, %03d)",lpTarget->Account,lpTarget->Name,lpObj->Account,lpObj->Name,lpTarget->Inventory[lpMsg->slot].m_PShopValue,lpTarget->Inventory[lpMsg->slot].m_Index,lpTarget->Inventory[lpMsg->slot].m_Level,lpTarget->Inventory[lpMsg->slot].m_Serial,lpTarget->Inventory[lpMsg->slot].m_SkillOption,lpTarget->Inventory[lpMsg->slot].m_LuckOption,lpTarget->Inventory[lpMsg->slot].m_Option,lpTarget->Inventory[lpMsg->slot].m_NewOption,lpTarget->Inventory[lpMsg->slot].m_JewelOfHarmonyOption,lpTarget->Inventory[lpMsg->slot].m_ItemOptionEx,lpTarget->Inventory[lpMsg->slot].m_SocketOption[0],lpTarget->Inventory[lpMsg->slot].m_SocketOption[1],lpTarget->Inventory[lpMsg->slot].m_SocketOption[2],lpTarget->Inventory[lpMsg->slot].m_SocketOption[3],lpTarget->Inventory[lpMsg->slot].m_SocketOption[4]);
