...
void CViewport::GCViewportCostumeSend(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		gCostumeSystem.GCCostumeSend(lpTarget,aIndex);
	}
}
...
void CViewport::GCViewportNewPetSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		//if (lpObj->VpPlayer[n].index == aIndex)
		//{
		//	continue;
		//}
...
void CViewport::GCViewportPlayerSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.max_life = lpTarget->MaxLife+lpTarget->AddLife;

		info.reset = lpTarget->Reset;
		info.master_reset = lpTarget->MasterReset;
		info.shield = lpTarget->Shield;
		info.max_shield = lpTarget->MaxShield + lpTarget->AddShield;
		...
		info.count = gEffectManager.GenerateEffectList(lpTarget,info.buff_effect_list);

		memcpy(&send[size],&info,sizeof(info));
		size += InfoSize - (MAX_EFFECT_LIST - info.count);
...
/*void CViewport::GCViewportPlayerSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		//info.MuunItem[0] = SET_NUMBERHB(-1);//SET_NUMBERHB(lpTarget->MuunInventory[0].IsItem()?lpTarget->MuunInventory[0].m_Index:-1);
		//info.MuunItem[1] = SET_NUMBERLB(-1);//SET_NUMBERLB(lpTarget->MuunInventory[0].IsItem()?lpTarget->MuunInventory[0].m_Index:-1);

		//info.MuunSubItem[0]=SET_NUMBERHB(lpTarget->m_wMuunSubItem);
		//info.MuunSubItem[1]=SET_NUMBERLB(lpTarget->m_wMuunSubItem);
...
void CViewport::GCViewportChangeSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.MuunItem[0] = SET_NUMBERHB(lpTarget->MuunInventory[0].IsItem()?lpTarget->MuunInventory[0].m_Index:-1);
		info.MuunItem[1] = SET_NUMBERLB(lpTarget->MuunInventory[0].IsItem()?lpTarget->MuunInventory[0].m_Index:-1);
...
void CViewport::GCViewportSimplePlayerSend(LPOBJ lpObj) // OK
{
	...
	info.DirAndPkLevel = (lpObj->Dir*16) | (((this->CheckCustomEventPkViewport(lpObj,lpObj)==0)?lpObj->PKLevel:6) & 0x0F);
		...
		info.max_life = lpObj->MaxLife + lpObj->AddLife;

		info.reset = lpObj->Reset;
		info.master_reset = lpObj->MasterReset;
		info.shield = lpObj->Shield;
		info.max_shield = lpObj->MaxShield + lpObj->AddShield;
	...
	if(lpObj->MuunInventory[0].m_Index== -1)
	{
		info.MuunItem[0] = SET_NUMBERHB(lpObj->MuunInventory[1].IsItem()?lpObj->MuunInventory[1].m_Index:-1);
		info.MuunItem[1] = SET_NUMBERLB(lpObj->MuunInventory[1].IsItem()?lpObj->MuunInventory[1].m_Index:-1);
	}
	else
	{
		info.MuunItem[0] = SET_NUMBERHB(lpObj->MuunInventory[0].IsItem()?lpObj->MuunInventory[1].m_Index:-1);
		info.MuunItem[1] = SET_NUMBERLB(lpObj->MuunInventory[0].IsItem()?lpObj->MuunInventory[1].m_Index:-1);
	}
	info.MuunSubItem[0]=SET_NUMBERHB(lpObj->m_wMuunSubItem);
	info.MuunSubItem[1]=SET_NUMBERLB(lpObj->m_wMuunSubItem);
...
#pragma message("Отправка эффект листа клиенту");
	info.count = gEffectManager.GenerateEffectList(lpObj,info.buff_effect_list);
