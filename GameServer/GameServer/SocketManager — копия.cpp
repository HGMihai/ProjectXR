...
//unsigned long long pakcet_counter = 0;
...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		//for(int i=8;i<lpIoBuffer->size;i++)
		//{
		//	lpMsg[i-8] = lpIoBuffer->buff[i];
		//}

		BYTE test[8];

		memmove(test,&lpMsg[count],8);
		
		unsigned long long recv_index = *((unsigned long long*)test);
		...
		if (recv_index != gObj[index].recv_index)
		{
			...
			LogAdd(LOG_RED,"Error OnRecv Name: %s [%llu - %llu] size: %d",gObj[index].Name,recv_index,gObj[index].recv_index,lpIoBuffer->size);
			return 0;
		}
		...
		//memmove(lpMsg,&lpMsg[count+8],lpIoBuffer->size);

		for(int i=count+8;i<lpIoBuffer->size;i++)
		{
			lpMsg[count+i-8] = lpMsg[i];
		}
