...
void CInvasionManager::Load(char* path) // OK
{
	...
	for(int n=0;n < MAX_INVASION;n++)
	{
		this->m_InvasionInfo[n].NeedToShowMap = 0;
		...
		this->m_InvasionInfo[n].currentMonsterInfo.clear();
		this->m_InvasionInfo[n].currentRespawnInfo.clear();
	}
		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 1)
				{
					...
					this->m_InvasionInfo[index].NeedToShowMap = lpMemScript->GetAsNumber();
...
{
	if (lpInfo->currentRespawnInfo.empty() || lpInfo->currentMonsterInfo.empty())
	{
			...
			if(lpInfo->RespawnInfo[n].empty() == 0)
			{
				...
				for(std::vector<INVASION_MONSTER_INFO>::iterator it=lpInfo->MonsterInfo.begin();it != lpInfo->MonsterInfo.end();it++)
				{
					if(it->Group == lpRespawnInfo->Group)
					{
						...
						lpInfo->currentRespawnInfo.push_back(lpRespawnInfo);
						lpInfo->currentMonsterInfo.push_back(&(*it));
						//lpInfo->currentRespawnInfo = lpRespawnInfo;
						//lpInfo->currentMonsterInfo = &(*it);
					}
				}
			}
		}
	...
	{
		//LogAdd(LOG_TEST,"Map = %d",lpInfo->currentRespawnInfo->Map);

		std::vector<int> maps;

		for (int i=0;i<lpInfo->currentRespawnInfo.size();i++)
		{
			/*bool state=0;
		...
		if (lpInfo->NeedToShowMap)
		{
//			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(530),lpInfo->InvasionName,gMessage.GetMap(lpInfo->currentRespawnInfo->Map));
		}
		else
		{
...
void CInvasionManager::SetState_EMPTY(INVASION_INFO* lpInfo) // OK
{
	...
	lpInfo->currentRespawnInfo.clear();
	lpInfo->currentMonsterInfo.clear();
}
...
void CInvasionManager::SetState_START(INVASION_INFO* lpInfo) // OK
{
	...
	if (!lpInfo->currentRespawnInfo.empty() && !lpInfo->currentMonsterInfo.empty())
	{
		for	(int i=0; i < lpInfo->currentRespawnInfo.size(); i++)
		{
			this->SetMonster(lpInfo,lpInfo->currentRespawnInfo[i],lpInfo->currentMonsterInfo[i]);
		}
		//this->SetMonster(lpInfo,lpInfo->currentRespawnInfo,lpInfo->currentMonsterInfo);
	}
...
void CInvasionManager::SetMonster(INVASION_INFO* lpInfo,INVASION_RESPWAN_INFO* lpRespawnInfo,INVASION_MONSTER_INFO* lpMonsterInfo) // OK
{
	...
	for(int n=0;n < gMonsterSetBase.m_count;n++)
	{
		...
		LogAdd(LOG_BLUE, "[InvasionManager] [%s] Has spawned! Map %d X-%d Y-%d", lpObj->Name, lpObj->Map, lpObj->X, lpObj->Y);
		...
		if(lpInfo->NeedToShowMap && lpMonsterInfo->MonsterClass == lpInfo->BossIndex && !is_message_send)
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(530),lpInfo->InvasionName,gMessage.GetMap(lpRespawnInfo->Map));
...
void CInvasionManager::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	for(int n=0;n < MAX_INVASION;n++)
	{
		...
		gActiveInvasions.monster_del(lpObj->Class, true);
		...
		if (lpInfo->NeedToShowMap != 0)
		{
			if (lpObj->Class == lpInfo->BossIndex)
			{
				MONSTER_INFO* lpMonster = gMonsterManager.GetInfo(lpObj->Class);
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(531),lpTarget->Name,lpMonster->Name,gMessage.GetMap(lpObj->Map));
				continue;
			}
		}
		else
