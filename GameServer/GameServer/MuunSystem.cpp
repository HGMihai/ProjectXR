...
void CMuunSystem::CGReqRideSelect(PMSG_MUUN_RIDE_SELECT *lpMsg,int aIndex)
{
	...
	//MuunViewPortInfo.NumberH = SET_NUMBERHB(lpObj->MuunInventory[num].m_Index);
	//MuunViewPortInfo.NumberL = SET_NUMBERLB(lpObj->MuunInventory[num].m_Index);
...
void CMuunSystem::CGMuunConvertRecv(PMSG_MUUN_ITEM_CONVERT_RECV* lpMsg,int aIndex)
{
	...
	for(int n = 0;n<102;n++)
	{
	#if(GAMESERVER_UPDATE>=1100)
		if(lpObj->MuunInventory[n].m_Index==GET_ITEM(13,240) ||
			lpObj->MuunInventory[n].m_Index==GET_ITEM(16,240))
#else
		if(lpObj->MuunInventory[n].m_Index==GET_ITEM(13,240))
#endif
		{
			...
			//if(this->GetMuunSystemConvertInfo(lpObj->MuunInventory[n].m_Index,&MuunConvertInfo) == 0)
		}
