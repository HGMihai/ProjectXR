...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo, bool byreflect) // OK
{
	...
	if (lpTarget->Type == OBJECT_USER && lpObj->PKCount >= gServerInfo.m_MaxPKKillCount)
	{
		return 0;
	}
	...
	{
		if (DE_MAP_RANGE(lpObj->Map) == 1 && lpTarget->Type == OBJECT_MONSTER && lpTarget->Class == 12)
		{
			gDiablo.GetHit(lpObj, lpTarget);
		}
	}
	#pragma endregion
	...
	if(damage == 0)
	{
		...
		if(skill == SKILL_FALLING_SLASH || skill == SKILL_LUNGE || skill == SKILL_UPPERCUT || skill == SKILL_CYCLONE || skill == SKILL_SLASH || skill == SKILL_TWISTING_SLASH || skill == SKILL_RAGEFUL_BLOW || skill == SKILL_DEATH_STAB || skill == SKILL_CRESCENT_MOON_SLASH || skill == SKILL_STAR_FALL || skill == SKILL_IMPALE || skill == SKILL_FIRE_BREATH || skill == SKILL_ICE_ARROW || skill == SKILL_PENETRATION || skill == SKILL_FIRE_SLASH || skill == SKILL_POWER_SLASH || skill == SKILL_SPIRAL_SLASH || skill == SKILL_FROZEN_STAB || skill == SKILL_SWORD_SLASH || skill == SKILL_CHARGE || skill == SKILL_BLOOD_STORM)
		{
			if(skill != SKILL_IMPALE || lpObj->Inventory[8].m_Index == GET_ITEM(13,2) || lpObj->Inventory[8].m_Index == GET_ITEM(13,3) || lpObj->Inventory[8].m_Index == GET_ITEM(13,37))
			{
				...
				{
					//damage = ( damage * ( 200 + ( lpObj->Energy / 10 ) ) ) / 100;//(damage*lpObj->DKDamageMultiplierRate)/100;
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			{
				damage = (damage*gCharacterCalc.calculate_damage_rate_pvp(lpObj->Index))/100;
				damage = (damage*gCharacterCalc.calculate_damage_class_vs_class(lpObj->Index,lpTarget->Index))/100;
			}
			else
			{
			...
			if (gServerInfo.m_DamageMax[lpObj->Class] != 0)
			{
				if (damage > gServerInfo.m_DamageMax[lpObj->Class])
				{
					damage = gServerInfo.m_DamageMax[lpObj->Class];
				}
			}
			...
			else if(DE_MAP_RANGE(lpObj->Map) != 0 && DE_MAP_RANGE(lpTarget->Map) != 0)
			{
				damage = (damage*gServerInfo.m_DiabloEventDamageRate)/100;
			}
		...
		else
		{
			...
			if(gCharacterCalc.is_enabled)
			{
				damage = (damage*gCharacterCalc.calculate_damage_rate_pvm(lpObj->Index))/100;
			}
			else
			{
				damage = (damage*gServerInfo.m_DamageRatePvM[lpObj->Class])/100;
	...
	if(damage > 0)
	{
		if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER && lpObj->Index != lpTarget->Index)
		{
			...
			if(DE_MAP_RANGE(lpObj->Map) != 0 && DE_MAP_RANGE(lpTarget->Map) != 0)
			{
				CheckSelfDefense = 0;
			}
...
void CAttack::HelperSprite(LPOBJ lpObj,LPOBJ lpTarget,int* damage) // OK
{
	if(lpObj != 0 && lpObj->Type == OBJECT_USER)
	{
		...
		if(lpItem->IsItem() != 0 && lpItem->m_Durability > 0)
		{
			...
			else if(gCustomPets.CheckCustomPetByItem(lpItem->m_Index) != 0)
			{
				(*damage) = ((*damage)*(100+gCustomPets.GetCustomPetDamageRate(lpItem->m_Index)))/100;
			}
		}
	}
...
void CAttack::DamageSprite(LPOBJ lpObj,int damage) // OK
{
	...
	short TempItemIndex = lpItem->m_Index;
	short TempItemLevel = lpItem->m_Level;
	...
	if(lpItem->m_Durability < 1)
	{
		...
		if (gServerInfo.m_PetsRenew)
		{
			int slot = gItemManager.GetInventoryItemSlot(lpObj,TempItemIndex,TempItemLevel);
			if (slot != -1)
			{
				...
				gItemManager.ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[slot]);
		...
		if(lpObj->Map == MAP_ICARUS && lpObj->Inventory[SLOT_PET].IsItem() == 0 && lpObj->Inventory[SLOT_WINGS].IsItem() == 0 && (TempItemIndex == GET_ITEM(13,3) || TempItemIndex == GET_ITEM(13,37)))
		{
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gObjTargetGuildWarCheck(lpObj,lpTarget) == 0)
	{
		if(lpTarget->Guild != 0 && lpTarget->Guild->WarState != 0)
		{
			...
			if(DE_MAP_RANGE(lpTarget->Map) == 0 && CA_MAP_RANGE(lpTarget->Map) == 0 && CC_MAP_RANGE(lpTarget->Map) == 0 && IT_MAP_RANGE(lpTarget->Map) == 0 && gDuel.CheckDuel(lpObj,lpTarget) == 0 && gGensSystem.CheckGens(lpObj,lpTarget) == 0)
			{
				return 0;
			}
		}
...
int CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	else
	{
		...
		if(lpObj->Class == CLASS_DL)
		{
			if(Right->IsItem() != 0 && Right->m_IsValidItem != 0 && ((Right->m_Index >= GET_ITEM(2,0) && Right->m_Index < GET_ITEM(3,0))))
			{
