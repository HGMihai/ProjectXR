...
void CGuild::CGGuildRequestRecv(PMSG_GUILD_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpTarget->GuildStatus != GUILD_STATUS_MASTER && lpTarget->GuildStatus != GUILD_STATUS_OFFICE_MASTER)
	{
		this->GCGuildResultSend(aIndex,4);
		return;
	}
...
void CGGuildMasterAnswerRecv(PMSG_GUILDMASTERANSWER * lpMsg, int aIndex) // 0x54
{
	if ( lpMsg->Result == 1 )
	{
		...
		{
			if ( gObj[aIndex].Interface.use && gObj[aIndex].Interface.type == INTERFACE_GUILD_CREATE )
			{
...
void GCGuildWarRequestResult(LPSTR GuildName, int aIndex, int type)
{
	...
	if ( lpNode )
	{
		...
		if ( warmaster >= 1 )
		{
			if ( DE_MAP_RANGE(gObj[aIndex].Map) || DE_MAP_RANGE(gObj[warmaster].Map) )
			{
				return;
			}
...
void GCGuildWarRequestSendRecv(PMSG_GUILDWARSEND_RESULT * lpMsg, int aIndex) // 61
{
	...
	if ( lpMyNode->TargetGuildNode && lpMsg->Result )
	{
		...
		if ( DE_MAP_RANGE(gObj[iTarGetIndex].Map) || CA_MAP_RANGE(gObj[iTarGetIndex].Map) || BC_MAP_RANGE(gObj[iTarGetIndex].Map) || CC_MAP_RANGE(gObj[iTarGetIndex].Map) || DS_MAP_RANGE(gObj[iTarGetIndex].Map) || IT_MAP_RANGE(gObj[iTarGetIndex].Map))
		{
			lpMsg->Result = 0;
		}
	}
