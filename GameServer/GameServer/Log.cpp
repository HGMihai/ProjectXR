...
void CLog::Output(eLogType type,char* text,...) // OK
{
	...
	if(systemtime.wDay != lpInfo->Day || systemtime.wMonth != lpInfo->Month || systemtime.wYear != lpInfo->Year)
	{
		CloseHandle(lpInfo->File);

		lpInfo->Day = systemtime.wDay;

		lpInfo->Month = systemtime.wMonth;

		lpInfo->Year = systemtime.wYear;
	...
	wsprintf(buff,"%02d:%02d:%02d %s\r\n",systemtime.wHour,systemtime.wMinute,systemtime.wSecond,temp);
