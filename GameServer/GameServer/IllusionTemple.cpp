...
CIllusionTemple::CIllusionTemple() // OK
{
	...
	for(int i=0;i<MAX_IT_LEVEL-1;i++)
	{
		this->m_illusion_temple_min_level[i] = 0;
		this->m_illusion_temple_max_level[i] = 0;
	}
		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if (section == 3)
				{
					...
					if (IT_LEVEL_RANGE(BridgeNum) != false)
					{
						this->m_illusion_temple_min_level[BridgeNum] = lpMemScript->GetAsNumber();
						this->m_illusion_temple_max_level[BridgeNum] = lpMemScript->GetAsNumber();
					}
				}
				else
				{
...
{
	if(gServerInfo.m_IllusionTempleNewLevelSettings)
		return this->GetUserAbleLevelNew(lpObj);
	
	int level = -1;

	if(lpObj->ChangeUp < 2)
	{
		if(lpObj->Level >= this->m_illusion_temple_min_level[0] && lpObj->Level <= this->m_illusion_temple_max_level[0])
		{
			level = 0;
		}
		else if(lpObj->Level >= this->m_illusion_temple_min_level[1] && lpObj->Level <= this->m_illusion_temple_max_level[1])
		{
			level = 1;
		}
		else if(lpObj->Level >= this->m_illusion_temple_min_level[2] && lpObj->Level <= this->m_illusion_temple_max_level[2])
		{
			level = 2;
		}
		else if(lpObj->Level >= this->m_illusion_temple_min_level[3] && lpObj->Level <= this->m_illusion_temple_max_level[3])
		{
			level = 3;
		}
		else if(lpObj->Level >= this->m_illusion_temple_min_level[4] && lpObj->Level <= this->m_illusion_temple_max_level[4])
		{
			level = 4;
		}
	}
...
int CIllusionTemple::GetUserAbleLevelNew(LPOBJ lpObj) // OK
{
	if(lpObj->Level >= this->m_illusion_temple_min_level[0] && lpObj->Level <= this->m_illusion_temple_max_level[0])
		return 0;
	
	if(lpObj->Level >= this->m_illusion_temple_min_level[1] && lpObj->Level <= this->m_illusion_temple_max_level[1])
		return 1;
	
	if(lpObj->Level >= this->m_illusion_temple_min_level[2] && lpObj->Level <= this->m_illusion_temple_max_level[2])
		return 2;
	
	if(lpObj->Level >= this->m_illusion_temple_min_level[3] && lpObj->Level <= this->m_illusion_temple_max_level[3])
		return 3;
	
	if(lpObj->Level >= this->m_illusion_temple_min_level[4] && lpObj->Level <= this->m_illusion_temple_max_level[4])
		return 4;
...
void CIllusionTemple::start_now()
{
	...
	LogAdd(LOG_GREEN,"[Set IT Start] At %2d:%2d:00",hour,minute);
