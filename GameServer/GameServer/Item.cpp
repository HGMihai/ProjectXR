...
BOOL CItem::IsExtLifeAdd()
{
	if (( this->m_Index >= GET_ITEM(12,36) && this->m_Index <= GET_ITEM(12,40) ) || (this->m_Index == GET_ITEM(12,43)) || (this->m_Index == GET_ITEM(12,50)) // Third Wings Prevent Wings Level 2 Attributes
			|| gCustomWing.CheckCustomWingByItem(this->m_Index))
	{
		return 0;
	}
...
void CItem::Convert(int index,BYTE Option1,BYTE Option2,BYTE Option3,BYTE NewOption,BYTE SetOption,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus) // OK
{
	...
	this->m_SkillOption = Option1;

	this->m_LuckOption = Option2;

	this->m_Option = Option3;
	...
	{
		this->m_RequireLeadership = (((ItemInfo.RequireLeadership*((this->m_Level*3)+ItemLevel))*3)/100)+20;
		if (this->m_Index == GET_ITEM(12, 35))
		{
			LogAdd(LOG_RED, "InfoCmd = %d mLvl = %d ItemLvl = %d ReqCmd = %d", ItemInfo.RequireLeadership, this->m_Level, ItemLevel, this->m_RequireLeadership);
		}
	}*/
	...
	{
		this->m_Option = ((this->m_Option==0)?1:this->m_Option);
		//this->m_SpecialIndex[SPECIAL_OPTION3] = this->m_SpecialIndex[SPECIAL_OPTION3];
		this->m_SpecialValue[SPECIAL_OPTION3] = this->m_SpecialValue[SPECIAL_OPTION3]*this->m_Option;

		switch(this->m_Index)
		{
			case GET_ITEM(12,1): // Wings of Angel
				this->m_RequireStrength += (this->m_Option*4);
				break;
			...
			case GET_ITEM(13,30): // Cloak of Lord
				...
				break;
			default:
				this->m_RequireStrength += ((this->m_Index<GET_ITEM(12,0))?(this->m_Option*4):0);
				break;
		}
	}
	...
	if (gCostumeSystem.checkIsCostumeItem(this->m_Index) != 0)
	{
		if((this->m_NewOption & 1) != 0)
		{
			this->m_SpecialIndex[SPECIAL_EXCELLENT1] = ITEM_OPTION_MUL_DEFENSE;
			this->m_SpecialValue[SPECIAL_EXCELLENT1] = gCostumeSystem.GetCostumeSystemNewOptionValue(this->m_Index,0);
		}
		if((this->m_NewOption & 2) != 0)
		{
			this->m_SpecialIndex[SPECIAL_EXCELLENT2] = ITEM_OPTION_MUL_DAMAGE;
			this->m_SpecialValue[SPECIAL_EXCELLENT2] = gCostumeSystem.GetCostumeSystemNewOptionValue(this->m_Index,1);
		}
		if((this->m_NewOption & 4) != 0)
		{
			this->m_SpecialIndex[SPECIAL_EXCELLENT3] = ITEM_OPTION_ADD_DAMAGE_REDUCTION;
			this->m_SpecialValue[SPECIAL_EXCELLENT3] = gCostumeSystem.GetCostumeSystemNewOptionValue(this->m_Index,2);
		}
		if((this->m_NewOption & 8) != 0)
		{
			this->m_SpecialIndex[SPECIAL_EXCELLENT4] = ITEM_OPTION_ADD_IGNORE_DEFENSE_RATE;
			this->m_SpecialValue[SPECIAL_EXCELLENT4] = gCostumeSystem.GetCostumeSystemNewOptionValue(this->m_Index,3);
		}
		if((this->m_NewOption & 16) != 0)
		{
			this->m_SpecialIndex[SPECIAL_EXCELLENT5] = ITEM_OPTION_ADD_DOUBLE_DAMAGE_RATE;
			this->m_SpecialValue[SPECIAL_EXCELLENT5] = gCostumeSystem.GetCostumeSystemNewOptionValue(this->m_Index,4);
		}
		if((this->m_NewOption & 32) != 0)
		{
			this->m_SpecialIndex[SPECIAL_EXCELLENT6] = ITEM_OPTION_ADD_EXPERIENCE_RATE;
			this->m_SpecialValue[SPECIAL_EXCELLENT6] = gCostumeSystem.GetCostumeSystemNewOptionValue(this->m_Index,5);
		}
	}
...
void CItem::Value() // OK
{
	...
	if(gItemValue.GetItemValue(this, &type, &BuyPrice, &SellPrice) != 0)
	{
		...
		{
			this->m_BuyCredits = BuyPrice;

			this->m_SellCredits = SellPrice;
	...
	else
	{
		...
		if(((this->m_Index/MAX_ITEM_TYPE) == 12 && ((this->m_Index > GET_ITEM(12,6) && this->m_Index < GET_ITEM(12,36)) || this->m_Index > GET_ITEM(12,43)) && this->m_Index != GET_ITEM(12,50) && (this->m_Index < GET_ITEM(12,262) || this->m_Index > GET_ITEM(12,265)) && gCustomWing.CheckCustomWingByItem(this->m_Index) == 0) || (this->m_Index/MAX_ITEM_TYPE) == 13 || (this->m_Index/MAX_ITEM_TYPE) == 15)
		{
			...
			price = ((this->m_SpecialIndex[SPECIAL_OPTION3]==ITEM_OPTION_ADD_HP_RECOVERY_RATE)?(price+(price*this->m_Option)):price);
		}
		else
		{
			switch(this->m_Level)
				...
				if(this->m_SpecialIndex[n] != 0)
				{
					switch(n)
					{
						...
						case SPECIAL_OPTION3:
							price += ((this->m_Option==1)?((price*60)/100):0);
							price += ((this->m_Option==2)?((price*140)/100):0);
							price += ((this->m_Option==3)?((price*280)/100):0);
							price += ((this->m_Option==4)?((price*560)/100):0);
							break;
...
void CItem::SetPetItemInfo(int PetLevel,int PetExp) // OK
{
	if(this->m_Index == GET_ITEM(13,4)) // Dark Horse
	{
		...
		this->Convert(this->m_Index,this->m_SkillOption,this->m_LuckOption,this->m_Option,this->m_NewOption,this->m_SetOption,this->m_JewelOfHarmonyOption,this->m_ItemOptionEx,this->m_SocketOption,this->m_SocketOptionBonus);
...
bool CItem::ArmorDurabilityDown(int aIndex,int damage) // OK
{
	...
	{
		defense *= 5;
		defense += this->m_Option*5;
	}
	else if(ItemInfo.RequireClass[CLASS_DW] != 0)
	{
		defense *= 3;
		defense += this->m_Option*4;
	}
