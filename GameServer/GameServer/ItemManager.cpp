	...
	try
	{
		while(true)
		{
			...
			while(true)
			{
				...
				memcpy(info.Name,lpMemScript->GetAsString(),sizeof(info.Name));
...
bool CItemManager::IsItemExist(int index)
{
...
ITEM_INFO* CItemManager::GetInfo(int index)
{
	std::map<int,ITEM_INFO>::iterator it = this->m_ItemInfo.find(index);
...
int CItemManager::GetItemSkill(int index,int isExcl) // OK
{
	ITEM_INFO ItemInfo;
	...
	else
	{
		...
		{
			BYTE Skill = 0;
			gItemOptionRate.GetItemOption1(((isExcl==0)?0:1),&Skill);
			return Skill;
		}
...
int CItemManager::GetItemHaveSerial(int index)
{
	ITEM_INFO ItemInfo;
...
int CItemManager::GetItemDurability(int index,int level,int NewOption,int SetOption) // OK
{
	...
	/*if (gCostumeSystem.checkIsCostumeItem(index))
	{
		return 255;
	}*/
...
int CItemManager::GetItemRepairMoney(CItem* lpItem,int type) // OK
{
	...
	{
		if(lpItem->m_Index == GET_ITEM(13,4) || lpItem->m_Index == GET_ITEM(13,5)) // Dark Horse,Dark Raven
		{
			value *= (float)2.0f;
		}
		else
...
void CItemManager::GuildWarehouseItemSet(int aIndex,int slot,BYTE type) // OK
{
	if(WAREHOUSE_RANGE(slot) == 0)
	{
		return;
	...
	if(this->GetInfo(gObj[aIndex].GuildWarehouse[slot].m_Index,&ItemInfo) == 0)
	{
		return;
	}
	...
	for(int sy=0;sy < ItemInfo.Height;sy++)
	{
		for(int sx=0;sx < ItemInfo.Width;sx++)
		{
			gObj[aIndex].GuildWarehouseMap[(((sy+y)*8)+(sx+x))] = type;
		}
	}
}

BYTE CItemManager::GuildWarehouseRectCheck(int aIndex,int x,int y,int width,int height) // OK
{
	int slot = ((y*8)+x);
	...
	for(int sy=0;sy < height;sy++)
	{
		for(int sx=0;sx < width;sx++)
		{
			if(gObj[aIndex].GuildWarehouseMap[(((sy+y)*8)+(sx+x))] != 0xFF)
			{
				return 0xFF;
			}
		}
...
BYTE CItemManager::GuildWarehouseAddItem(int aIndex,CItem item,int slot) // OK
{
	if(WAREHOUSE_RANGE(slot) == 0)
	{
		return 0xFF;
	...
	if(lpObj->GuildWarehouse[slot].IsItem() != 0 || item.IsItem() == 0)
	{
		return 0xFF;
	}
	...
	if(this->GuildWarehouseRectCheck(aIndex,x,y,ItemInfo.Width,ItemInfo.Height) == 0xFF)
	{
		return 0xFF;
	}

	lpObj->GuildWarehouse[slot] = item;
	this->GuildWarehouseItemSet(aIndex,slot,1);
	return slot;
}

void CItemManager::GuildWarehouseDelItem(int aIndex,int slot) // OK
{
	this->GuildWarehouseItemSet(aIndex,slot,0xFF);
	gObj[aIndex].GuildWarehouse[slot].Clear();
}
...
BYTE CItemManager::ChaosBoxFindPlace(int aIndex,CItem item) // OK
{
	ITEM_INFO ItemInfo;
...
void CItemManager::ItemByteConvert(BYTE* lpMsg,CItem item) // OK
{
	...
	//else
	//{
	//	//LogAdd(LOG_TEST, "socket bonus %d",item.m_SocketOptionBonus);
...
void CItemManager::DBItemByteConvert(BYTE* lpMsg,CItem* lpItem) // OK
{
	...
	lpMsg[7] |= ((lpItem->m_Option>3)?64:0);
	...
	//if(lpItem->IsSocketItem(1) == 0 && lpItem->IsPentagramItem() == 0 && lpItem->IsPentagramJewel() == 0 && lpItem->IsPentagramMithril() == 0 && lpItem->IsMuunItem() == 0)
	//{
	...
	//LogAdd(LOG_GREEN,"iteminfo index %d socketbonus %x",lpItem->m_Index,lpMsg[10]);

	memcpy(&lpMsg[11],lpItem->m_SocketOption,MAX_SOCKET_OPTION);

#if (ITEM_BYTES_SIZE == 32)
	memset(&lpMsg[16],0xFF,16);
#endif
}
...
bool CItemManager::ConvertItemByte(CItem* lpItem,BYTE* lpMsg) // OK
{
	...
	lpItem->m_Option = (lpMsg[1] & 3)+((lpMsg[7] & 64)/16);
...
BYTE CItemManager::MoveItemToInventoryFromWarehouse(LPOBJ lpObj,BYTE SourceSlot,BYTE TargetSlot,BYTE TargetFlag) // OK
{
	...
	if (lpObj->LoadGuildWarehouse!=0)
	{
		if(gObjInventorySearchSerialNumber(lpObj,lpObj->GuildWarehouse[SourceSlot].m_Serial) == 0)
		{
			return 0xFF;
		}

		if(gObjGuildWarehouseSearchSerialNumber(lpObj,lpObj->GuildWarehouse[SourceSlot].m_Serial) == 0)
		{
			return 0xFF;
		}

		if(this->CheckItemMoveToInventory(lpObj,&lpObj->GuildWarehouse[SourceSlot],TargetSlot) == 0)
		{
			return 0xFF;
		}

		if(this->InventoryAddItem(lpObj->Index,lpObj->GuildWarehouse[SourceSlot],TargetSlot) == 0xFF)
		{
			return 0xFF;
		}

		this->GuildWarehouseDelItem(lpObj->Index,SourceSlot);
	}
	else
	{
...
{
	//LogAdd(LOG_TEST,"Move from inv to ware");
	if(INVENTORY_FULL_RANGE(SourceSlot) == 0)
	{
		//LogAdd(LOG_RED, "0");
		return 0xFF;
	...
	if (lpObj->LoadGuildWarehouse!=0)
	{
		if(gObjGuildWarehouseSearchSerialNumber(lpObj,lpObj->Inventory[SourceSlot].m_Serial) == 0)
		{
			return 0xFF;
		}
	}
	...
	if (lpObj->LoadGuildWarehouse!=0)
	{
		if(this->GuildWarehouseAddItem(lpObj->Index,lpObj->Inventory[SourceSlot],TargetSlot) == 0xFF)
		{
			return 0xFF;
		}
	}
...
BYTE CItemManager::MoveItemToWarehouseFromWarehouse(LPOBJ lpObj,BYTE SourceSlot,BYTE TargetSlot,BYTE TargetFlag) // OK
{
	...
	if (lpObj->LoadGuildWarehouse!=0)
	{
		...
		memcpy(map,lpObj->GuildWarehouseMap,WAREHOUSE_SIZE);

		this->GuildWarehouseItemSet(lpObj->Index,SourceSlot,0xFF);

		if(this->GuildWarehouseAddItem(lpObj->Index,lpObj->GuildWarehouse[SourceSlot],TargetSlot) == 0xFF)
		{
			memcpy(lpObj->GuildWarehouseMap,map,WAREHOUSE_SIZE);
			return 0xFF;
		}

		this->GuildWarehouseDelItem(lpObj->Index,SourceSlot);

		this->GuildWarehouseItemSet(lpObj->Index,TargetSlot,1);
	}
	else
	{
		if(gObjWarehouseSearchSerialNumber(lpObj,lpObj->Warehouse[SourceSlot].m_Serial) == 0)
...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	if (g_ExWinQuestSystem.PickUpItem(aIndex, lpItem->m_Index) == 1)
	{
		lpItem->Clear();
		pMsg.result = -1;
	...
	if (g_ExWinQuestSystem.PickUpItem(aIndex, lpItem->m_Index) == 2)
	{
		pMsg.result = -1;
		pMsg.header.size -= sizeof(pMsg.ItemInfo);
		DataSend(aIndex, (LPBYTE)&pMsg, pMsg.header.size);
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg,int aIndex) // OK
{
	...
	else if (gRandomMonsterCall.CheckMonsterCallItem(lpItem->m_Index) != 0)
	{
		if(gRandomMonsterCall.CreateMonster(lpObj,lpItem->m_Index) == 0)
		{
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	...
	{
		if(gMap[lpObj->Map].ItemDrop(lpItem->m_Index,lpItem->m_Level,lpItem->m_Durability,lpMsg->x,lpMsg->y,lpItem->m_SkillOption,lpItem->m_LuckOption,lpItem->m_Option,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_Serial,aIndex,lpItem->m_PetItemLevel,lpItem->m_PetItemExp,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption,lpItem->m_SocketOptionBonus,lpItem->m_PeriodicItemTime) == 0)
		{
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	else if(lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 2) // Inventory -> Warehouse
	{
		if((pMsg.result=this->MoveItemToWarehouseFromInventory(lpObj,lpMsg->SourceSlot,lpMsg->TargetSlot,lpMsg->TargetFlag)) != 0xFF)
		{
			if (lpObj->LoadGuildWarehouse!=0)
			{
				this->ItemByteConvert(pMsg.ItemInfo,lpObj->GuildWarehouse[lpMsg->TargetSlot]);
			}
			else
			{
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg,int aIndex) // OK
{
	...
	if ( gMonsterCall.SetMonster(lpObj, lpItem) )
	{
	...
	/*else if (lpItem->m_Index == GET_ITEM(13,121))
	{
	}*/
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg,int aIndex) // OK
{
	...
	else
	{
		...
		else if (item.m_PriceType == CREDITS)
		{
			if((DWORD)lpObj->BankEx.Credits < item.m_BuyCredits)
			{
		...
		{
			GDCreateItemSend(aIndex,0xEB,0,0,item.m_Index,(BYTE)item.m_Level,(BYTE)item.m_Durability,item.m_SkillOption,item.m_LuckOption,item.m_Option,-1,item.m_NewOption,item.m_SetOption,item.m_JewelOfHarmonyOption,item.m_ItemOptionEx,item.m_SocketOption,item.m_SocketOptionBonus,0);
			if (item.m_PriceType == ZEN)
			{
			...
			{
				gAchievements.BuyDonateShopItem(lpObj->Index,item.m_BuyCredits);
				lpObj->BankEx.Credits -= item.m_BuyCredits;
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyCredits,gMessage.GetMessage(527), lpObj->BankEx.Credits);
				gBankEx.GCUpdateBankEx(lpObj->Index);
				gBankEx.GDSavePoint(lpObj->Index);
				gLog.Output(LOG_ITEMVALUE_BUY,"[%s][%s] Buy item %d [%d] for %d Credits. Balanñe %d",lpObj->Account,lpObj->Name,item.m_Index,item.m_Serial,item.m_BuyCredits,lpObj->BankEx.Credits);
			}
...
void CItemManager::CGItemBuyRecvCallback(LPOBJ lpObj,BYTE slot,DWORD null,DWORD WCoinC,DWORD WCoinP,DWORD GoblinPoint)
{
	...
	// Ïðîâåðêà åñòü ëè íåîáõîäèìàÿ ñóììà
	if (item.m_PriceType == ZEN)
	{
		if(((DWORD)lpObj->Money) < (item.m_BuyMoney+tax))
		{
	...
	if(item.m_Index == GET_ITEM(13,4) || item.m_Index == GET_ITEM(13,5)) //Dark Horse,Dark Reaven
	{
		GDCreateItemSend(lpObj->Index,0xEB,0,0,item.m_Index,(BYTE)item.m_Level,(BYTE)item.m_Durability,item.m_SkillOption,item.m_LuckOption,item.m_Option,-1,item.m_NewOption,item.m_SetOption,item.m_JewelOfHarmonyOption,item.m_ItemOptionEx,item.m_SocketOption,item.m_SocketOptionBonus,0);
		if (item.m_PriceType == ZEN)
		{
			lpObj->Money -= item.m_BuyMoney+tax;
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyWCoinC, gMessage.GetMessage(524), WCoinC-item.m_BuyWCoinC);
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyWCoinP,gMessage.GetMessage(525), WCoinP-item.m_BuyWCoinP);
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You buy item for %d %s. Balance: %d", item.m_BuyWCoinG,gMessage.GetMessage(526), GoblinPoint-item.m_BuyWCoinG);
	...
	LogAdd(LOG_TEST, "[%s - %s] Buy [%d] slot %d", lpObj->Account,lpObj->Name,item.m_Index,slot);
}
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpItem->m_IsExpiredItem == 0)
	{
		...
		else if (lpItem->m_PriceType == CREDITS)
		{
			lpObj->BankEx.Credits += lpItem->m_SellCredits;
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s! Balance: %d", lpItem->m_SellCredits,gMessage.GetMessage(527), lpObj->BankEx.Credits);
			gBankEx.GCUpdateBankEx(lpObj->Index);
			gBankEx.GDSavePoint(lpObj->Index);
			gLog.Output(LOG_ITEMVALUE_SELL,"[%s][%s] Sell item %d [%d] for %d Credits. Balance %d",lpObj->Account,lpObj->Name,lpItem->m_Index,lpItem->m_Serial,lpItem->m_SellCredits,lpObj->BankEx.Credits);
		...
		else if (lpItem->m_PriceType == WCOINC)
		{
			...
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s!", lpItem->m_SellWCoinC,gMessage.GetMessage(524));
		...
		else if (lpItem->m_PriceType == WCOINP)
		{
			...
			gCashShop.CGCashShopPointRecv(lpObj->Index);
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s!", lpItem->m_SellWCoinP,gMessage.GetMessage(525));
		...
		else if (lpItem->m_PriceType == WCOING)
		{
			...
			gCashShop.CGCashShopPointRecv(lpObj->Index);
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s!", lpItem->m_SellWCoinG,gMessage.GetMessage(526));
	...
	else
	{
		...
		if(sell_type != 0)
		{
			if(sell_type == CREDITS)
			{
				lpObj->BankEx.Credits += sell_price;
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s! Balance: %d", sell_price, gMessage.GetMessage(527), lpObj->BankEx.Credits);
				gBankEx.GCUpdateBankEx(lpObj->Index);
			}
			else if (sell_type == WCOINC)
			{
				gCashShop.GDCashShopAddPointSaveSend(lpObj->Index, 0, sell_price, 0, 0);
				gCashShop.CGCashShopPointRecv(lpObj->Index);
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s!", sell_price, gMessage.GetMessage(524));
			}
			else if (sell_type == WCOINP)
			{
				gCashShop.GDCashShopAddPointSaveSend(lpObj->Index, 0, 0, sell_price, 0);
				gCashShop.CGCashShopPointRecv(lpObj->Index);
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s!", sell_price, gMessage.GetMessage(525));
			}
			else if (sell_type == WCOING)
			{
				gCashShop.GDCashShopAddPointSaveSend(lpObj->Index, 0, 0, 0, sell_price);
				gCashShop.CGCashShopPointRecv(lpObj->Index);
				gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s!", sell_price, gMessage.GetMessage(526));
			}
		}
		else
		{
...
void CItemManager::GCItemMoveSend(int aIndex,BYTE result,BYTE slot,BYTE* ItemInfo) // OK
{
	...
	if(slot >= 204)
	{
		g_PersonalShopEx.UpdatePSInventoryUser(aIndex);
	}
#endif
...
//		if (lpObj->Inventory[aRecv->Source].IsLuckyItem())
//		{
//			return;
//		}
//
...
void CItemManager::CGMoveItemProc(PMSG_MOVEITEM* aRecv, short aIndex)
{
	...
	else if (aRecv->tFlag == 0 && aRecv->sFlag == 0) //equip/unequip item
	{
		...
		if (aRecv->Source > 11)
		{
			...
			pMsg.TargetSlot = ItemInfo.Slot;
			if (pMsg.TargetSlot == 0 && lpObj->Inventory[0].IsItem() && !lpObj->Inventory[1].IsItem() && lpObj->
				Inventory[aRecv->Source].m_Index < GET_ITEM(1, 0)) //sword item
				pMsg.TargetSlot = 1;
			else if (pMsg.TargetSlot == 10 && lpObj->Inventory[10].IsItem() && !lpObj->Inventory[11].IsItem())
				//slot 11 empty
				pMsg.TargetSlot = 11;
		}
		else
		{
			if (lpObj->Map == MAP_ICARUS && (lpObj->Inventory[aRecv->Source].m_Index == GET_ITEM(13,3) || lpObj->Inventory[aRecv->Source].m_Index == GET_ITEM(13,4) || lpObj->Inventory[aRecv->Source].m_Index == GET_ITEM(13,37)) && lpObj->Inventory[7].m_Index == -1)
			{
				return;
			}
			if (lpObj->Map == MAP_ICARUS && aRecv->Source == 7 && (lpObj->Inventory[8].m_Index != GET_ITEM(13,3) && lpObj->Inventory[8].m_Index != GET_ITEM(13,37) && lpObj->Inventory[8].m_Index != GET_ITEM(13,4)))
			{
				return;
			}
			//unequipment case
