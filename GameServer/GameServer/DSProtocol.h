...
struct PMSG_ENTRY_COUNT_REQ_SEND
{
...
struct PMSG_ENTRY_COUNT_REQ_RECV
{
	PSBMSG_HEAD header; // C1:9A
	BYTE type;
	int EntryCount;
	char LastEntryTime[20];
	DWORD lpObjAddr;
};

struct PMSG_ENTRY_COUNT_UPDATE_SEND
{
	PSBMSG_HEAD header; // C1:9A
	BYTE type;
	char account[11];
...
struct PMSG_SET_CHARACTER_BLOCK_SEND
{
	PSBMSG_HEAD	header;
	char account[11];
	char name[11];
