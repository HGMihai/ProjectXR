...
void CObjectManager::ObjectSetStateProc() // OK
{
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Type == OBJECT_USER)
		{
			...
			{
				gEffectManager.AddEffect(lpObj,0,EFFECT_PREMIUM_BUFF1+(lpObj->AccountLevel-1),0,0,0,0,0);
			}

			if (lpObj->Reset < gServerInfo.m_CustomNoobBuffMaxReset[lpObj->AccountLevel] && lpObj->MasterReset == 0)
			{
				gEffectManager.AddEffect(lpObj,0,EFFECT_NOOB_BUFF,0,0,0,0,0);
			}

			if ( lpObj->Guild != NULL )
			{
				gGuildClass.AddGuildBuff(lpObj->Guild);
			}
		}
	}
...
void CObjectManager::ObjectStateProc(LPOBJ lpObj,int MessageCode,int aIndex,int SubCode) // OK
{
	...
	switch(MessageCode)
	{
		...
		case 10:
			...
			{
				gAttack.Attack(lpObj,&gObj[aIndex],0,0,0,SubCode,0,0,1);
			}
			break;
		case 12:
			if (lpObj->Live != 0)
...
void CObjectManager::ObjectStateAttackProc(LPOBJ lpObj,int MessageCode,int aIndex,int SubCode1,int SubCode2) // OK
{
	...
	switch(MessageCode)
	{
		...
		case 55:
			if(lpObj->Live != 0 && (SubCode1 == 0 || gSkillManager.GetSkill(lpObj,SubCode1) != 0))
			{
				gSkillManager.SkillGhostPhantomAreaAttack(lpObj->Index,aIndex,gSkillManager.GetSkill(lpObj,SubCode1),(BYTE)SET_NUMBERHW(SubCode2),(BYTE)SET_NUMBERLW(SubCode2),0);
			}
			break;
	}
}
	...
	for(int n=0;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnected(n) != 0)
		{
			...
			else
			{
				...
				gPickup.PickMsgProc(&gObj[n]);

				if(gObj[n].is_use_auto_health)
					gObjFindAndUseHealthPotion(&gObj[n]);
			}
		}
	}
...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	if ( (lpObj->Interface.use != 0 )
		&&  (lpObj->Interface.type == 10 ) 
		&& lpObj->TargetShopNumber == 229 && gServerInfo.m_QuestNPCTeleport != 0)
	{
	...
	//GUILD_INFO_STRUCT *tempGuild = lpObj->Guild;
	...
	{
		LogAdd(LOG_BLACK,"[ObjectManager][%d] DelCharacterInfo (%s) Guild (%s)",lpObj->Index,lpObj->Name,tempGuild->Name);
	}*/
...
void CObjectManager::CharacterGameCloseSet(int aIndex,int type) // OK
{
	...
	lpObj->CloseType = ((type>2)?0:type);//type;
...
void CObjectManager::CharacterCalcExperienceAlone(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag,bool type,int AttackDamage) // OK
{
	...
	if (gMasterSkillTree.CheckMasterLevel(lpObj) == 0 && lpMonster->Level > gServerInfo.m_MaxExperienceMonsterLevel[lpObj->AccountLevel])
	{
		GCMonsterDieSend(lpObj->Index,lpMonster->Index,0,((type==0)?AttackDamage:0),flag);
		return;
	}
...
void CObjectManager::CharacterCalcExperiencePartyNew(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag) // OK
{
	if(OBJECT_RANGE(lpObj->PartyNumber) == 0)
	{
		return;
	...
	//PartyCount = lpParty->Count;

	for(int n = 0; n < MAX_PARTY_USER; n++)
	{
		number = lpParty->Index[n];

		if(number >= 0)
		{
			lpPartyObj = &gObj[number];

			if(lpMonster->Map == lpPartyObj->Map)
			{
				dis[n] = gObjCalcDistance(lpMonster,&gObj[number]);

				if(dis[n] < MAX_PARTY_DISTANCE)
				{
					lpPartyObj = &gObj[number];
	...
	if ( bCheckSetParty[0] && bCheckSetParty[1] && bCheckSetParty[2]
		|| bCheckSetParty[3] && bCheckSetParty[4] && bCheckSetParty[5]
		|| bCheckSetParty[4] && bCheckSetParty[5] && bCheckSetParty[6] )
		{
			bApplaySetParty = 1;
		}
	...
	if(viewplayer > 1)
	{
		if(bApplaySetParty == 0)
		{
			if(viewplayer == 3)
			{
				viewpercent = gServerInfo.gParty3ExpPercent;
			}
			else if(viewplayer == 4)
			{
				viewpercent = gServerInfo.gParty4ExpPercent;
			}
			else if(viewplayer >= 5)
			{
				viewpercent = gServerInfo.gParty5ExpPercent;
			}
			else
			{
				viewpercent = gServerInfo.gPartyExpPercentOther;
			}
		}
		else
		{
			if(viewplayer == 2)
			{
				viewpercent = gServerInfo.gParty2ExpSetPercent;
			}
			else if(viewplayer == 3)
			{
				viewpercent = gServerInfo.gParty3ExpSetPercent;
			}
			else if(viewplayer == 4)
			{
				viewpercent = gServerInfo.gParty4ExpSetPercent;
			}
			else if(viewplayer >= 5)
			{
				viewpercent = gServerInfo.gParty5ExpSetPercent;
			}
			else
			{
				viewpercent = gServerInfo.gParty1ExpSetPercent;
			}
		}
	...
	if(viewplayer == 0){return;}

	int ExperienceRate = viewpercent;
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		...
		if(gMasterSkillTree.CheckMasterLevel(lpTarget) == 0)
		{
			...
			experience = (experience*(lpTarget->ExperienceRate+lpTarget->EffectOption.AddExperienceRate+(lpTarget->EffectOption.AddPartyBonusExperienceRate*(viewplayer-1))))/100;
		...
		else
		{
			...
			experience = (experience*(lpTarget->MasterExperienceRate+lpTarget->EffectOption.AddMasterExperienceRate+(lpTarget->EffectOption.AddPartyBonusExperienceRate*(viewplayer-1))))/100;
		...
		lpMonster->Money += (DWORD)(experience/viewplayer);
		...
		if (gMasterSkillTree.CheckMasterLevel(lpTarget) == 0 && lpMonster->Level > gServerInfo.m_MaxExperienceMonsterLevel[lpTarget->AccountLevel])
		{
			GCMonsterDieSend(lpTarget->Index,lpMonster->Index,0,damage,flag);
			return;
		}
...
void CObjectManager::CharacterCalcExperienceParty(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag) // OK
{
	...
	if (PartyClass == 0)
	{
		...
		{
			//LogAdd(LOG_TEST,"Exp single = %d PartyExpMul %d PartyCount = %d",ExperienceRate, gServerInfo.m_PartyGeneralExperience[(PartyCount-1)], PartyCount);
	...
	if(lpMonster->Level >= 65)
	{
		...
		{
			if (gServerInfo.m_PartyUseOriginalPartyExpFormula)
			{
				if(gMasterSkillTree.CheckMasterLevel(lpObj) == 0)
				{
					level += (200-(lpObj->Level/5));
...
{
	if(gProtection.is_license_end && gProtection.license_days_over >= 3)
		return false;
...
void CObjectManager::CharacterMakePreviewCharSetTest(int aIndex) // OK
{

	LPOBJ lpObj = &gObj[aIndex];

	memset(lpObj->CharSet,0,sizeof(lpObj->CharSet));
#if(GAMESERVER_UPDATE>803)//2018/02/08
...
//			if(n == 1)
//			{
//#if(GAMESERVER_UPDATE>=902)//2017/06/15
//			if(lpObj->m_btInvenPetDkSpiritPos != 0)
//			{
//				TempInventory[n] = lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index;
//			}
//#endif
//			}
		}
	...
	#if(GAMESERVER_UPDATE>803)//2017/04/16
...
//	{
//		TempInventory[8] = lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index%MAX_ITEM_TYPE;
//	}
//#endif
	...
	//}
#if(GAMESERVER_UPDATE>803)//2018/04/10
#pragma message("Need to add pet slot")
...
void CObjectManager::CharacterMakePreviewCharSet(int aIndex) // OK
{
	...
	{
		lpObj->CharSet[5] |= CS_SET_WING1(4);
	...
	{
		/*info.CharSet[5]	|= CS_SET_WING1(3);
		info.CharSet[17]	|= (lpObj->pInventory[7].m_Type - ITEMGET(12, 180) + 1) << 2;*/
		lpObj->CharSet[5] |= 12;
		lpObj->CharSet[9] |= 8+gCustomWing.GetCustomWingIndex(GET_ITEM(12,TempInventory[7]))+1;
		/*info.CharSet[5] |= 12;
	...
	else if((TempInventory[8] >= 145 && TempInventory[8] <= 257) && gCustomPets.CheckCustomPetByItem(GET_ITEM(13,TempInventory[8])) && gProtection.GetCustomState(CUSTOM_PETS) == 1)
	{
		lpObj->PetCharSet[0] |= (TempInventory[8]-144) << 1;
	}
	else if((TempInventory[8] >= 258 && TempInventory[8] <= 350) && gCustomPets.CheckCustomPetByItem(GET_ITEM(13,TempInventory[8])) && gProtection.GetCustomState(CUSTOM_PETS) == 1)
	{
		lpObj->PetCharSet[1] |= (TempInventory[8]-257) << 1;
	}
	else if((TempInventory[8] >= 351 && TempInventory[8] <= 450) && gCustomPets.CheckCustomPetByItem(GET_ITEM(13,TempInventory[8])) && gProtection.GetCustomState(CUSTOM_PETS) == 1)
	{
		lpObj->PetCharSet[2] |= (TempInventory[8]-350) << 1;
	}
	else if((TempInventory[8] >= 451 && TempInventory[8] <= 511) && gCustomPets.CheckCustomPetByItem(GET_ITEM(13,TempInventory[8])) && gProtection.GetCustomState(CUSTOM_PETS) == 1)
	{
...
bool CObjectManager::CharacterUseScroll(LPOBJ lpObj,CItem* lpItem) // OK
{
	...
	if((lpItem->m_Index >= GET_ITEM(12,44) && lpItem->m_Index <= GET_ITEM(12,46)) || lpItem->m_Index == GET_ITEM(15,18) || lpItem->m_Index == GET_ITEM(15,28))
	{
...
bool CObjectManager::CharacterUseJewelOfBles(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	/*if (lpItem->m_Index == GET_ITEM(13,80))
	{
		lpItem->m_Durability = 255;
		return 1;
	}*/

	if (gCustomPets.CheckRestoreHPByItem(lpItem->m_Index) != 0)
	{
		lpItem->m_Durability = 255;
		gAchievements.UsedJewel(lpObj->Index);
		return 1;
...
bool CObjectManager::CharacterUseJewelOfLife(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	if(lpItem->m_Option >= gServerInfo.m_MaxItemOption)
	{
		return 0;
	}
	...
	{
		if(lpItem->m_Option == 0 && ((lpItem->m_Index >= GET_ITEM(12,3) && lpItem->m_Index <= GET_ITEM(12,6)) || lpItem->m_Index == GET_ITEM(12,42) || lpItem->m_Index == GET_ITEM(12,49)))
		{
			lpItem->m_NewOption &= 0xDF;
			lpItem->m_NewOption |= (32*(GetLargeRand()%2));
		}

		if(lpItem->m_Option == 0 && ((lpItem->m_Index >= GET_ITEM(12,36) && lpItem->m_Index <= GET_ITEM(12,40)) || lpItem->m_Index == GET_ITEM(12,43) || lpItem->m_Index == GET_ITEM(12,50)))
		{
			lpItem->m_NewOption &= 0xCF;
			lpItem->m_NewOption |= (16*(GetLargeRand()%3));
		}

		if(lpItem->m_Option == 0 && gCustomWing.CheckCustomWingByItem(lpItem->m_Index) != 0)
		{
			lpItem->m_NewOption &= 0xCF;
			lpItem->m_NewOption |= (16*(GetLargeRand()%3));
		}
...
bool CObjectManager::CharacterUseCustomJewel(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	//LogAdd(LOG_TEST,"Apply jewel to ring");
	...
	if((GetLargeRand()%100) < gCustomJewel.GetCustomJewelSuccessRate(lpObj->Inventory[SourceSlot].m_Index,lpObj->AccountLevel))
	{
		...
		lpItem->m_SkillOption = (((lpItem->m_SkillOption+lpInfo->Option1)>1)?1:(lpItem->m_SkillOption+lpInfo->Option1));

		lpItem->m_LuckOption = (((lpItem->m_LuckOption+lpInfo->Option2)>1)?1:(lpItem->m_LuckOption+lpInfo->Option2));

		lpItem->m_Option = (((lpItem->m_Option+lpInfo->Option3)>gServerInfo.m_MaxItemOption)?gServerInfo.m_MaxItemOption:(lpItem->m_Option+lpInfo->Option3));
		...
		gCustomJewel.custom_jewel_replace_item(lpObj->Index,lpItem,TargetSlot,gCustomJewel.get_replace_info(lpObj->Inventory[SourceSlot].m_Index,lpItem->m_Index));
	...
	else
	{
		...
		lpItem->m_SkillOption = (((lpItem->m_SkillOption-lpInfo->Option1)<0)?0:(lpItem->m_SkillOption-lpInfo->Option1));

		lpItem->m_LuckOption = (((lpItem->m_LuckOption-lpInfo->Option2)<0)?0:(lpItem->m_LuckOption-lpInfo->Option2));

		lpItem->m_Option = (((lpItem->m_Option-lpInfo->Option3)<0)?0:(lpItem->m_Option-lpInfo->Option3));
...
{
	if (gDisableFunctions.m_disableExpandedInventory == true)
	{
		return 0;
	}
...
{
	if(gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].VitalityToShield == 1.0)
	{
	...
	{
		lpObj->MaxShield = ((lpObj->Vitality-gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].Vitality)+lpObj->AddVitality)*lpObj->VitalityToShield;
		//LogAdd(LOG_TEST,"SDValue: %d",lpObj->MaxShield);
	}
}
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
...
#if(GAMESERVER_UPDATE>803)//2017/06/02
	...
	#if(GAMESERVER_UPDATE>803)//2017/06/02
	#pragma message("Need to add pets new slot!")
	...
	while(change != 0)
	{
		...
		gCustomPets.CalcCustomPetOption(lpObj,1);
		#if(GAMESERVER_UPDATE>=701)
...
#if(GAMESERVER_UPDATE>803)//2017/06/02
#pragma message("Need add DamageDecRate, DamageIncRate to OBJECTSTRUCT")
	...
	//}
	//else if ((Wings->m_Index >= GET_ITEM(12, 3) && Wings->m_Index <= GET_ITEM(12, 6)) || Wings->m_Index == GET_ITEM(12, 42)) // 2sd wing
	//{
	//	lpObj->DamageDecRate = (float)(25 + (Wings->m_Level * 2));
	//	lpObj->DamageIncRate = (float)(32 + Wings->m_Level);
	//}
	//else if (Wings->m_Index == GET_ITEM(12, 269))
	//{
	//	lpObj->DamageDecRate = (float)(10 + (Wings->m_Level * 2));
	//	lpObj->DamageIncRate = (float)(20 + (Wings->m_Level * 2));
	//}
	//else if (Wings->m_Index == GET_ITEM(12, 270) || (Wings->m_Index >= GET_ITEM(12, 36) && Wings->m_Index <= GET_ITEM(12, 39)) || Wings->m_Index == GET_ITEM(12, 43) || Wings->m_Index == GET_ITEM(12, 50)) // 3rd wing
	//{
	...
	//	lpObj->DamageIncRate = (float)(20 + (Wings->m_Level * 2));
	//}
	//else if (gCustomWing.CheckCustomWingByItem(Wings->m_Index) != 0)
	//{

	//	lpObj->DamageIncRate = (float)(gCustomWing.GetCustomWingIncDamage(Wings->m_Index, Wings->m_Level));
	//	lpObj->DamageDecRate = (float)(gCustomWing.GetCustomWingDecDamage(Wings->m_Index, Wings->m_Level));
	...
	else if(lpObj->Class == CLASS_DK)
	{
		...
		if(gCharacterCalc.is_enabled)
		{
			lpObj->DKDamageMultiplierRate = gCharacterCalc.calculate_dk_damage_multiplier_rate(lpObj->Index);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_DL)
	{
		...
		if(gCharacterCalc.is_enabled)
		{
			lpObj->DLDamageMultiplierRate = gCharacterCalc.calculate_dl_damage_multiplier_rate(lpObj->Index);
		}
		else
		{
	...
	else if(lpObj->Class == CLASS_RF)
	{
		...
		if(gCharacterCalc.is_enabled)
		{
			lpObj->RFDamageMultiplierRate[0] = gCharacterCalc.calculate_rf_damage_multiplier_rate1(lpObj->Index);
			lpObj->RFDamageMultiplierRate[1] = gCharacterCalc.calculate_rf_damage_multiplier_rate2(lpObj->Index);
			lpObj->RFDamageMultiplierRate[2] = gCharacterCalc.calculate_rf_damage_multiplier_rate3(lpObj->Index);
		}
		else
		{
...
#if(GAMESERVER_UPDATE>803)//2018/02/03
#pragma message("Need add statSpec")
	...
	if(gCharacterCalc.is_enabled)
	{
		gCharacterCalc.calculate_attack_speed(lpObj->Index);
	}
	else
	{
		if(lpObj->Class == CLASS_DW)
	...
	if(gCharacterCalc.is_enabled)
	{
		lpObj->Defense = gCharacterCalc.calculate_defense(lpObj->Index);
	}
	else
	{
		if(lpObj->Class == CLASS_DW)
	...
	gCustomPets.CalcCustomPetOption(lpObj,0);
	#if(GAMESERVER_UPDATE>=701)
	...
	if (gProtection.GetCustomState(eCustomsType::CUSTOM_STAT))
	{
		lpObj->MaxLife += lpObj->NewStat;

		int value = lpObj->NewStat / 100;
	...
	#if(GAMESERVER_EXTRA==0 || IS_NEW_CLIENT==1)
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	lpObj->PointsCount = lpMsg->PointsCount;
	lpObj->PointsTime = lpMsg->PointsTime;

	lpObj->ChatLimitTime = lpMsg->ChatLimitTime;

	lpObj->MapQuestSystemMap = lpMsg->MapQuestSystemMap;
	lpObj->MapQuestSystemIndex = lpMsg->MapQuestSystemIndex;
	lpObj->MapQuestSystemState = lpMsg->MapQuestSystemState;
	
	//LogAdd(LOG_TEST, "%d %d %d", lpObj->MapQuestSystemMap, lpObj->MapQuestSystemIndex, lpObj->MapQuestSystemState);

//	lpObj->Rank = lpMsg->Rank;
	...
	lpObj->VitalityToShield = gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].VitalityToShield;
	...
	lpObj->BCEntryCount = lpMsg->BCEntryCount;
	...
	//gEffectManager.AddEffect(lpObj,0,EFFECT_SILVER_PARTY,0,0,0,0,0);
	...
	gInvasionManager.CheckGreatGoldDragon(lpObj->Map);
	gInvasionManager.CheckRedDragon(lpObj->Map);
	/*if (gInvasionManager.GoldDerconRegen == true)
	{
		if (lpObj->Map == gInvasionManager.GoldDerconMap)
		{
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			if (gCustomQuest.AddForEachPartyMember)
			{
				if( lpObj->PartyNumber >= 0 )
				{
					for(int n=0;n < MAX_PARTY_USER;n++)
					{
						...
						{
							if(gObj[gParty.m_PartyInfo[lpObj->PartyNumber].Index[n]].Map == lpObj->Map && gObjCalcDistance(lpObj,&gObj[gParty.m_PartyInfo[lpObj->PartyNumber].Index[n]]) < MAX_PARTY_DISTANCE)
							{
								gCustomQuest.MonsterKill(&gObj[gParty.m_PartyInfo[lpObj->PartyNumber].Index[n]], lpTarget);
							}
						}
					}
				}
			...
			if (lpTarget->Map == MAP_DIABLO_ARENA)
			{
				gDiablo.DiabloMonsterDieProc(lpTarget,&gObj[SummonIndex]);
			}
		...
		if(gObj[SummonIndex].Type == OBJECT_USER && lpTarget->Type == OBJECT_MONSTER)
		{
			...
			else
			{
				...
				//this->CharacterCalcExperiencePartyNew(&gObj[SummonIndex],lpTarget,damage,flag);
			}
		}
	}
	else
	{
		...
		#if(GAMESERVER_UPDATE>=701 || IS_NEW_CLIENT == 1)
