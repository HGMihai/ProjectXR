...
#if (GAMESERVER_UPDATE == 603 || GAMESERVER_UPDATE == 803 || IS_NEW_CLIENT)
#include "UserPanel.h"
...
{
	//LogAdd(LOG_TEST,"protocol start");
	/*BYTE lpMsg[8192];*/
	...
	//else
	//{
	//	gLog.Output(LOG_HACK,"[HackPacketCheck][%s][%s] Strange Packet error (Index: %d, Value: %d)",gObj[aIndex].Account,gObj[aIndex].Name,aIndex,lpMsg[0]);
	//	return;
	//}

	//LogAdd(LOG_BLUE, "Protocol [%x] [%x] [%x]", lpMsg[0], head,((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]));
	...
	switch(head)
	{
		...
		case 0x03: CGMainCheckRecv((PMSG_MAIN_CHECK_RECV*)lpMsg,aIndex); break;
		case 0x0E: CGLiveClientRecv((PMSG_LIVE_CLIENT_RECV*)lpMsg,aIndex); break;
		case PROTOCOL_CODE2: gAttack.CGAttackRecv((PMSG_ATTACK_RECV*)lpMsg,aIndex); break;
		case PROTOCOL_CODE3: CGPositionRecv((PMSG_POSITION_RECV*)lpMsg,aIndex); break;
		...
		case 0x32: gItemManager.CGItemBuyRecv((PMSG_ITEM_BUY_RECV*)lpMsg,aIndex); break;
		...
		case 0x4E:
			switch(lpMsg[3])
			{
				...
				case 0x09:
					...
					break;
				case 0x11:
					#if(GAMESERVER_UPDATE>=803)//2017/02/28
		...
		case 0x82: gWarehouse.CGWarehouseClose(aIndex, 2); break;
		...
		case 0xBF:
			switch(lpMsg[3])
			{
				...
				//	break;
				default:
					LogAdd(LOG_RED, "[0xBF] %02X Not Found!",lpMsg[3]);
					break;
			}
			break;
		case 0xC0:
		...
		case PROTOCOL_CODE1:
			...
			break;
		case PROTOCOL_CODE4:
			gSkillManager.CGMultiSkillAttackRecv((PMSG_MULTI_SKILL_ATTACK_RECV*)lpMsg,aIndex,false);
			break;
		case 0xE1:
		...
		case 0xEF:
			...
			break;
		case 0xF1:
			//LogAdd(LOG_GREEN, "Protocol 0xF1 [%x]", lpMsg[3]);
			switch(lpMsg[3])
			{
				case 0x01:
					//LogAdd(LOG_GREEN,"KEK0?");
					CGConnectAccountRecv((PMSG_CONNECT_ACCOUNT_RECV*)lpMsg,aIndex);
					//LogAdd(LOG_GREEN,"KEK1?");
					break;
				case 0x87:
					//LogAdd(LOG_GREEN,"KEK0?");
					CGConnectAccountRecv((PMSG_CONNECT_ACCOUNT_RECV*)lpMsg,aIndex);
			...
			break;
		case 0xF3:
			//LogAdd(LOG_GREEN, "Protocol 0xF3 [%x]", lpMsg[3]);
			switch(lpMsg[3])
			{
				case 0x00:
				...
				case 0x15:
					...
					struct PMSG_CHARCHECK_RESULT
					{
						PSBMSG_HEAD h; // C1:15
						char szName[10];
						BYTE IsFromOtherAccount; // if i put 0x0e == 1 it pop's the selection screen
					};

					PMSG_CHARCHECK_RESULT pMsg;

					pMsg.h.set(0xF3,0x15,sizeof(pMsg));
					memcpy(pMsg.szName, gObj[aIndex].Name, 10);
					pMsg.IsFromOtherAccount = 0; // if i put 0x0e == 1 it pop's the selection screen
				...
				case 0xE5:
					...
					break;
				case 0xE6:
					gDiablo.RegisterRena((DIABLO_RENARECV*)lpMsg, aIndex);
					break;
				case 0xE7:
					gDiablo.Teleport((DIABLO_RENARECV*)lpMsg, aIndex);
					break;
				case 0xE8:
					gDiablo.SendDialog(&gObj[aIndex], DIALOG_UPDATE);
					break;
				case 0xE9:
				...
				#if (GAMESERVER_UPDATE == 603)
				case 0xEA:
					gDonateShop.CGPageRecv((PMSG_DONATESHOP_PAGE_RECV*)lpMsg, aIndex);
					break;
				#endif
				case 0xEB:
			...
			break;
		case 0xF6:
			//LogAdd(LOG_RED,"[Protocol_0xF6][%x]", lpMsg[3]);
			switch(lpMsg[3])
			{
				case 0x0A:
					#if(GAMESERVER_UPDATE>=501)
		...
		case 0xFE:
			...
			break;
		case 0x79:
			//LogAdd(LOG_TEST,"Head: 0x%x; Subhead: 0x%x",head,((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]));
		...
		case 0x80:
			...
			{
			case 0x00:
				g_ItemMarket.CGReqItemList((PMSG_REQ_MARKET_ITEM*)lpMsg, aIndex);
				break;
			case 0x01:
				g_ItemMarket.CGReqItemSell((PMSG_REQ_MARKET_SELL*)lpMsg, aIndex);
				break;
			case 0x02:
				g_ItemMarket.CGReqItemBuy((PMSG_REQ_MARKET_BUY*)lpMsg, aIndex);
				break;
			}
			break;
		case 0xFF:
			...
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				...
				case 0x02:
					...
					gCommandManager.ManagementCore(&gObj[aIndex],gCommandManager.GetLabelByCode(COMMAND_PK_CLEAR));
					/*PMSG_CHAT_RECV pMsg;
					memcpy(pMsg.message,"/pkclear",sizeof("/pkclear"));
					memcpy(pMsg.name,gObj[aIndex].Name,sizeof(gObj[aIndex].Name));
					CGChatRecv(&pMsg,aIndex);*/
					break;
				case 0x03:
				...
				case 0x18:
					...
					{
						gNpcTalk.NpcChaosGoblin(nullptr,&gObj[aIndex]);
					}
					break;
				/*case 0x18:
					gPersonalShop.CGPShopSetItemPriceRecvEx((PMSG_PSHOP_SET_ITEM_PRICE_EX_RECV*)lpMsg,aIndex);
				...
				case 0x1A:
					...
					break;
				case 0x1B:
					gParty.PartyMoveUser(&gObj[aIndex],(BYTE)*(lpMsg+4));
					break;
				case 0x1C:
					gEventTimer.CG_RequestEvents((CG_EVENTTIMER_REQ*)lpMsg,aIndex);
					break;
				case 0x1D:
					CGItemDropInfoReq((PMSG_ITEMDROPINFO_REQ*)lpMsg,aIndex);
					break;
				case 0x1E:
					gDailyReward.SendInfo(aIndex);
					break;
				case 0x1F:
					gDailyReward.SendRewardInfo(aIndex);
					break;
				case 0x20:
					gGuildWarehouse.CGGetUsersWareStatus(aIndex);
					break;
				case 0x21:
					gGuildWarehouse.CGUsersWareStatusSave((PMSG_WARE_SATUS_RECV*)lpMsg,aIndex,size);
					break;
				case 0x22:
					GCItemPostRecv((PMSG_POSTITEM_RECV*)lpMsg,aIndex);
					break;
				case ACH_PROTOCOL1:
					gAchievements.CGWindowOpen((PMSG_CG_ACH_WINOPEN*)lpMsg, aIndex);
					break;
				case 0x24:
					CGRecvJBankAct((PMSG_JBANK_ACT_SEND*)lpMsg, aIndex);
					break;
				case 0x25:
					gRankings.GDSendRequest(aIndex);
					break;
				case 0x26:
					gRankings.CGRecvCharacterInfoRequest((CG_RANKINGS_INFO_REQUEST_SEND*)lpMsg, aIndex);
					break;
				case 0x30:
					gCustomItemMove.CGItemMove((PMSG_CUSTOM_ITEM_MOVE*)lpMsg,aIndex);
					break;
				case 0x31:
					g_ExWinQuestSystem.CG_AcceptQuest(aIndex, (CG_Accept_Done*)lpMsg);				
					break;
				case 0x32:
					GC_RequestSend(aIndex,0xFF,0x32);
					break;
				case 0x33:
					gRoulette.recv_roulette_start(aIndex);
					break;
				case 0x34:
					CGAutoHealthStateRecv((PMSG_AUTO_HEALTH_RECV*)lpMsg,aIndex);
					break;
				case 0x35:
					//gMapQuestSystem.receive_request((PMSG_MAP_QUEST_SYSTEM_REQUEST*)lpMsg, aIndex);
					break;
#endif
			}
			break;
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg,int aIndex) // OK
{
	...
	gLog.Output(LOG_CHAT,"[General][%s][%s] - (Message: %s) %x %x %x",lpObj->Account,lpObj->Name,lpMsg->message,lpMsg->message[0],lpMsg->message[1],lpMsg->message[2]);

//#if (TEST_COMMAND_FIX != 1)
	if (gFilter.CheckSyntax(lpMsg->message))
	{
		gAdminChat.add_to_list(name,lpMsg->message,eChatType::eCHAT_BAD_WORD);
		strcpy_s(lpMsg->message, "*censored*");
	}
//#endif

	if (gProtection.GetCustomState(CUSTOM_MARRY) != 0)
	{
		if (!memcmp(lpMsg->message, "/yes", strlen("/yes")))
		{
			gMarrySystem.YesCommand(&gObj[aIndex]);
			return;
		}
		if (memcmp(lpMsg->message, gMessage.GetMessage(53), strlen(gMessage.GetMessage(53))))
		{
			if (!memcmp(lpMsg->message, "/no", strlen("/no")))
			{
				gMarrySystem.NoCommand(&gObj[aIndex]);
				return;
			}
		}
	}
	...
	if(lpMsg->message[0] == '/')
	{
		//gCashShop.GDCashShopRecievePointSend(lpObj->Index,(DWORD)&CCommandManager::ManagementCodeCallback,(DWORD)lpMsg->message,0);
	...
	if(lpMsg->message[0] == '~')
	{
		...
		{
			gAdminChat.add_to_list(name,lpMsg->message,eChatType::eCHAT_PARTY);
	...
	else if(lpMsg->message[0] == '@')
	{
		if(lpObj->Guild != 0)
		{
			if(lpMsg->message[1] == '>')
			{
				...
				{
					gAdminChat.add_to_list(name,lpMsg->message,eChatType::eCHAT_GUILD_NOTICE);					
			...
			else if(lpMsg->message[1] == '@')
			{
				...
				{
					gAdminChat.add_to_list(name,lpMsg->message,eChatType::eCHAT_ALLIANCE);
			...
			else
			{
				...
				{
					gAdminChat.add_to_list(name,lpMsg->message,eChatType::eCHAT_GUILD);
	...
	else if(lpMsg->message[0] == '$')
	{
		...
		{
			gAdminChat.add_to_list(name,lpMsg->message,eChatType::eCHAT_GENS);
	...
	{
		gAdminChat.add_to_list(name,lpMsg->message,eChatType::eCHAT_SIMPLE);
...
void CGMoveRecv(PMSG_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	if (DE_MAP_RANGE(lpObj->Map) == 1)
	{
		gDiablo.GetMove(lpObj);
	}
...
void CGConnectAccountRecv(PMSG_CONNECT_ACCOUNT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(memcmp(gServerInfo.m_ServerVersion,lpMsg->ClientVersion,sizeof(lpMsg->ClientVersion)) != 0)
	{
		...
		LogAdd(LOG_GREEN,"err 2 [%s] - [%s]",gServerInfo.m_ServerVersion,(char*)lpMsg->ClientVersion);
		return;
	}
	...
	{
		GCConnectAccountSend(aIndex,6);
		LogAdd(LOG_GREEN,"err 3 [%s] - [%s] size: %d",gServerInfo.m_ServerSerial, lpMsg->ClientSerial,sizeof(PMSG_CONNECT_ACCOUNT_RECV));
		return;
	}
...
void CGLevelUpPointRecv(PMSG_LEVEL_UP_POINT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gObjectManager.CharacterLevelUpPointAdd(lpObj,lpMsg->type,1) != 0)
	{
		...
		#if(GAMESERVER_EXTRA==1 && IS_NEW_0CLIENT==0)
		...
		if (lpObj->Class == CLASS_DL)
			...
			pMsg.ViewLeadership = 0;

		pMsg.ViewNewStat = lpObj->NewStat;
		#endif
	}
...
#ifdef IS_NEW_CLIENT
	GDOptionDataSaveSend(aIndex, lpMsg->SkillKey, lpMsg->GameOption, lpMsg->QKey, lpMsg->WKey, lpMsg->EKey, lpMsg->ChatWindow, lpMsg->RKey, ntohl(lpMsg->QWERLevel), lpMsg->game_settings);
#else
...
void GCChatTargetSend(LPOBJ lpObj,int senduser,char* message) // OK
{
	...
	DataSend(senduser,(BYTE*)&pMsg,pMsg.header.size);
	//MsgSendV2(lpObj,(BYTE*)&pMsg,pMsg.header.size);
}

void GCChatSend(LPOBJ lpObj,char* message) // OK
{
	int size = strlen(message);
...
void GCDamageSend(int aIndex,int bIndex,BYTE flag,int damage,int type,int ShieldDamage) // OK
{
	...
	pMsg.damage = damage;
	pMsg.shield_damage = ShieldDamage;
	...
	#if(GAMESERVER_EXTRA==1 && IS_NEW_CLIENT==0)
...
void GCMonsterDamageSend(int aIndex,int damage,int ShieldDamage) // OK
{
	...
	pMsg.damage = damage;
	pMsg.ShieldDamage = ShieldDamage;
...
void GCNewCharacterInfoSend(LPOBJ lpObj) // OK
{
	...
	pMsg.Level = lpObj->Level;
	pMsg.ResetCount = lpObj->Reset;
	pMsg.MasterResetCount = lpObj->MasterReset;
	pMsg.LevelUpPoint = lpObj->LevelUpPoint;
...
void GCNewHealthBarSend(LPOBJ lpObj) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		info.monster_id = lpTarget->Class;
		...
		if (lpTarget->Type == OBJECT_USER)
		{
			...
			info.MasterReset = lpTarget->MasterReset;
			
			if (lpTarget->MaxShield+lpTarget->AddShield > 0)
			{
				info.sd_rate = (BYTE)((lpTarget->Shield*100)/(lpTarget->MaxShield+lpTarget->AddShield));
		...
		else
		{
			...
			info.defense = lpTarget->Defense;
			info.min_damage = lpTarget->PhysiDamageMin;
			info.max_damage = lpTarget->PhysiDamageMax;
		}
...
void GCItemPostRecv(PMSG_POSTITEM_RECV* aRecv, int aIndex)
{
	...
	if(INVENTORY_FULL_RANGE(aRecv->itemPlace) == 0)
	{
		return;
	}
	...
	if (!lpObj->Inventory[aRecv->itemPlace].IsItem())
	{
		return;
	}

	PMSG_POSTITEM_SEND pMsg = {0};

	pMsg.h.set(0xFB,0x1C,sizeof(pMsg));
	gItemManager.ItemByteConvert(pMsg.ItemInfo,lpObj->Inventory[aRecv->itemPlace]);
	memcpy ( &pMsg.itemName[0], &aRecv->itemName[0], 60 );
	memcpy_s(&pMsg.Name[0],sizeof(pMsg.Name),lpObj->Name,sizeof(lpObj->Name));

	DataSendAll((BYTE*)&pMsg,pMsg.h.size);
}

void GCPostItemProc ( PMSG_POSTITEM* aRecv, short aIndex ) {
    if ( OBJECT_RANGE ( aIndex ) == 0 ) { return; }

    LPOBJ lpObj = &gObj[aIndex];
    int slen = strlen ( aRecv->chatmsg );

    if ( slen < 1 || slen > ( MAX_CHAT_MESSAGE_SIZE - 1 ) ) { return; }

   // if ( ReadConfig.SCFPSON == 1 ) {
     //SCFPostItemSend ( lpObj->m_Index, aRecv );
	PMSG_POSTITEM pResult = { 0 };
	memcpy ( &pResult.chatid[0], &aRecv->chatid[0], 10 );
    memcpy ( &pResult.chatmsg[0], &aRecv->chatmsg[0], 60 );
    memcpy ( &pResult.item_data[0], &aRecv->item_data[0], 107 );
	pResult.h.set(0x78,sizeof(pResult));
   // memcpy ( &pResult, ( LPBYTE ) aRecv, sizeof ( pResult ) );
	DataSendAll((BYTE*)&pResult,pResult.h.size);
   // }
}

void GCEntryCountSend(LPOBJ lpObj, BYTE type)
{
	PMSG_ENTRY_COUNT_SEND pMsg;
	pMsg.header.set(0xFB, 0x97, sizeof(pMsg));

	pMsg.type = type;

	if(type == 0)
	{
		pMsg.EntryCount = lpObj->BCEntryCount;
		pMsg.MaxEntryCount = gServerInfo.m_BloodCastleMaxDayEntry[lpObj->AccountLevel];
	}
	else
	{
		pMsg.EntryCount = lpObj->DSEntryCount;
		pMsg.MaxEntryCount = gServerInfo.m_DevilSquareMaxDayEntry[lpObj->AccountLevel];
	}
...
void CGReqRegGuildMark(PMSG_REQ_REGGUILDMARK * lpMsg, int iIndex) 
{
	...
	//if(gObj[iIndex].Interface.use != 0 || gObj[iIndex].State == OBJECT_DELCMD || gObj[iIndex].DieRegen != 0 || gObj[iIndex].Teleport != 0 || gObj[iIndex].PShopOpen != 0 || gObj[iIndex].SkillSummonPartyTime != 0)
	...
	{
		if(lpObj->Interface.type != INTERFACE_COMMON)
		{
			return;
		}
...
void CGItemDropInfoReq(PMSG_ITEMDROPINFO_REQ* lpMsg, int aIndex)
{
	CItem *Item = gMap[gObj[aIndex].Map].m_Item[lpMsg->curItem].GetItem();
	...
	if (Item->m_Index == GET_ITEM(14,15)) //Money
	{
		return;
	}

	PMSG_ITEMDROPINFO_SEND pMsg;
	pMsg.Head.set(0xFB,0x19,sizeof(pMsg));
	gItemManager.ItemByteConvert(pMsg.ItemInfo,(*Item));
	//gItemManager.ItemByteConvert(pMsg.ItemInfo,gObj[aIndex].Inventory[0]);
...
void CGGetWarehouseNumber(PMSG_CHANGEWAREHOUSE_RECV* lpMsg, int aIndex)
{
	...
	gWarehouse.CGWarehouseClose(lpObj->Index, 1);
	// Сначала приходит загрузка, а потом закрытие //
...
void CGUserPanelReq(int aIndex)
{
#if (GAMESERVER_UPDATE == 603)
...
void CGRecvJBankAct(PMSG_JBANK_ACT_SEND* lpMsg, int aIndex)
{
	switch(lpMsg->index)
	{
		case 0:
			(lpMsg->actType==0 ? gCommandManager.SendZenCustomVault(&gObj[aIndex],"10000000"):gCommandManager.RecvZenCustomVault(&gObj[aIndex],"10000000"));
			break;
		case 1:
			(lpMsg->actType==0 ? gCommandManager.SendChaosCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvChaosCustomVault(&gObj[aIndex],"1"));
			break;
		case 2:
			(lpMsg->actType==0 ? gCommandManager.SendBlessCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvBlessCustomVault(&gObj[aIndex],"1"));
			break;
		case 3:
			(lpMsg->actType==0 ? gCommandManager.SendSoulCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvSoulCustomVault(&gObj[aIndex],"1"));
			break;
		case 4:
			(lpMsg->actType==0 ? gCommandManager.SendLifeCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvLifeCustomVault(&gObj[aIndex],"1"));
			break;
		case 5:
			(lpMsg->actType==0 ? gCommandManager.SendCreationCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvCreationCustomVault(&gObj[aIndex],"1"));
			break;
		case 6:
			(lpMsg->actType==0 ? gCommandManager.SendGuardianCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvGuardianCustomVault(&gObj[aIndex],"1"));
			break;
		case 7:
			(lpMsg->actType==0 ? gCommandManager.SendHarmonyCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvHarmonyCustomVault(&gObj[aIndex],"1"));
			break;
		case 8:
			(lpMsg->actType==0 ? gCommandManager.SendLowStoneCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvLowStoneCustomVault(&gObj[aIndex],"1"));
			break;
		case 9:
			(lpMsg->actType==0 ? gCommandManager.SendHighStoneCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvHighStoneCustomVault(&gObj[aIndex],"1"));
			break;
		case 10:
			(lpMsg->actType==0 ? gCommandManager.SendGemStoneCustomVault(&gObj[aIndex],"1"):gCommandManager.RecvGemStoneCustomVault(&gObj[aIndex],"1"));
			break;
	}
}

void CGAutoHealthStateRecv(PMSG_AUTO_HEALTH_RECV*lpMsg, int aIndex)
{
	gObj[aIndex].is_use_auto_health = lpMsg->state;
}
...
void CGResetStatsRecv(PMSG_RESET_STATS_RECV* lpMsg, int aIndex)
{
	...
	if (lpMsg->Type == 1)
	{
		...
		if (gProtection.GetCustomState(CUSTOM_STAT))
			NewPoints += lpObj->NewStat;

		NewPoints -= gServerInfo.m_CustomResetPointPricePoint[lpObj->AccountLevel];
		...
		if (gProtection.GetCustomState(CUSTOM_STAT))
			lpObj->LevelUpPoint -= gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].NewStat;

		GCNewCharacterInfoSend(lpObj);
	}
	else if (lpMsg->Type == 0)
	{
		gCashShop.GDCashShopRecievePointSend(lpObj->Index,(DWORD)&ResetStatsCallback,(DWORD)gServerInfo.m_CustomResetPointPriceWC[lpObj->AccountLevel],0);
	}
}
...
void CGOffTradeRecv(PMSG_OFFTRADE_RECV* lpMsg, int aIndex)
{
	...
	////gCustomStore.OpenCustomStore(&gObj[aIndex], 0);
	//				break;
	//			case 0x12:
	//				gCustomStore.OpenCustomStore(&gObj[aIndex], 1);
	//				break;
	//			case 0x13:
	//				gCustomStore.OpenCustomStore(&gObj[aIndex], 2);
	//				break;
	//			case 0x14:
	//				gCustomStore.OpenCustomStore(&gObj[aIndex], 3);
	//				break;
	//			case 0x15:
	//				gCustomStore.OpenCustomStore(&gObj[aIndex], 4);
	//				break;
	//			case 0x16:
	//				gCustomStore.OpenCustomStore(&gObj[aIndex], 5);
	//				break;
	//			case 0x17:
	//				gCustomStore.CommandCustomStoreOffline(&gObj[aIndex], NULL);
	//				break;
...
void ResetStatsCallback(LPOBJ lpObj,int type,DWORD slot,DWORD WCoinC,DWORD WCoinP,DWORD GoblinPoint)
{
	if (WCoinC < gServerInfo.m_CustomResetPointPriceWC[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, "You don't have enough WCoinC");
		return;
	}
...
#if (GAMESERVER_UPDATE == 603)
	lpObj->Leadership = gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].Leadership+(g_ResetSystem.m_BonusCommand*lpObj->Reset)+(gGRSystem.m_BonusCommand*lpObj->MasterReset);
#else if 
	lpObj->Leadership = gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].Leadership;
#endif
	...
	gCashShop.GDCashShopSubPointSaveSend(lpObj->Index,0,gServerInfo.m_CustomResetPointPriceWC[lpObj->AccountLevel],0,0);
...
void GCSendCustomWingsCharset(LPOBJ lpObj, int WingIndex)
{
