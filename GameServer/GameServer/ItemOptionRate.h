...
#define MAX_ITEM_EXCELLENT_RATE 6
...
struct ITEM_EXCELLENT_RATE_INFO
{
	int Index;
	int Rate[MAX_ITEM_EXCELLENT_RATE];
};
...
public:
	...
	bool GetItemExcellent(int index,int max_option,BYTE* option);
	bool MakeNewOption(int ItemIndex,int value,BYTE* option, int index);
...
private:
	...
	std::map<int,ITEM_EXCELLENT_RATE_INFO> m_ItemExcellentRateInfo;
};
