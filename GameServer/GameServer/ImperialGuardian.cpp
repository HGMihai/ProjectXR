...
CImperialGuardian::CImperialGuardian() // OK
{
	...
	for(int i=0;i<7;i++)
	{
		this->m_SecromiconDropRate[i].Rate[0] = 100;
		this->m_SecromiconDropRate[i].Rate[1] = 100;
		this->m_SecromiconDropRate[i].Rate[2] = 100;
		this->m_SecromiconDropRate[i].Rate[3] = 100;
	}
		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_NotifyTime = lpMemScript->GetFloatNumber();
				...
				else if(section == 3)
				{
					...
					int DayOfWeek = lpMemScript->GetNumber();

					this->m_SecromiconDropRate[DayOfWeek-1].Rate[0] = lpMemScript->GetAsNumber();

					this->m_SecromiconDropRate[DayOfWeek-1].Rate[1] = lpMemScript->GetAsNumber();

					this->m_SecromiconDropRate[DayOfWeek-1].Rate[2] = lpMemScript->GetAsNumber();

					this->m_SecromiconDropRate[DayOfWeek-1].Rate[3] = lpMemScript->GetAsNumber();
...
void CImperialGuardian::DropSideBossItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	gAchievements.ImperialGuardianEvent(lpTarget->Index, lpObj->Index);

	for(int n=0;n < MAX_IG_USER;n++)
	{
		if(OBJECT_RANGE(this->m_User[n].Index) != 0 && this->m_DayOfWeek != 0)
		{
			if((GetLargeRand()%100) < this->m_SecromiconDropRate[this->m_DayOfWeek-1].Rate[gObj[this->m_User[n].Index].AccountLevel])
			{
				GDCreateItemSend(lpTarget->Index,lpObj->Map,(BYTE)lpObj->X,(BYTE)lpObj->Y,(GET_ITEM(14,102)+this->m_DayOfWeek),0,0,0,0,0,this->m_User[n].Index,0,0,0,0,nullptr,0xFF,0);
			}
		}
	}
