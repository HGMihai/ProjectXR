...
cChaosRecipe::cChaosRecipe(int RecipeIndex)
{
...
void cChaosRecipe::ActStart(LPOBJ lpUser, cChaosRecipe* lpRecipe, ChaosCombination* lpCombination)
{
	...
	lpUser->ChaosSuccessRate += (int)(lpUser->ChaosSuccessRate*lpUser->EffectOption.MulChaosSuccessRate)/100;

	lpUser->ChaosSuccessRate = ((lpUser->ChaosSuccessRate>100)?100:lpUser->ChaosSuccessRate);
...
//	ChaosBoxWingMixItemDown(lpObj);
	...
	gLog.Output(LOG_CUSTOM_CHAOSBOX,"[CustomChaosBox] [%s] [%s] Combination has failed (Recipe: %s, Combination: %s, Rate: %d)",
...
void cChaosRecipe::ActSuccess(LPOBJ lpUser, cChaosRecipe* lpRecipe, ChaosCombination* lpCombination, ChaosProduce* lpProduce)
{
	...
	else
	{
		...
		gLog.Output(LOG_CUSTOM_CHAOSBOX,"[CustomChaosBox] [%s] [%s] (Warning) Produce have wrong level (Recipe: %s, Combination: %s)",
			lpUser->Account, lpUser->Name, lpRecipe->Name, lpCombination->Name);
	...
	else
	{
		...
		gLog.Output(LOG_CUSTOM_CHAOSBOX,"[CustomChaosBox] [%s] [%s] (Warning) Produce have wrong option (Recipe: %s, Combination: %s)",
			lpUser->Account, lpUser->Name, lpRecipe->Name, lpCombination->Name);
	}
	...
	gLog.Output(LOG_CUSTOM_CHAOSBOX,"[CustomChaosBox] [%s] [%s] Combination has success (Recipe: %s, Combination: %s, Rate: %d)",
		lpUser->Account, lpUser->Name, lpRecipe->Name, lpCombination->Name, 
		lpUser->ChaosSuccessRate);
}
// -------------------------------------------------------------------------------
...
bool cChaosRecipe::CheckIngredient(LPOBJ lpUser, short ChaosPos, ChaosIngredient* Ingredient)
{
	...
	if (lpUser->ChaosBox[ChaosPos].m_Option < Ingredient->OptionMin && Ingredient->OptionMin != (BYTE)-1
		|| lpUser->ChaosBox[ChaosPos].m_Option > Ingredient->OptionMax && Ingredient->OptionMax != (BYTE)-1)
	{
		return false;
	}
...
bool cChaosRecipe::CheckTalisman(LPOBJ lpUser, short ChaosPos, ChaosTalisman* lpTalisman)
{
	...
	if (lpUser->ChaosBox[ChaosPos].m_Option < lpTalisman->OptionMin && lpTalisman->OptionMin != (BYTE)-1
		|| lpUser->ChaosBox[ChaosPos].m_Option > lpTalisman->OptionMax && lpTalisman->OptionMax != (BYTE)-1)
	{
		return false;
	}
...
void cChaosBoxManager::Read(char* File)
{
	...
	for (pugi::xml_node CombinationNode = Recipe.child("mixlist").child("recipe"); CombinationNode; CombinationNode = CombinationNode.next_sibling())
	{
		...
		sprintf(FileName, "%s%s", gPath.GetFullPath("..\\LTPData\\ChaosMachine\\"/*PATH_CHAOSMACHINE*/), CombinationNode.text().as_string());
...
bool cChaosBoxManager::StartMix(LPOBJ lpUser, int RecipeIndex)
{
