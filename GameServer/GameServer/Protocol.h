...
struct PSSBMSG_HEAD
{
	void set(BYTE head, BYTE subh, BYTE subh2, BYTE size) // OK
	{
		this->type = 0xC1;
		this->size = size;
		this->head = head;
	...
	void setE(BYTE head, BYTE subh, BYTE subh2, BYTE size) // OK
	{
		this->type = 0xC3;
		this->size = size;
		this->head = head;
...
struct PSSWMSG_HEAD
{
	void set(BYTE head, BYTE subh, BYTE subh2, WORD size) // OK
	{
		this->type = 0xC2;
		this->size[0] = SET_NUMBERHB(size);
		this->size[1] = SET_NUMBERLB(size);
	...
	void setE(BYTE head, BYTE subh, BYTE subh2, WORD size) // OK
	{
		this->type = 0xC4;
		this->size[0] = SET_NUMBERHB(size);
		this->size[1] = SET_NUMBERLB(size);
...
struct PMSG_ITEMDROPINFO_SEND
{
	PSBMSG_HEAD Head;
	BYTE ItemInfo[12];
};

struct PMSG_ITEMDROPINFO_REQ
{
	PSBMSG_HEAD	Head;
	DWORD curItem;
};
...
struct PMSG_CONNECT_ACCOUNT_RECV
{
	...
	//#endif
	//#pragma pack()
	//#elif (GAMESERVER_LANGUAGE == 0 && GAMESERVER_UPDATE >= 803)
	//
	//PSBMSG_HEAD header; // C3:F1:01
	//char account[10];
	//char password[20];
...
struct PMSG_HEALTH_BAR_MAP_SEND
{
	PSWMSG_HEAD	header;
	int count;
	HEALTH_BAR_MAP map_data[150];
};
...
struct PMSG_CHARACTER_INFO_DATA
{
	PSWMSG_HEAD header;
	int Count;
	int CharacterDeleteGReset;
	int CharacterDeleteReset;
	int CharacterDeleteLevel;
};
...
void GCChatTargetSend(LPOBJ lpObj,int senduser,char* message);
void GCChatSend(LPOBJ lpObj,char* message);
...
struct PMSG_ENTRY_COUNT_SEND
{
	PSBMSG_HEAD header; // C1:9A
	BYTE type;
	int EntryCount;
	int MaxEntryCount;
};
