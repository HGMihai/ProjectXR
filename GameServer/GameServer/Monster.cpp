...
CQuestNpcTeleport gQuestNpcTeleport;
...
void CQuestNpcTeleport::Run(int aIndex)
{
	...
	if ( this->TimeCount > gServerInfo.m_QuestNPCTeleportTime )
	{
		...
		pMsg.h.setE(0x19,sizeof(pMsg));
		pMsg.MagicNumberH = SET_NUMBERHB(6);
		pMsg.MagicNumberL = SET_NUMBERLB(6);
		pMsg.SourceNumberH = SET_NUMBERHB(aIndex);
		pMsg.SourceNumberL = SET_NUMBERLB(aIndex);
		pMsg.TargetNumberH = SET_NUMBERHB(aIndex);
		pMsg.TargetNumberL = SET_NUMBERLB(aIndex);
		...
		gObj[aIndex].Map = this->m_QuestNPCTeleportPos[tableindex].mapnum;
		...
		LogAdd(LOG_BLUE,"[Marlon] [%d] teleported to %d map", tableindex, gObj[aIndex].Map);
	}
}
...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		/*if (gProtection.m_Index == eCustomers::MuLambda)
		{
			gEffectManager.AddEffect(lpTarget,0,EFFECT_CHRISTMAS1,10800,0,0,0,0);
		}
		else
		{*/
	...
	if((DropIndex=gMonsterManager.GetMonsterItem(DropLevel,ExcItemDrop,gMapManager.GetMapSocketItemDrop(lpObj->Map))) >= GET_ITEM(0,0) && (GetLargeRand()%((lpObj->ItemRate==0)?1:lpObj->ItemRate)) < ItemDropRate)
	{
		...
		ItemOption1 = gItemManager.GetItemSkill(ItemIndex,ExcItemDrop);
		...
		gItemOptionRate.MakeNewOption(ItemIndex,ItemNewOption,&ItemNewOption,((ExcItemDrop==0)?0:1));
	...
	else if(lpObj->Money > 0 && (GetLargeRand()%((lpObj->MoneyRate==0)?1:lpObj->MoneyRate)) < 10)
	{
		...
		{
			money = ((__int64)lpObj->Money*gServerInfo.m_MoneyAmountDropRate[lpTarget->AccountLevel])/100;
		}
		else
		{
			money = ((__int64)lpObj->Money*gServerInfo.m_MoneyMasterAmountDropRate[lpTarget->AccountLevel])/100;
		}
		...
		/*if (money > 0)
		{
			int MoneyRand = GetLargeRand() % (money/100*30);
			int TypeRand = GetLargeRand()%2;
		...
		money += (int)(money*lpObj->EffectOption.MulDropZen)/100;
...
#if (GAMESERVER_UPDATE == 603 || IS_NEW_CLIENT)
	if (MonsterClass >= 580 && MonsterClass < 800)
	{
		lpObj->Type = OBJECT_MONSTER;
	}
	/*else if (MonsterClass >= 620 && MonsterClass <= 650)
	{
		lpObj->Type = OBJECT_NPC;
	}*/
#endif
...
BOOL gObjSetMonsterTest(int aIndex, int MonsterClass, LPSTR FunctionName)
{
	int Level;
	LPOBJ lpObj;
	...
	/*if (MonsterClass >= 580 && MonsterClass <= 619)
	{
	...
	strncpy(gObj[aIndex].Name, lpInfo->Name , 10 );
	...
	//memcpy(lpObj->Resistance, lpInfo->Resistance, 7 );
	gObjSetInventory1Pointer(lpObj);
...
int gObjAddPlayer(int map) // OK
{
	...
	int index = -1;
	int count = gObjTotalUser+OBJECT_START_USER;
...
BOOL gObjMonsterGetTargetPosOLD(LPOBJ lpObj)
{
...
{
	int search_count = MAX_ROAD_PATH_TABLE/2-1;
	BYTE attr;
	BOOL result;
	...
	int target_player_x = lpTargetObj->X;
	int target_player_y = lpTargetObj->Y;
	
	int monster_target_x = target_player_x;
	int monster_target_y = target_player_y;
	
	int dis = lpObj->AttackRange;
	...
	if ( lpObj->X < monster_target_x ) target_player_x -= dis;
	if ( lpObj->X > monster_target_x ) target_player_x += dis;
	if ( lpObj->Y < monster_target_y ) target_player_y -= dis;
	if ( lpObj->Y > monster_target_y ) target_player_y += dis;

	int search_hp = GetPathPacketDirPos((lpTargetObj->X - target_player_x), (lpTargetObj->Y - target_player_y)) * 2;

	if ( gMap[lpObj->Map].CheckStandAttr(target_player_x, target_player_y) == 0 )
	{
		while ( search_count-- )
		{
			monster_target_x = lpTargetObj->X + RoadPathTable[search_hp];
			monster_target_y = lpTargetObj->Y + RoadPathTable[search_hp+1];
			attr = gMap[lpObj->Map].GetAttr(monster_target_x, monster_target_y);
			result = gObjMonsterMoveCheck(lpObj, monster_target_x, monster_target_y);

			if ( lpObj->Class == 249 )
			{
				if ( (attr&2)!=2 && result == TRUE )
				{
					lpObj->MTX = monster_target_x;
					lpObj->MTY = monster_target_y;
					return TRUE;
				}
			}
			else if ( (attr&1) != 1 && (attr&2) != 2 && (attr&4) != 4 && (attr&8) != 8 && result == TRUE )
			...
			if ( search_hp > MAX_ROAD_PATH_TABLE-1 )
			{
				search_hp = 0;
			}
		}
	...
	attr = gMap[lpObj->Map].GetAttr(target_player_x, target_player_y);
	result = gObjMonsterMoveCheck(lpObj, monster_target_x, monster_target_y);
	...
	if ( lpObj->Class == 249 )
	{
		if ( (attr&2)!=2 && result == TRUE )
		{
			lpObj->MTX = target_player_x;
			lpObj->MTY = target_player_y;
			return TRUE;
		}
	}
	else if ( (attr&1) != 1 && (attr&2) != 2 && (attr&4) != 4 && (attr&8) != 8 && result == TRUE )
	...
	for (n=0;n<MAX_VIEWPORT;n++)
	{
		...
		if ( tObjNum >= 0 )
		{
			if ( (gObj[tObjNum].Type == t1 || gObj[tObjNum].Type == t2) && (gObj[tObjNum].Live != FALSE) )
			{
				...
				else if ( (gObj[tObjNum].Authority &2) != 2 && (gObj[tObjNum].Authority &32) != 32 && gObj[tObjNum].Teleport == 0 && gMap[gObj[tObjNum].Map].CheckAttr(gObj[tObjNum].X,gObj[tObjNum].Y,1) == 0 )
				{
					//gMap[gObj[tObjNum].Map].CheckAttr(gObj[tObjNum].X,gObj[tObjNum].Y,1) == 0
					//BYTE attr = gMap[lpObj->Map].GetAttr(gObj[tObjNum].X, gObj[tObjNum].Y);

					//LogAdd(LOG_TEST,"Name: %s, attr: %d, X: %d Y: %d",gObj[tObjNum].Name,attr,gObj[tObjNum].X,gObj[tObjNum].Y);
...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	if ( lpObj->Attribute == 100 )
	{
		if ( lpObj->SummonIndex >= 0 && lpObj->SummonIndex < MAX_OBJECT )
		{
			...
			LPOBJ lpCallMonObj = &gObj[lpObj->SummonIndex];
	...
	if ( lpObj->ActionState.Move != 0 )
	{
		...
		else
		{
			...
			lpObj->ActionState.Move = (DWORD)0; // monster stuck fix
			return;
		}
	}
...
void gObjMonsterBaseAct(LPOBJ lpObj)
{
					...
					if(lpObj->IsBot >= 1)
					{
						switch(lpObj->IsBot)
						{
							case 2:		//Bot Alchemist
							{
								if ( !(rand()%70) )
								{
									GCChatSend(lpObj, "I combine items!");
							...
							case 3:		//Bot Buffer
							{
								if ( !(rand()%70) )
								{
									GCChatSend(lpObj, "Trade me to get lucky buff!");
									GCActionSend(lpObj,147,lpObj->Index,0);
								}
							}
							break;
							...
							case 5:		//Bot Trader
							{
								if ( !(rand()%70) )
								{
									GCChatSend(lpObj, "I trade items for jewels!");
									GCActionSend(lpObj,147,lpObj->Index,0);
								}
							}
							break;
							case 6:		//Bot Vip Shop
							{
								if ( !(rand()%70) )
								{
									GCChatSend(lpObj, "Trade me to see VIP Shop!");
									GCActionSend(lpObj,147,lpObj->Index,0);
								}
							}
							break;
	...
	else if ( lpObj->ActionState.Emotion == 1 )
	{
		...
		if ( lpObj->TargetNumber >= 0 && lpObj->PathStartEnd == 0)
		{
			...
			//LogAdd(LOG_TEST,"MonsterName: %s, Dis: %d, Range: %d",lpObj->Name,dis,attackrange);

			if ( dis <= attackrange )
			{
				int target_user_index = lpObj->TargetNumber;
				int map = gObj[target_user_index].Map;

				if ( gMap[map].CheckWall_new(lpObj->X, lpObj->Y, gObj[target_user_index].X, gObj[target_user_index].Y) == TRUE )
				{
					BYTE attr = gMap[map].GetAttr(gObj[target_user_index].X, gObj[target_user_index].Y);
				...
				else
				{
					...
					lpObj->ActionState.Emotion = 0;
					lpObj->ActionState.Move = 1;*/
					gObjMonsterMoveAction(lpObj); // monster stuck fix
				}
			}
			else
			{
