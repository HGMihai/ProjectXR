...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x05:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				case 0x00:
					...
					break;
				case 0x10:
					gGuildWarehouse.DGGuildWarehouseItemRecv((SDHP_GUILDWAREHOUSE_ITEM_RECV*)lpMsg);
					break;
				case 0x01:
		...
		case 0xFE:
		{
			...
			{
				g_ItemMarket.DGAnsItemList((SDHP_ANS_MARKET_ITEM*)lpMsg);
			}

			if (lpMsg[0] == 0xC1 && lpMsg[3] == 0x01)
			{
				g_ItemMarket.DGAnsItemSell((SDHP_ANS_MARKET_SELL*)lpMsg);
			}

			if (lpMsg[0] == 0xC1 && lpMsg[3] == 0x02)
			{
				g_ItemMarket.DGAnsItemBuy((SDHP_ANS_MARKET_BUY*)lpMsg);
			}

			if (lpMsg[0] == 0xC1 && lpMsg[3] == 0x03)
			{
				g_ItemMarket.DGAnsItemStatus((SDHP_ANS_MARKET_STATUS*)lpMsg);
			}
#if(MARKET_ITEM_RECV)
			if (lpMsg[0] == 0xC1 && lpMsg[3] == 0x04)
			{
				g_ItemMarket.GDReqItemDate((SDHP_REQ_MARKET_ITEMRECV*)lpMsg);
			}
#endif
		}
		break;
...
#if (GAMESERVER_UPDATE == 603)
				case 0x00:
					gDiablo.DGGetGuildList((DG_DIABLO_GUILDLIST*)lpMsg);
					break;
#endif
				case 0x01:
					...
					break;
				case 0x02:
					gGuildWarehouse.DGUsersWareStatusRecv((PMSG_WARE_SATUS_RECV*)lpMsg,size);
					break;
				case 0x03:
					gGuildWarehouse.DGCheckAllowWarehouseRecv((PMSG_STATUS_CHECK_ANS*)lpMsg);
					break;
				case 0x04:
					gAchievements.DGPlayerLoad((PMSG_DGANS_ACH_LOAD*)lpMsg);
					break;
				case 0x05:
					gRankings.GDRecvRankings((SDHP_RANKINGS_INFO_SEND*)lpMsg);
					break;
				case 0x06:
					gRankings.DGRecvCharacterInfo((DG_RANKINGS_INFO_REQUEST_RECV*)lpMsg);
					break;
				case 0x07: DGRecvEntryCount((PMSG_ENTRY_COUNT_REQ_RECV*)lpMsg); break;
			}
			break;
	}
...
void DGCharacterListRecv(SDHP_CHARACTER_LIST_RECV* lpMsg) // OK
{
...
#if GAMESERVER_UPDATE == 601 && GAMESERVER_LANGUAGE == 0
	pMsg.charsCount = 5;
#endif

		#if(GAMESERVER_UPDATE>=602)
#if(GAMESERVER_UPDATE>803)//2017/08/03
	...
	PetpMsg.header.set(0xF3,0xF6,0);
	...
	for(int n=0;n < lpMsg->count;n++)
	{
		...
		else if(gCustomWing.CheckCustomWingByItem(GET_ITEM(12,TempInventory[7])) != 0)
		{
			...
			info.CharSet[5] |= 12;
			info.CharSet[9] |= 8+gCustomWing.GetCustomWingIndex(GET_ITEM(12,TempInventory[7]))+1;
			/*info.CharSet[5] |= 12;
		...
		{
			petinfo.PetCharSet[0] |= (TempInventory[8]-144) << 1;
		}
		...
		#endif
		/*else if (TempInventory[8] == 145)
		{
			info.CharSet[16] |= 0x70;
		}*/
		#pragma endregion
		...
		#if (GAMESERVER_UPDATE > 803)
		info.PK_Level=2;
#pragma message("Need to fix PK LEVEL")
		#endif
...
#if(GAMESERVER_UPDATE>803)//判断是否可以创建女骑士//2017/02/28
...
#if (GAMESERVER_UPDATE == 603)
	gServerInfo.SendServerInfo(lpMsg->index);
#endif
	GCCharacterCreationEnableSend(lpMsg->index,0,gObj[lpMsg->index].ClassCode);
	//gObjectManager.CharacterMakePreviewCharSetTest(lpMsg->index);
}
...
void GCCharacterInfoSend(SDHP_CHARACTER_LIST_RECV* lpMsg)
{
	...
	pMsg.CharacterDeleteGReset = gServerInfo.m_CharacterDeleteGReset[gObj[lpMsg->index].AccountLevel];
	pMsg.CharacterDeleteReset = gServerInfo.m_CharacterDeleteReset[gObj[lpMsg->index].AccountLevel];
	pMsg.CharacterDeleteLevel = gServerInfo.m_CharacterDeleteLevel[gObj[lpMsg->index].AccountLevel];
...
#if (GAMESERVER_UPDATE < 803)
	CGCharacterListRecv(lpMsg->index);
#endif
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	if(lpObj->MapServerMoveRequest == 0)
	{
		if((lpObj->NextServerCode=gMapServerManager.CheckMapServerMove(lpObj->Index,lpObj->Map,lpObj->LastServerCode)) != gServerInfo.m_ServerCode)
		{
			...
			{
				//LogAdd(LOG_TEST,"GJMapServerMoveSend ");
...
//	pMsg.Rank = lpObj->Rank;
	...
	//gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,"Hello %s, Now online: %d", lpObj->Name, gObjTotalUser);

	if (lpObj->Reset < gServerInfo.m_ServerMinUserReset || lpObj->Reset > gServerInfo.m_ServerMaxUserReset)
	{

		gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, "Server requirements: %d - %d Reset", gServerInfo.m_ServerMinUserReset, gServerInfo.m_ServerMaxUserReset);
		gObjectManager.CharacterGameCloseSet(lpObj->Index, 2);
		return;
	}

	if (lpObj->MasterReset < gServerInfo.m_ServerMinUserGReset || lpObj->MasterReset > gServerInfo.m_ServerMaxUserGReset)
	{
		gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, "Server requirements: %d - %d Grand Reset", gServerInfo.m_ServerMinUserGReset, gServerInfo.m_ServerMaxUserGReset);
		gObjectManager.CharacterGameCloseSet(lpObj->Index, 2);
		return;
	}
	...
	{
		gParty.GCPartyListSend(lpObj->PartyNumber);
	}*/

	if (gIpManager.CheckIpAddress(lpObj->IpAddr) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, "You can connect only from %d accounts!", gServerInfo.m_MaxIpConnection);
		gObjUserKill(lpObj->Index);
	}

	gCostumeSystem.GCCostumeListSend(lpObj->Index);

	gFileCheck.GCChecksumListSend(lpObj->Index);

	gActiveInvasions.send_list_to_client();

#if GAMESERVER_UPDATE == 603 || IS_NEW_CLIENT
	GCNewCharacterInfoSend(lpObj);
	gServerInfo.SendServerInfo(lpObj->Index);
	g_ResetSystem.SendResetData(lpObj, false);
	gGRSystem.SendResetData(lpObj, false);
	g_UserPanel.SendUserPanel(lpObj->Index);
	//gDonateShop.GCDonateShopInfo(lpObj->Index);
	gCustomStore.SendNamesToClient(lpObj->Index);
	...
	gPackSystem.GCSendData(lpObj->Index);
	//gBankEx.GCUpdateBankEx(lpObj->Index);
	gAchievements.GCSettingsData(lpObj->Index);
	gRoulette.send_roulette_data(lpObj->Index);

	gCustomMove.move_data_send(lpObj->Index);

#ifdef IS_NEW_CLIENT
	//gItemValue.GCItemValueSend(lpObj->Index,gItemValue.GetBeginIterator());
#endif

	if (!lpMsg->IsStartItem)
	{
		gCustomStartItem.MakeStartItem(lpObj);
	}

	if( gNewsBoard.m_ShowOnLogin )
	{
		gNewsBoard.OpenMain(lpObj);
	}
#endif

	g_ExWinQuestSystem.UserConnect(lpObj->Index);
	
	gCustomQuest.connect(lpObj);
...
void DGCreateItemRecv(SDHP_CREATE_ITEM_RECV* lpMsg) // OK
{
	...
	else if(lpMsg->Map == 0xEB)
	{
		...
		/*if(lpMsg->ItemIndex == GET_ITEM(14,13))
			item.m_Serial = 1;*/
		...
		{
			slot = gItemManager.InventoryInsertItem(lpObj->Index,item);
		}
...
#ifdef IS_NEW_CLIENT
	pMsg.game_settings = lpMsg->game_settings;
#endif
...
void GDCharacterInfoSend(int aIndex,char* name) // OK
{
	if(gObjIsAccountValid(aIndex,gObj[aIndex].Account) == 0)
	{
		LogAdd(LOG_RED, "[GDCharacterInfoSend] Invalid Account [%d](%s)", aIndex, gObj[aIndex].Account);
		return;
	}
...
void GDCreateItemSend(int aIndex,BYTE map,BYTE x,BYTE y,int index,BYTE level,BYTE dur,BYTE Option1,BYTE Option2,BYTE Option3,int LootIndex,BYTE NewOption,BYTE SetOption,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus,DWORD duration) // OK
{
	...
	pMsg.HaveSerial = gItemManager.GetItemHaveSerial(index);
	...
	if(gPentagramSystem.IsPentagramItem(index) != 0 || gPentagramSystem.IsPentagramJewel(index) != 0 || gPentagramSystem.IsPentagramMithril(index) != 0)
	{
		...
		LogAdd(LOG_GREEN,"create pentagram opt %x",pMsg.SocketOptionBonus);
	}
	else
	{
		pMsg.SocketOptionBonus = SocketOptionBonus;
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	pMsg.PointsCount = lpObj->PointsCount;
	pMsg.PointsTime = lpObj->PointsTime;

	pMsg.MapQuestSystemMap = lpObj->MapQuestSystemMap;
	pMsg.MapQuestSystemIndex = lpObj->MapQuestSystemIndex;
	pMsg.MapQuestSystemState = lpObj->MapQuestSystemState;
...
#ifdef IS_NEW_CLIENT
void GDOptionDataSaveSend(int aIndex, BYTE* SkillKey, BYTE GameOption, BYTE QKey, BYTE WKey, BYTE EKey, BYTE ChatWindow, BYTE RKey, DWORD QWERLevel, DWORD game_settings)
#else
...
#ifdef IS_NEW_CLIENT
	pMsg.game_settings = game_settings;
#endif
...
void GDRankingTvTEventSaveSend(int aIndex, DWORD number, DWORD KillCount, DWORD DeathCount)
{
	LPOBJ lpObj = &gObj[aIndex];
...
void GDSetCharacterBlockSend(int aIndex, BYTE CtlCode)
{
	PMSG_SET_CHARACTER_BLOCK_SEND pMsg;

	pMsg.header.set(0xFB, 0x0F,sizeof(pMsg));
...
void GDSendEntryCountRequest(LPOBJ lpObj, BYTE type)
{
	...
	pMsg.lpObjAddr = (DWORD)lpObj;

	gDataServerConnection.DataSend((BYTE*)&pMsg, sizeof(pMsg));
}

void DGRecvEntryCount(PMSG_ENTRY_COUNT_REQ_RECV* lpMsg)
{
	auto lpObj = (LPOBJ)lpMsg->lpObjAddr;

	if (lpMsg->type == 0)
	{
		lpObj->BCEntryCount = lpMsg->EntryCount;
		gBloodCastle.NpcAngelMessenger(lpObj);
	}
	else
	{
		lpObj->DSEntryCount = lpMsg->EntryCount;
		gDevilSquare.NpcCharon(lpObj);
	}
}

void GDUpdateEntryCount(LPOBJ lpObj, BYTE type)
{
	PMSG_ENTRY_COUNT_UPDATE_SEND pMsg;
