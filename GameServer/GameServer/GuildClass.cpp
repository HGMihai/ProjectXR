...
GUILD_INFO_STRUCT * CGuildClass::AddGuild(int number, char* guildname, GUILDMARK mark, char * mastername, int score)
{
	...
	if ( pNewNode != NULL )
	{
		...
		//pNewNode->LoadGuildWarehouse = 0;
		pNewNode->WarehouseInUse = 0;
		pNewNode->WarehouseUserIndex = -1;
	...
	for ( int n=0;n<MAX_GUILD_USER;n++)
	{
		if ( pNode->Use[n] == TRUE )
		{
			if ( strcmp(pNode->Names[n], player_name ) == 0 )
			{
				...
				pNode->Count--;
				this->BuildMemberTotal(pNode);
				gGuildClass.AddGuildBuff(pNode);
				return TRUE;
			}
		}
	}
...
int CGuildClass::GetGuildBonus(GUILD_INFO_STRUCT* lpNode)
{
	//int value = 10;
	...
	for ( int n=0;n<MAX_GUILD_USER;n++)
	{
		if (lpNode->Use[n] != 0)
		{
			LPOBJ lpTarget = gObjFind(lpNode->Names[n]);
			if (!lpTarget)
			{
				continue;
			}
			totalReset += lpTarget->Reset;
			count++;
		}
	}
	...
	int value = (totalReset/count)/gServerInfo.m_CustomGuildBuffConstA;

	if (value < gServerInfo.m_CustomGuildBuffStartReset)
	{
		return 0;
	}

	if (value>gServerInfo.m_CustomGuildBuffMax)
	{
		value = gServerInfo.m_CustomGuildBuffMax;
	}

	if (value<gServerInfo.m_CustomGuildBuffMin)
	{
		value = gServerInfo.m_CustomGuildBuffMin;
	}

	return value;
}

void CGuildClass::AddGuildBuff(GUILD_INFO_STRUCT* lpNode)
{
	...
	int value = gGuildClass.GetGuildBonus(lpNode);

	for(int i=0;i < MAX_GUILD_USER;i++)
	{
		if(lpNode->Use[i] > 0)
		{
			int index = lpNode->Index[i];

			if (!gObjIsConnectedGP(index))
			{
				continue;
			}

			if (gEffectManager.CheckEffect(&gObj[index],EFFECT_GUILD_BUFF))
			{
				gEffectManager.DelEffect(&gObj[index],EFFECT_GUILD_BUFF);
			}

			if (value != 0)
			{
				gEffectManager.AddEffect(&gObj[index],0,EFFECT_GUILD_BUFF,0,value,0,0,0);
			}
		}
	}
}
