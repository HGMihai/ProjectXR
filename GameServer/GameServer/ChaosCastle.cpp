...
void CChaosCastle::CGChaosCastleEnterRecv(PMSG_CHAOS_CASTLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	if(OBJECT_RANGE(lpObj->PartyNumber) != 0)
	{
		...
		int pindex = gParty.GetMemberNumber(lpObj->PartyNumber, aIndex);

		if ( pindex >= 0 )
		{
			pMsg.number = pindex;
			gParty.CGPartyDelMemberRecv(&pMsg,aIndex);
		}
	}
...
void CChaosCastle::start_now()
{
	...
	LogAdd(LOG_GREEN,"[Set CC Start] At %2d:%2d:00",hour,minute);
