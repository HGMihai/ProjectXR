...
SOCKET_ITEM_TYPE_INFO* CSocketItemType::GetInfoById(int index)
{
	//this->m_SocketItemTypeInfo.at(index);
	if (this->m_SocketItemTypeInfo[index].Index != -1)
	{
		return &this->m_SocketItemTypeInfo[index];
	}
	return 0;
}


int CSocketItemType::GetCountOfSocketItems()
{
	return (int)this->m_SocketItemTypeInfo.size();
}
