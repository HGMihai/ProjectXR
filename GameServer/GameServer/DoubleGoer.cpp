...
CDoubleGoer::CDoubleGoer() // OK
{
	...
	this->m_CreateMonsterTimeTick = GetTickCount();
		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_CreateMonsterTime = lpMemScript->GetAsNumber();
				}
				else if(section == 1)
				{
					if(strcmp("end",lpMemScript->GetAsString()) == 0)
...
void CDoubleGoer::ProcState_START() // OK
{
	...
	if((GetTickCount()-this->m_CreateMonsterTimeTick) > this->m_CreateMonsterTime*1000)
	{
		this->m_CreateMonsterTimeTick = GetTickCount();
