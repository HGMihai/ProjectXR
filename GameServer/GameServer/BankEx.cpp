...
void CBankEx::InitUser(LPOBJ lpObj)
{
	...
	lpObj->BankEx.JLowStone		= 0;
	lpObj->BankEx.JHighStone	= 0;
...
void CBankEx::GDSavePoint(int aIndex)
{
	...
	pRequest.Credits	= lpUser->BankEx.Credits;
	...
	pRequest.LowStone	= lpUser->BankEx.JLowStone;
	pRequest.HighStone	= lpUser->BankEx.JHighStone;
...
{
	//LogAdd(LOG_RED, "BankEx %s", __FUNCTION__);
	//if( !gObjIsConnected(aIndex) )
	//{
	//	//LogAdd(LOG_RED, "BankEx return %s", __FUNCTION__);
	//	return;
	//}
	// ----
	LPOBJ lpUser = &gObj[aIndex];
...
void CBankEx::DGGetPoint(BANKEX_DG_GET_POINT * aRecv)
{
	...
	// ----
	lpUser->BankEx.Credits		= aRecv->Credits;
	...
	lpUser->BankEx.JLowStone	= aRecv->LowStone;
	lpUser->BankEx.JHighStone	= aRecv->HighStone;
...
void CBankEx::GCUpdateBankEx(int aIndex)
{
	...
	// ----
	pMsg.Credits = lpObj->BankEx.Credits;
	...
	pMsg.LowStone	= lpObj->BankEx.JLowStone;
	pMsg.HighStone	= lpObj->BankEx.JHighStone;
