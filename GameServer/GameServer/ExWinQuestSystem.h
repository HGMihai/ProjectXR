...
struct WinQuest_Table
{
	...
	BYTE MissionPercent[EW_MAX_MISSION];
	//-> Reward
...
struct ExWinQuest_DATA
{
	...
	int MissionPercent[EW_MAX_MISSION];
...
public:
	...
	static void GC_MiniInfoEx(int aIndex, int a, int m);
	void CG_AcceptQuest(int aIndex, CG_Accept_Done* aRecv);
	static void GC_Start(int aIndex, int a, int s);
	//Mission
	...
	bool NewDrop(int aIndexMob, int aIndexUser) const;
	//DataServer
