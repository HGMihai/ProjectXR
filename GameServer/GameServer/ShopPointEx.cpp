...
void ShopPointEx::AddTimeBonus(int UserIndex)
{
	...
	//}
	// ----
	gCashShop.GDCashShopAddPointSaveSend(UserIndex,0, this->m_TimeBonusWCoinC, this->m_TimeBonusWCoinP, this->m_TimeBonusWCoinG);
...
void ShopPointEx::AddEventBonus(int UserIndex, ShopPointExEvent::T Event)
{
	...
	gCashShop.GDCashShopAddPointSaveSend(UserIndex,0, this->m_EventBonus[Event].AddCoinC, this->m_EventBonus[Event].AddCoinP, this->m_EventBonus[Event].AddCoinG);
	// ----
...
void ShopPointEx::AddMonsterBonus(LPOBJ lpMonster, LPOBJ lpObj)
{
	...
	gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0, this->m_MobsData[MonsterSlot].AddCoinC, this->m_MobsData[MonsterSlot].AddCoinP, this->m_MobsData[MonsterSlot].AddCoinG);
	// ----
...
void ShopPointEx::SendNotify(int UserIndex, int CoinC, int CoinP, int CoinG, int Credits, char* Text)
{
	...
	if( CoinC > 0 )
	{
		gNotice.GCNoticeSend(UserIndex,1,0,0,0,0,0,"[%s] Obtained %d %s", Text, CoinC,gMessage.GetMessage(524));
	}
	// ----
	if( CoinP > 0 )
	{
		gNotice.GCNoticeSend(UserIndex,1,0,0,0,0,0,"[%s] Obtained %d %s", Text, CoinP,gMessage.GetMessage(525));
	}
	// ----
	if( CoinG > 0 )
	{
		gNotice.GCNoticeSend(UserIndex,1,0,0,0,0,0,"[%s] Obtained %d %s", Text, CoinG,gMessage.GetMessage(526));
	}
	if ( Credits > 0)
	{
		gNotice.GCNoticeSend(UserIndex,1,0,0,0,0,0,"[%s] Obtained %d %s", Text, Credits,gMessage.GetMessage(527));
	}
}
// -------------------------------------------------------------------------------