...
void CMove::Move(LPOBJ lpObj,int index) // OK
{
	...
	if((gGate.GetGateMap(MoveInfo.Gate) == MAP_ICARUS || gGate.GetGateMap(MoveInfo.Gate) == MAP_KANTURU3) && (lpObj->Inventory[7].IsItem() == 0 && lpObj->Inventory[8].m_Index != GET_ITEM(13,3) && lpObj->Inventory[8].m_Index != GET_ITEM(13,4) && lpObj->Inventory[8].m_Index != GET_ITEM(13,37))) // Dinorant,Dark Horse,Fenrir
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(228));
		return;
	}
	...
	if(gCustomArena.m_CustomArenaSwitch == 0 || CA_MAP_RANGE(gGate.GetGateMap(MoveInfo.Gate)) == 0 || gCustomArena.CheckEnterEnabled(lpObj,MoveInfo.Gate) != 0)
	{
