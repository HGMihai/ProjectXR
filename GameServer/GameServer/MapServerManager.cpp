...
short CMapServerManager::CheckMapServerMove(int aIndex,int map,short ServerCode) // OK
{
	...
	int region_code = gRegionSystem.get_server_code_by_name(gObj[aIndex].region);
	if (region_code != -1)
	{
		/*CMapServerData * lpDestMapSvrData = NULL;

		LogAdd(LOG_TEST,"Tru to find server data. Code = %d",sMAP_MOVE_INFO);
		EnterCriticalSection(&this->m_critical);
		...
		if (lpDestMapSvrData != NULL && lpDestMapSvrData->m_MapMove[map] != -3)*/
		if (sMAP_MOVE_INFO != gRegionSystem.get_cs_server_code())
		{
			LogAdd(LOG_TEST,"mapservermove region code = %d [map = %d]",region_code,sMAP_MOVE_INFO);
			//GJMapServerMoveSend(lpObj->Index,lpObj->NextServerCode,lpObj->Map,(BYTE)lpObj->X,(BYTE)lpObj->Y);
			return (short)region_code;
		}
	}
